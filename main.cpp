...
//Segmentation Params
int g_Segthresh = 30; //Image Threshold for FIsh Features
...
unsigned int trackImageSequencefiles(MainWindow& window_main)
{
    ...
    cv::createTrackbar( " Threshold:", gstrwinName + " FG Fish Mask", &g_Segthresh, 255, thresh_callback );
    thresh_callback( 0, 0 );
...
//      frame.copyTo(frameMasked,fgMask);
        ...
        frame.copyTo(frameMasked,fgMaskFish );
    ...
    for( size_t i = 0; i< contours.size(); i++ )
       {
         ...
         if (hull[i].size() > 5 )
         {
            ...
            for( int j = 0; j < 4; j++ )
                    line( maskedImg, featurePnts[j], featurePnts[(j+1)%4], colorFeature, 1, 8 );
         }

         cv::drawContours( maskedImg, hull, (int)i, CV_RGB(150,150,150), 1, 8, std::vector<cv::Vec4i>(), 0, cv::Point() );
       }
