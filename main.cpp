...
unsigned int trackImageSequencefiles(MainWindow& window_main)
{
    ...
    itfile.toFront();
    //QListIterator<QString> itfile (imgFiles); //Restart Iterator
    while (itfile.hasNext() && !bExiting)
    {
      QString filename = itfile.next();
      qDebug() << filename;
      ...
      // std::cout << " Now Processing : "<< itimgDir.fileName().toStdString() ;
       ...
       //cv::drawContours( frameMasked,fishBlob->contour, (int)0, CV_RGB(255,255,255), CV_FILLED, 8, std::vector<cv::Vec4i>(), 0, cv::Point() );
       /// Frame The Fish ///
...
//      frame.copyTo(frameMasked,fgMask);
        frameMasked = cv::Mat::zeros(frame.rows, frame.cols,CV_8UC3);
        cv::Mat m = cv::cvarrToMat(&framefishImg);

        frame.copyTo(frameMasked,m);
    ...
    for (std::vector<ltROI>::iterator it = vRoi.begin(); it != vRoi.end(); ++it)
    {
        ...
        for (cvb::CvBlobs::const_iterator it = blobs.begin(); it!=blobs.end(); ++it)
        {
            ...
            if (iroi.contains(pnt))
            {
                ...
                if (fishBlob == blob ) //Render Fish
                    cvb::cvRenderBlob(labelImg, blob, &fgMaskImg, &frameImg, CV_BLOB_RENDER_COLOR | CV_BLOB_RENDER_CENTROID|CV_BLOB_RENDER_BOUNDING_BOX , cv::Scalar(200,200,200),1);
                else  //Render Paramecium
                    cvb::cvRenderBlob(labelImg, blob, &fgMaskImg, &framefishImg, CV_BLOB_RENDER_CENTROID|CV_BLOB_RENDER_BOUNDING_BOX , cv::Scalar(0,200,0),0.4);
