...
//    cv::erode(fgMask,fgMask,kernelOpen, cv::Point(-1,-1),1);
    ...
    if (bTracking)
    {
       ...
       // Filters Blobs between fish and food - save into global vectors
        processBlobs(frame,fgMask, blobs,tracks,gstroutDirCSV,frameNumberString,dMeanBlobArea);
        ...
        //Show Tracks
        cvb::cvRenderTracks(tracks, &lplframeImg, &lplframeImg,CV_TRACK_RENDER_ID | CV_TRACK_RENDER_PATH,&trackFnt);
...
/// \return
///
int processBlobs(cv::Mat& fullsrcimg,cv::Mat& maskimg,cvb::CvBlobs& blobs,cvb::CvTracks& tracks,QString outDirCSV,std::string& frameNumberString,double& dMeanBlobArea)
{
...
///  REGION OF INTEREST - UPDATE - SET
     lplframeImg        =  fullsrcimg; //Convert The Global frame to lplImage
     //fgMaskFish       =
     framefishMaskImg   = (IplImage)cv::Mat::zeros(fullsrcimg.rows, fullsrcimg.cols,CV_8UC3); //Empty Canvas For Just Fish Blob
   ...
   //std::cout << "Roi Sz:" << vRoi.size() <<std::endl;
    labelImg=cvCreateImage(cvGetSize(&lplframeImg), IPL_DEPTH_LABEL, 1);
    ...
    for (std::vector<ltROI>::iterator it = vRoi.begin(); it != vRoi.end(); ++it)
    {
        ...
        for (cvb::CvBlobs::const_iterator it = fishblobs.begin(); it!=fishblobs.end(); ++it)
        {
            ...
            if (iroi.contains(pnt))
            {
                //cnt++; //CV_BLOB_RENDER_COLOR
                    cvb::cvRenderBlob(labelImg, blob, &lplfgMaskImg, &lplframeImg, CV_BLOB_RENDER_ANGLE | CV_BLOB_RENDER_BOUNDING_BOX, CV_RGB(250,10,10),1);
        ...
        for (cvb::CvBlobs::const_iterator it = foodblobs.begin(); it!=foodblobs.end(); ++it)
        {
            ...
            if (iroi.contains(pnt))
            {
                    cvb::cvRenderBlob(labelImg, blob, &lplfgMaskImg, &lplframeImg,CV_BLOB_RENDER_COLOR | CV_BLOB_RENDER_CENTROID|CV_BLOB_RENDER_BOUNDING_BOX ,CV_RGB(0,10,200),1.0);
            }
        }
    ...
    {
        saveImage(frameNumberString,outDirCSV,fullsrcimg);
...
void detectZfishFeatures(cv::Mat& fullImg, cv::Mat& maskfishFGImg, std::vector<std::vector<cv::Point> >& contours_body,std::vector<cv::Vec4i>& hierarchy_body)
{
    ...
    //maskedImg_gray.copyTo(maskedfishImg_gray,maskImg); //Mask The Laplacian //Input Already Masked
