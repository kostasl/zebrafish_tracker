...
class trackerState
{
 ...
 void loadState(std::string strFilename);
  ...
  int g_SegInnerthreshMult; //Image Threshold for Inner FIsh Features //Deprecated
  int g_BGthresh; //BG threshold segmentation
  float gfVidfps;
  const  unsigned int MOGhistory = 100;//Use 100 frames Distributed across the video length To Find What the BGModel is
  double gdMOGBGRatio; ///If a foreground pixel keeps semi-extern const ant value for about backgroundRatio*history frames, it's considered background and added to the model as a center of a new component.

  int gi_ThresholdMatching; /// Minimum Score to accept that a contour has been found
  bool gOptimizeShapeMatching; ///Set to false To disable matchShapes in FindMatching Contour
  int gnumberOfTemplatesInCache; //INcreases As new Are Added
  float gDisplacementThreshold; //Distance That Fish Is displaced so as to consider active and Record A point For the rendered Track /
  int gFishBoundBoxSize; /// pixel width/radius of bounding Box When Isolating the fish's head From the image
  int gFishTailSpineSegmentLength;

  double gUserReward; //User Given reward via GUI



  int giEyeIsolationMaskRadius; //Radius of circle masking between eyes
  ///Fish Features Detection Params
  int gFishTemplateAngleSteps;
  int gEyeTemplateAngleSteps;
  double eyeStepIncrement;
  double gTemplateMatchThreshold; //If not higher than 0.9 The fish body can be matched at extremeties
  int iLastKnownGoodTemplateRow;
  int iTemplateMatchFailCounter;
  int iFishAngleOffset; //Manual User Corrected Fish Angle added to detected one
    //using namespace std;

  int iEyeMaskSepWidth; //Line width separating the eyes in the Head Image
 ...
 //bool bSaveBlobsToFile; //Check in fnct processBlobs - saves output CSV
  ...
  ///Segmentation Params
 int gi_CannyThres;
 int gi_CannyThresSmall; //Aperture size should be odd between 3 and 7 in function Canny
 int gi_maxEllipseMajor; /// thres  for Eye Ellipse Detection methods
 int gi_minEllipseMajor; ///thres for Eye Ellipse Detection methods (These Values Tested Worked Best)
 int gi_minEllipseMinor; //The threshold in ellipse detection for how slim an ellipsoid can be
 int gi_VotesEllipseThres; //Votes thres for The Backup Ellipse Detection Based on the Hough Transform
 int gi_MaxEllipseSamples; //The number of fitted ellipsoids draw from the ranked queue to calculate mean fitted eye Ellipse
 int gthresEyeSeg; //Central Threshold For Eye Segmentation In Isolated Head IMage
 int gthresEyeSegL; //Lower Eye Seg Threshold

  int g_Segthresh; //Image Threshold to segment BG - Fish Segmentation uses a higher 2Xg_Segthresh threshold
  int g_SegFoodThesMax; //Scan Thresholds to detect Food items in
  int g_SegFoodThesMin;
