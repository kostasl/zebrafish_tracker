...
\[
\frac{d\theta}{dr}=\frac{x}{r^2cos(\theta)}
\]
...
```{r}
## Function to Compute the derivative of Prey Azimuth as distance to prey changes for a prey located preyspeed*PauseTime Off Midline axis 
aimDeflectionAngle<- function(preyspeed,boutInterval,captureDistance,PreyAzimuth)
{beta <- preyspeed*boutInterval
  return(beta/((captureDistance^2)*sqrt(1-  (beta^2)/captureDistance^2) ))
}


preyspeed = 0.25 ##Mm/sec
boutInterval = 0.200
captureDistanceNear = 0.25 ##mm
captureDistanceFar = captureDistanceNear*2 ##mm
PreyAzimuth= 0 ##Larva's Initial Aiming angle prior to Prey Moving 


print(paste0("dtheta/dr at small distance ",captureDistanceNear,"mm : ",aimDeflectionAngle(preyspeed,boutInterval,captureDistanceNear,PreyAzimuth)*(180/pi)) )
thetaNear <- asin(preyspeed*boutInterval/(captureDistanceNear) ) *180/pi
DthetaNear <- (asin(preyspeed*boutInterval/(captureDistanceNear) ) - asin(preyspeed*boutInterval/(captureDistanceNear-captureDistanceNear*0.01) ))*180/pi
print(paste0( "Angle theta at Near Distance: ", asin(preyspeed*boutInterval/(captureDistanceNear) ) *180/pi ) )
print(paste0("dtheta/dr at 2x distance ",captureDistanceFar,"mm : ", aimDeflectionAngle(preyspeed,boutInterval,captureDistanceFar,PreyAzimuth)*(180/pi) ) )
print(paste0("Angle theta at Far Distance: ", asin(preyspeed*boutInterval/(captureDistanceFar))*180/pi ) );

## Plot derivatives Across different R, for fixed prey Speed
preyspeed = 0.00 ##Mm/sec
#r <- seq((preyspeed*boutInterval),(preyspeed*boutInterval*5),by=1/20) ##Use Speed to Relate to R
r <- seq((0.2),0.6,by=1/20) ##Use Speed to Relate to R
s <- seq((preyspeed),1,by=1/10)
mp <- expand.grid(r=r,preyspeed=s)
for(i in  1:NROW(mp) )
{
  y[i] <- aimDeflectionAngle((mp[i,]$preyspeed),boutInterval,(mp[i,]$r),PreyAzimuth)*(180/pi)  
}

## Plot the derivative across different r prey distances for various prey offsets
plot(mp$r,y,ylim=c(0,500),xlab="Prey distance mm",ylab="dθ/dr speed of azimuth change" )
## Faster Prey Speed
ySlow <- aimDeflectionAngle(s[1],boutInterval,r,PreyAzimuth)*(180/pi) 
## Plot the derivative across different r prey distances for fixed B
lines(r,ySlow,ylim=c(0,500),col="blue" )
yFast <- aimDeflectionAngle(tail(s,1),boutInterval,r,PreyAzimuth)*(180/pi) 
## Plot the derivative across different r prey distances for fixed B
lines(r,yFast,ylim=c(0,500),col="red" )
