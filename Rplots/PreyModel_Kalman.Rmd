...
##This simply calculates the change in Prey Azimuth Between two positions defined prior and After BoutInterval, given prey Speed
...
print(paste0("Change in theta at FAR Dist: ", DeltaAimingAngleAgainstPreyMotion(preyspeed,boutInterval,captureDistanceFar,PreyAzimuth)*(180/pi) ) );
print(paste0("Change in theta at NEAR Dist: ", DeltaAimingAngleAgainstPreyMotion(preyspeed,boutInterval,captureDistanceNear,PreyAzimuth)*(180/pi) ) );

## Plot derivatives Across different R, And prey Speeds
preyspeedR = range(0.01,0.5) ##Mm/sec
...
s <- seq(preyspeedR[1],preyspeedR[2],by=preyspeedR[2]/10) ##Prey Speed Range
...
for(i in  1:NROW(mp) )
{
  ...
  DrDb <- (sqrt(Db^2+mp[i,]$r^2)-mp[i,]$r)/Db ##Get Change In R, due to Dbeta - R Increases with Db
  ##Estimate Angle Change Using PartDeriv, assuming Change in Dr 
  y[i] <- (180/pi)*(dthetadr*DrDb*0 + dthetadb)*Db ##*cos(PreyAzimuth
  DeltaTheta[i] <- DeltaAimingAngleAgainstPreyMotion((mp[i,]$preyspeed),boutInterval,(mp[i,]$r),PreyAzimuth)*(180/pi)
}

paletta <- colorRampPalette(rev(brewer.pal(8,'Spectral')));
colSp <- c(paletta(NROW(s)),"#FF0000");
### Sampled DeltaTheta from Positions Geometry
plot(mp$r,DeltaTheta,xlab="Initial prey distance r (mm)",ylab="Change in azimuth due to prey motion",xlim=c(0.0,0.6),pch=2,cex.axis=cex,cex=0.7,ylim=c(0.0,80) ) #
...
##Plot Heat Map Legend
    x <- tail(r,1)/2+(1:NROW(s)/130) ##Make Narrow 1/2 length bar
    points(x,rep(60,NROW(x) ),pch=15,col=colSp,cex=1.5)
    text( x[1]+(x[1]-tail(x,1))/2 +strwidth("Prey speed")*0.8  ,64,labels="Prey speed",col="black",font=2.2)  ##0 V Angle
