...
table(datPrey$FoodID)
datPreyItem <- datPrey[datPrey$FoodID == "54",]
...
NoisePoints = N/100
NoiseX = cbind(round(runif(n=NoisePoints,min=1,max=N)), rnorm(n=NoisePoints,mean=5,sd=10))
NoiseY = cbind(round(runif(n=NoisePoints,min=1,max=N)), rnorm(n=NoisePoints,mean=5,sd=10))
...
datFilteredY = g_h_filter(datBrokenY,x0,dx,g,h,dt)
...
plot(datPreyItem$Centroid_X,datPreyItem$Centroid_Y)
lines(datBrokenX,datBrokenY,lty=2,col="grey",lwd=1)
lines(datFilteredX,datFilteredY,lty=2,col="green",lwd=2)
##Combined
```
