...
## Plot CDF ##
strPlotName = paste(strPlotExportPath,"/stat/efficiency/ecdf_huntpower.pdf",sep="")
pdf(strPlotName,width=8,height=8,title="Compare Hunt Power Between Groups",onefile = TRUE) #col=(as.integer(filtereddatAllFrames$expID))
 
plot(cdfDLScore,lty=2,lwd=1,col=colourH[1],xlim=c(0,12),xlab=expression(S^2/(S+F),paste("")),cex.axis=1.5 ,main="Hunt Power ECDF",pch=pchL[1])
plot(cdfLLScore,add=T,lty=1,lwd=2,col=colourH[2],pch=pchL[2])
plot(cdfNLScore,add=T,lty=1,lwd=2,col=colourH[3],pch=pchL[3])
