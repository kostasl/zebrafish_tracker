...
detectHuntEvents <- function(datAllFrames,vexpID,vdatasetID)
{
  ...
  {
    idx = idx+1;
    datLarvaFrames <- datAllFrames[which(datAllFrames$expID == i),]
    ...
    groupID <- unique((datLarvaFrames$group))
    larvaID = unique((datLarvaFrames$larvaID))
    
    ##Filter To Keep Only data in range ###
    datLarvaFrames <- datLarvaFrames[which(datLarvaFrames$REyeAngle > -35 & datLarvaFrames$REyeAngle <  35 & datLarvaFrames$LEyeAngle > -35 & datLarvaFrames$LEyeAngle <  35),]
    ...
    for (k in vEventID)
    {
      ...
      if  (NROW(datHuntFrames$frameN) > G_MINEPISODEDURATION)
      {
        ...
        if (length(unique(datHuntFrames$fileIdx) ) > 1)
        {
          ...
          message(paste("Duplicate Track File for same Event - check CSVs for expID:",i," event",k, " fileIds:",unlist(unique(datHuntFrames$fileIdx) ) )  )
