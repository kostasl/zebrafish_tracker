...
```{r  analyse bout structure within domains, fig.show="hold",fig.cap="Distribution of free paths (Straight runs before turning)", out.width="100%", echo=FALSE, cache=TRUE, results=FALSE}
  ...
  freePathDat <- list()
  freePathDat[["LL"]] <- ldatFreePathD[[cD ]][ldatFreePathD[[cD ]]$groupID == "LL" & ldatFreePathD[[cD ]]$travelDistance_mm < 30,]
  freePathDat[["LE"]] <- ldatFreePathD[[cD ]][ldatFreePathD[[cD ]]$groupID == "LE" & ldatFreePathD[[cD ]]$travelDistance_mm < 30,]
  
  freePathDat[["NL"]] <- ldatFreePathD[[cD ]][ldatFreePathD[[cD ]]$groupID == "NL" & ldatFreePathD[[cD ]]$travelDistance_mm < 30,]
  freePathDat[["NE"]] <- ldatFreePathD[[cD ]][ldatFreePathD[[cD ]]$groupID == "NE" & ldatFreePathD[[cD ]]$travelDistance_mm < 30,]
  
  hist(unlist(freePathDat[["LL"]]$travelDistance_mm), breaks=seq(-1,30,1),ylim=c(0,700) )
