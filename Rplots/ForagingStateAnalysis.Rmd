...
saveRDS(results.NF.E,file=paste0(strDataExportDir,'jagsModelResults_clustDispersion_NL.rds') )
...
saveRDS(results.LF.E,file=paste0(strDataExportDir,'jagsModelResults_clustDispersion_LL.rds') )
...
saveRDS(results.DF.E,file=paste0(strDataExportDir,'jagsModelResults_clustDispersion_DL.rds') )
...
saveRDS(results.NF.S,file=paste0(strDataExportDir,'jagsModelResults_clustDispersion_NE.rds') )
...
saveRDS(results.LF.S,file=paste0(strDataExportDir,'jagsModelResults_clustDispersion_LE.rds') )
...
saveRDS(results.DF.S,file=paste0(strDataExportDir,'jagsModelResults_clustDispersion_DE.rds') )
...
```{r dispersion-clustering-results,fig.show="hold", out.width="33%", eval=TRUE, include=FALSE}
...
plotClusterModel <- function(results,vDispersion,groupID)
{
  ...
  clustID <- round(colMeans(custcoeff[[ichain]]))

    #hist( mucoeff[[3]],xlim=c(0,10), breaks=20,col=colourDataScheme[[groupID]],main=groupID,xlab = "Sample Means (mm)" )  
  tauClustAllChain <- as.data.frame(tauOfClustcoeff[[ichain]])#rbind( as.data.frame(tauOfClustcoeff[[3]]),as.data.frame(tauOfClustcoeff[[2]]),as.data.frame(tauOfClustcoeff[[1]]))
  muClustAllChain <- as.data.frame(muOfClustcoeff[[ichain]])#rbind( as.data.frame(muOfClustcoeff[[3]]),as.data.frame(muOfClustcoeff[[2]]),as.data.frame(muOfClustcoeff[[1]]) )
  ...
  #plot(density(vDispersion[clustID == 1]),xlim=c(0,10), col=clustCol[1], main=groupID, xlab = "Dispersion (mm)",lwd=3,lty=1)
  #lines(density(vDispersion[clustID == 2]),xlim=c(0,10), col=clustCol[2],lwd=3,lty=2)

  breaksSlots <-  seq(0,12,by=0.15)
  list_histo <- hist(vDispersion, breaks=breaksSlots, freq=TRUE)
  hist(vDispersion[clustID == 1],xlim=c(0,10),ylim=c(0,max(list_histo$counts)), breaks=breaksSlots,col=clustCol[1],main=paste("Clustered densities", groupID ),xlab = "Dispersion (mm)",add=TRUE)
  hist(vDispersion[clustID == 2],xlim=c(0,10), breaks=breaksSlots,col=clustCol[2],main=groupID,xlab = NA,add=TRUE)  
  ...
  list_histo <- hist(pClustcoeff[[ichain]],breaks=breaksProb,main=groupID,freq = TRUE)
  hist( pClustcoeff[[ichain]][,1],xlim=c(0,1),ylim=c(0,max(list_histo$counts)), breaks=breaksProb,col=clustCol[1],xlab = "Prob. of cluster membership (pClust)" ,add=TRUE)
  hist( pClustcoeff[[ichain]][,2],xlim=c(0,1), breaks=breaksProb,col=clustCol[2],main=groupID,add=TRUE )

}

## Load Clustering Results
if (!exists("results.NF.E"))
  results.NF.E <<- readRDS(file=paste0(strDataExportDir,'jagsModelResults_clustDispersion_NL.rds') )
if (!exists("results.NF.S"))
  results.NF.S <<- readRDS(file=paste0(strDataExportDir,'jagsModelResults_clustDispersion_NE.rds') )
if (!exists("results.LF.E"))
  results.LF.E <<- readRDS(file=paste0(strDataExportDir,'jagsModelResults_clustDispersion_LL.rds') )
if (!exists("results.LF.S"))
  results.LF.S <<- readRDS(file=paste0(strDataExportDir,'jagsModelResults_clustDispersion_LE.rds') )
if (!exists("results.DF.E"))
  results.DF.E <<- readRDS(file=paste0(strDataExportDir,'jagsModelResults_clustDispersion_DL.rds') )
if (!exists("results.DF.S"))
  results.DF.S <<- readRDS(file=paste0(strDataExportDir,'jagsModelResults_clustDispersion_DE.rds') )
