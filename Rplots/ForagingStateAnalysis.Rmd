...
```{r  bout and turn structure , fig.show="hold",fig.cap="Bout Frequency and free paths", out.width="100%", echo=FALSE, cache=TRUE, results=FALSE}
 ...
 #### PROCESS BOUTS ###
  ...
  vPathLength_mm          <- cumsum(vEventSpeed_smooth_mm)
  ...
  for (tID in as.character(datTrackletTurns[abs(datTrackletTurns$turnMagnitude) >  5, ]$turnID) )
  {
  ...
  datDomainTrajectory$IndTurnBout <- 0
  if (length(TurnboutsIdx))
    datDomainTrajectory[TurnboutsIdx,]$IndTurnBout <- 1
  ...
  motionMagnitude <- vPathLength_mm[(motionEndFrame) - as.numeric(TrackLetInfo$startFrame)-1   ] - vPathLength_mm[(motionStartFrame) - as.numeric(TrackLetInfo$startFrame)  +1 ]
  datTrackletMotion <- data.frame(MotionID=names(motionEndFrame),motionStartFrame,motionEndFrame,motionMagnitude)
  datTrackletMotion <- datTrackletMotion[!is.na(datTrackletMotion$motionMagnitude), ]
  
  # Reset Indicators of Motion - On Each Frame
  datDomainTrajectory$IndMotionBout <- 0
  for (tID in as.character(datTrackletMotion$MotionID) )
  {
      message(tID,":",datTrackletMotion[tID,]$turnMagnitude)
    
      datDomainTrajectory[datDomainTrajectory$frameN >= datTrackletMotion[tID,]$motionStartFrame &
                        datDomainTrajectory$frameN <= datTrackletMotion[tID,]$motionEndFrame,"IndMotionBout"] = 1
       # plot(vAngleDisplacement[datTrackletTurns[tID,]$turnStartFrame:datTrackletTurns[tID,]$turnEndFrame] - as.numeric(TrackLetInfo$startFrame)  )
  }
  
  plot(datDomainTrajectory$posX,datDomainTrajectory$posY,col=c("black","red","purple")[datDomainTrajectory$IndMotionBout+1] )  
