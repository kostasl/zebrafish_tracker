...
#  datMoveIds <- ldply (blockMotionBoutFramesIDs, data.frame)
  ...
  boutStartFrame <- sapply((boutStartFrame) - as.numeric(TrackLetInfo$startFrame),max,1) + as.numeric(TrackLetInfo$startFrame) ##Fix Range TO Start form 1
  boutEndFrame <- tapply(datBoutBlockIds$frameN,datBoutBlockIds$boutID,max)
  boutEndFrame <- sapply((boutEndFrame) - as.numeric(TrackLetInfo$startFrame),max,1)+as.numeric(TrackLetInfo$startFrame) ##Fix Range TO Start form 1
  turn_deg <- vAngleDisplacement[(boutEndFrame) - as.numeric(TrackLetInfo$startFrame) ] - vAngleDisplacement[(boutStartFrame) - as.numeric(TrackLetInfo$startFrame)  ]
  pathLength_mm <- vPathLength_mm[(boutEndFrame) - as.numeric(TrackLetInfo$startFrame)] - vPathLength_mm[(boutStartFrame) - as.numeric(TrackLetInfo$startFrame)  ]
  ...
  for (i in 1:NROW(boutEndFrame))
  {
    ...
    vDeltaYFrames[i]        <- datDomainTrajectory[datDomainTrajectory$frameN == boutEndFrame[i],]$posY - datDomainTrajectory[datDomainTrajectory$frameN == boutStartFrame[i],]$posY 
  }
  ...
  plot(datDomainTrajectory$posX,datDomainTrajectory$posY,
       ...
       pch =c(25,20,3)[(datDomainTrajectory$BoutFlag>0)+1],
       cex=c(1,1,1.2)[(datDomainTrajectory$BoutFlag>0)+1] )  
  ...
  {   
      ...
      for (strGroup in names(lBoundedTrackInfo) )
      {
        ...
        TrackLetInfo <- lBoundedTrackInfo$LL["228-1-17",] 
...
plotBoundedPathTurns <- function(turnsPerPath,strG,addPlot=FALSE)  
{
 ...
 ##Turn Magnitude
  for (strG in vGroups)
  {
    datGTurns<- datTurnsPerDomain[datTurnsPerDomain$groupID == strG & abs(datTurnsPerDomain$turnMagnitude) < 180,] 
    hist(datGTurns$turnMagnitude,breaks=seq(-180,180,2),main=paste(strG,"R",DomainRadius) ) #ylim=c(0,80)
  }

```

*I do see an apparent difference in turn-frequency distributions or the distribution of free-path lengths!* within the paths in crossing a bounded circular domain.
But that is strange cause the groups seem to macroscopically behave differently.
