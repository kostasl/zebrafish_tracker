...
plotLogLogDistribution_MSD <- function(lLinModel_GE,addPlot=FALSE)
{
    ...
    if (!addPlot) 
    {
      plot(lLinModel_GE$xtick,lLinModel_GE$logDistribution,
         ...
         xlim=c(min(lLinModel_GE$xtick),max(lLinModel_GE$xtick)*1.01),
         ylim=c(-4,3),
         ylab="log N(l) %",xaxt="n" )
...
makeMSDPathDurationSummary <- function()
{
  ...
  saveRDS(lMSDSummaryData,paste0(strDataExportDir,"MSDPerPathDurationSummaryResults-1-180sec.rds"))
...
vtimeWindows <- c(vsmallTimeWindows,80,100,120,180)# c(1,2,4) # vsmallTimeWindows##c(4,10,20,80,200,260,360)
...
makeMSDPathLengthSummary <- function ()
{
  ...
  saveRDS(lMSDStat,paste0(strDataExportDir,"lMSDPerPathLength2mmSummaryResults1-60sec.rds"))
  saveRDS(datmeanMSDPerPathOft,paste0(strDataExportDir,"datMSDPerPathLength2mmSummaryResults1-60sec.rds"))
...
try(
{datMSDVsPathlengthSummaryData <<- readRDS(paste0(strDataExportDir,"datMSDPerPathLength2mmSummaryResults1-60sec.rds"));
lMSDStat <<- readRDS(paste0(strDataExportDir,"lMSDPerPathLength2mmSummaryResults1-60sec.rds"))
}
)
if (!exists("lMSDStat"))
{
...
for (strG in vGroups)
{
  ...
  if (!baddPlot)
  {
    plot(xtick, log10(datmeanMSDPerPathOft[[strG]]),col=colourDataScheme[[strG]],pch=pointTypeScheme[[strG]],lty=lineTypeScheme[[strG]],type="b",
         xlab="Path length (mm)",ylab="log Mean Squared Displacement (mm)",xaxt="n",yaxt="n" ,ylim=c(0.2,2))
  
    axis(side=1, at=xtick, labels = names(lMSDStat[[strG]]$`2`$breaks))
    axis(side=2, at=log10(axisTicks(c(0.3,2),log=TRUE,nint=5)), labels = axisTicks(c(0.3,2),log=TRUE,nint=5) )  
  }
  else
    points(xtick,log10(datmeanMSDPerPathOft[[strG]]),col=colourDataScheme[[strG]],pch=pointTypeScheme[[strG]],lty=lineTypeScheme[[strG]],type="b" )
  
  baddPlot <- TRUE
}
  ##Add Theoretical MSD growth vs Time
  propFactor <- 1.0
  lines(xtick ,log10(theoreticalMeanLength(xtick+2.0,propFactor )),pch=18,cex=0.5,type="b" ) #Up Lim
