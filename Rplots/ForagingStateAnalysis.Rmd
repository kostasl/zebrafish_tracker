...
plotClusterProb <- function(lmcmcRes,clustCol,groupIDOffset=1,clustIDOffset=6)
{
...
datTestSubDispersion <- datSubDispersion[sample(1:NROW(datSubDispersion),size=2500),] 

## Lastly / Model and Cluster All Data so we can then run a relative comparison between groupscompare
results.All <- runNBMixGroupClusterModel(datTestSubDispersion$Dispersion,as.numeric(datTestSubDispersion$groupID))
write.jagsfile(results.All, file=paste0(strDataExportDir,'jagsModel_NBMixGroupclustDispersion_All.txt') )
save(list=c("results.All","datTestSubDispersion"),file=paste0(strDataExportDir,'jagsModelResults_NBMixGroupclustDispersion_All.RData') )
...
plotClusterModel(results.All,datTestSubDispersion$Dispersion, "All",getMCMCNBGroupEstimatedParams(results.All,"All",2))
```
...
plotGroup<- which(levels(datTestSubDispersion$groupID)==lparamsAll$groupID)
