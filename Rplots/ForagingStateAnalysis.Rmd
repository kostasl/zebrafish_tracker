...
nsubsample <- 50000
vDispersion.LF <- sample(datDispersion[datDispersion$groupID == 'LL' & !is.na(datDispersion$Dispersion),"Dispersion",],nsubsample)

##Cluster Membership
N = NROW(vDispersion.LF)
Nclust <- 2
clust = rep(NA,N) 
clust[which.min(vDispersion.LF)]=1 # smallest value assigned to cluster 1
clust[which.max(vDispersion.LF)]=2 # highest value assigned to cluster 2 
...
results_LF <- run.jags(model=strDispersionClusterModel, n.chains=2,
                       inits=initfunct(nchains,dataList_LF$N),
                       thin=nthin,
                       sample=steps,
                       data= dataList_LF,
                       monitor=str_vars,
...
plot(results_LF, c('trace','histogram','ecdf','crosscorr','key'),
vars=c("mu","tau","clust","pClust"),layout=c(3,3))

# Extract only the coefficient as an mcmc.list object:
library('coda')
mucoeff <- as.mcmc.list(results_LF, vars="mu")
pClustcoeff <- as.mcmc.list(results_LF, vars="pClust")
custcoeff <- as.mcmc.list(results_LF, vars="clust")

clustID <- sapply(custcoeff[[1]][1,1:500],mean)

hist(dataList_LF$y[clustID == 1],xlim=c(0,10), breaks=20,col="red")
hist(dataList_LF$y[clustID == 2],xlim=c(0,10), breaks=20,col="blue",add=TRUE)

# Write the current model representation to file:
write.jagsfile(results, file=paste0(strDataExportDir,'jagsModel_clustDispersion_LF.txt') )
