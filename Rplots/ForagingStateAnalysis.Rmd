...
calcPathLengthsInDomain <- function(datAllFrames,vgroupID)
{
  ...
  for (strTrackID in names(pathDurations) )
  {
        ...
        TrackletInfo <- list(expID = vTrackID[[1]][1],EventID=vTrackID[[1]][2],TrackletID=vTrackID[[1]][3] )
        TrackletInfo$startFrame <- entryFramePerLarvaEvent[[strTrackID]]
        TrackletInfo$endFrame <-  exitFramePerLarvaEvent[[strTrackID]]
   
        datBoundedTracklet <- datDomainTrajectories[datDomainTrajectories$expID == TrackletID$expID &
                                                            datDomainTrajectories$eventID == TrackletID$EventID &
                                                            datDomainTrajectories$trackletID == TrackletID$TrackletID &
                                                            datDomainTrajectories$frameN >= TrackletID$startFrame &
                                                            datDomainTrajectories$frameN <= TrackletID$endFrame,
                                                            ]
             
        ## Measure Path Length - Can Skip Some Frames To Speed Up Calculation
        nSpace <- 3
        vIdx <- seq(from=1,by=nSpace,to=pathDurations[[strTrackID]]) ##Last nfrm
...
for(g in vGroups)
  plotpathDurationInDomain(datAllFrames, g)
