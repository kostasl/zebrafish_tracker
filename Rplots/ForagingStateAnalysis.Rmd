...
{
  datGroupTraj <- datAllFrames[datAllFrames$groupID %in% vgroupID & datAllFrames$posX > 0,]
  ...
  continuousTrackletsFrames <- split(datDomainTrajectories$frameN    ,cumsum(c(1, datDomainTrajectories$DnextFrame != 1)) ) #<- Label Consecutive Groups
  for (Tuid in names(continuousTrackletsFrames))
      datDomainTrajectories[datDomainTrajectories$frameN %in% continuousTrackletsFrames[[Tuid]],"TrackletUID" ] <- Tuid
  ...
  datBoundaryDomainTrajectories <- datDomainTrajectories[datDomainTrajectories$DnextFrame < 3 & datDomainTrajectories$distToDomainBoundary < 0.3,]

    ##Find Mean Path Duration From Entry to Exit Of the domain
  entryFramePerLarvaEvent <- tapply(datBoundaryDomainTrajectories$frameN,
                                    paste(datBoundaryDomainTrajectories$expID,datBoundaryDomainTrajectories$eventID,datBoundaryDomainTrajectories$trackletID,datBoundaryDomainTrajectories$TrackletUID,sep="-" ),min )
  exitFramePerLarvaEvent <- tapply(datBoundaryDomainTrajectories$frameN,
                                   paste(datBoundaryDomainTrajectories$expID,datBoundaryDomainTrajectories$eventID,datBoundaryDomainTrajectories$trackletID,datBoundaryDomainTrajectories$TrackletUID,sep="-"),max )
  ...
  lPathDistanceTravelled <- list()
  lTrackletInfo <- list()
  
  ##Calc Path Length for Each Tracklet Identified
  for (strTrackID in names(pathDurations) )
  {
        message("Path length for:",strTrackID)
        vTrackID <- strsplit(strTrackID,"-")
        
        TrackletInfo <- list(expID = vTrackID[[1]][1],EventID=vTrackID[[1]][2],TrackletID=vTrackID[[1]][3],sectionID=vTrackID[[1]][4], DomainRadius = rDomainRadius*DIM_MMPERPX)
        ...
        datBoundedTracklet <- datDomainTrajectories[datDomainTrajectories$expID == TrackletInfo$expID &
                                                            ...
                                                            datDomainTrajectories$trackletID == TrackletInfo$TrackletID &
                                                            datDomainTrajectories$frameN >= TrackletInfo$startFrame &
                                                            datDomainTrajectories$frameN <= TrackletInfo$endFrame,
                                                            ]
             
        
        TrackletInfo$duration <- pathDurations[[strTrackID]]/as.integer(unique(datBoundedTracklet$fps))
        
        ## Measure Path Length - Can Skip Some Frames To Speed Up Calculation
        nSpace <- as.integer(unique(datBoundedTracklet$fps)) / 3
...
hist(unlist(lBoundedTrackInfo[["LL"]]$pathLength),breaks=seq(0,300,10),main=mean(unlist(lBoundedTrackInfo[["LL"]]$pathLength)))
hist(unlist(lBoundedTrackInfo[["LE"]]$pathLength),breaks=seq(0,300,10),main=mean(unlist(lBoundedTrackInfo[["LE"]]$pathLength)) )

hist(unlist(lBoundedTrackInfo[["NL"]]$pathLength),breaks=seq(0,300,10),main=mean(unlist(lBoundedTrackInfo[["NL"]]$pathLength)) )
hist(unlist(lBoundedTrackInfo[["NE"]]$pathLength),breaks=seq(0,300,10),main=mean(unlist(lBoundedTrackInfo[["NE"]]$pathLength)) )



plot(lBoundedTrackInfo[["LL"]]$pathLength,lBoundedTrackInfo[["LL"]]$)
