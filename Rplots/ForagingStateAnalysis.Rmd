...
# Analysis of trajectory measures
...
## Path length per dispersion size (Invariance)
...
Normally the mean path-length for the entry and exit from a circle of size $r$ is measured. Here I have path-lengths whose 2-most distal points  fall on the Dispersal circle-boundary.
Nevertheless, I may nevertheless examine if such an *invariance principle holds* (see)[https://iopscience.iop.org/article/10.1209/epl/i2003-00208-x/meta], to see if the mean path length with a dispersion $r$ is only proportional to the Area/Circumfarence ratio for a random walk. 
For this, I look at mean path lengths for a set of fixed dispersion sizes:
...
plot_meanPathLengthPerDispersion <- function(datDispersion,strGroupID,addPlot=FALSE)
{
  ...
  if (!addPlot)
  {
    plot(seq(1,10.5,length.out = NROW(vmeanPathLengthPerDispersion)),
         vmeanPathLengthPerDispersion,pch=16,main=strTitle,xlab="Circle radius / Dispersion (mm)",
         ylim=c(0,45),xlim=c(0,11),col=colourDataScheme[[strGroupID]],cex=0.7 )
  }else
  {
...
```{r  mean-squared-distance over log-log distribution , fig.show="hold", out.width="50%", echo=FALSE, cache=TRUE }
...
getLogLogDistributions_MSDAndDispersion <-function(datDispersion,strG,addPlot=FALSE,powerLawRange = 100)
{
    ...
    #max(datDispersion$MSD,na.rm=TRUE)+15
    xbreaks_MSD <-seq(0,350,15)
    xbreaks_Dispersion <-seq(0,12,0.5)
    
    #strDispRange <-levels(datGroupDispersion$dispRange)[dispRange]
    #print(strDispRange) 
    if (NROW(datGroupDispersion[ ,"MSD"]) == 0)
    {
      warning(paste("No rows for MSD ", strG) )
      return(list(meanMSD=NA,groupID=strG,coeff=NA,ptx_line=NA,pty_line=NA))
    }
    h_MSD_GE <- hist(datGroupDispersion[ ,"MSD"], breaks=xbreaks_MSD,plot=FALSE)
    ...
    lLinModel_GE$sdMSD <- sd(datGroupDispersion[ ,"MSD"],na.rm=TRUE) ##Add Mean MSD for timeWindow
    lLinModel_GE$countMSD <- NROW(datGroupDispersion[!is.na(datGroupDispersion$MSD),]) ##Add Mean MSD for timeWindow
    ##Dispersion Stat
    ...
    lLinModel_GE$meanDispersion <- mean(datGroupDispersion[ ,"Dispersion"],na.rm=TRUE) ##Add Mean MSD for timeWindow
    lLinModel_GE$sdDispersion <- sd(datGroupDispersion[ ,"Dispersion"],na.rm=TRUE) ##Add Mean MSD for timeWindow
    lLinModel_GE$countDispersion <- NROW(datGroupDispersion[!is.na(datGroupDispersion$Dispersion),]) ##Add Mean MSD for timeWindow
...
plotLogLogDistribution_MSD <- function(lLinModel_GE,addPlot=FALSE)
{
...
plotMSDPerTime <- function(lModelwithMSD,addPlot=FALSE)
{
  ...
  {
    if (!is.null(lModelwithMSD[[i]]$meanMSD))
    {
  ...
  if (!addPlot)
  {
    plot(vTimeWindow[idxOrder],vMSD[idxOrder],xlim=xLim,ylim=c(0,100),
...
processMSDPerIntegrationTime <- function(ldatDispersion,strG,lLinModel,bAddPlot = FALSE)
{

  if (is.null(lLinModel[[strG]] ) ) ##list per twindow integration for this group
  {
      lLinModel[[strG]] <- list()
  }
  ...
  for (tt in names(ldatDispersion))
  {
    ...
    lLinModel[[strG]][[as.character(tt)]] = getLogLogDistributions_MSDAndDispersion(ldatDispersion[[tt]],strG,bAddPlot)
    bAddPlot <- TRUE
  }
...
makeMSDPathDurationSummary <- function()
{
  ...
  saveRDS(lMSDSummaryData,paste0(strDataExportDir,"statPerPathDurationSummaryResults-1-180sec.rds"))
...
vsmallTimeWindows <- c(1,2,4,5,6,7,8,10,15,20,30,40,50,60,70)
vLargeScaletimeWindows <- c(2,10,40,80,120,180,200,260) #360 No Data
vtimeWindows <- c(vsmallTimeWindows,80,90,95,100,120,180)# c(1,2,4) # vsmallTimeWindows##c(4,10,20,80,200,260,360)
##Process *  Serially * So we do not run out of memory
# Results are plotted and Aggregated In lLinModel
rm(lMSDSummaryData)
try(
lMSDSummaryData <<- readRDS(paste0(strDataExportDir,"statPerPathDurationSummaryResults-1-180sec.rds"))
)
if (!exists("lMSDSummaryData"))
{
  lMSDSummaryData <<- makeMSDPathDurationSummary()
}
...
for (strG  in vGroups)
{
 ...
 for (tt in names( lMSDSummaryData[[strG]]) )
  {
    plotLogLogDistribution_MSD(lMSDSummaryData[[strG]][[as.character(tt)]],bAddPlot )
   bAddPlot <- TRUE
  }
}
...
```{r mean-squared-distance over time , fig.show="hold",fig.cap="Mean squared distance over time for LF spontaneous shows distinctive quick growth", out.width="100%", echo=FALSE, cache=FALSE}

##Get Distribution of MSD and Mean MSD for each time integration window 
##load Precalc Results
  plotMSDPerTime(lMSDSummaryData[["NE"]])
  plotMSDPerTime(lMSDSummaryData[["NL"]],TRUE)
  plotMSDPerTime(lMSDSummaryData[["LE"]],TRUE)
  plotMSDPerTime(lMSDSummaryData[["LL"]],TRUE)
  plotMSDPerTime(lMSDSummaryData[["DE"]],TRUE)
  plotMSDPerTime(lMSDSummaryData[["DL"]],TRUE)
...
process_MSDVsPathLengthData <- function(ldatDispersion,strGroupID,addPlot=FALSE)
{
  ...
  vmeanMSDPerPathOft  <- list()
  vsdMSDPerPathOft    <- list()
  vcountMSDPerPathOft <- list()
  ...
  if (NROW(datGroupDispersion) == 0)
  {
    ...
    return(list(muMSDPerT=NA,sdMSDPerT=NA,N=NA,breaks=NA))
  }

  datGroupDispersion$dispRange <- cut(datGroupDispersion[datGroupDispersion$groupID %in% strGroupID ,"Length" ],breaks=dispBreaks)
  
  vmeanMSDPerPathOft  <- tapply(datGroupDispersion$MSD,datGroupDispersion$dispRange, mean,na.rm=TRUE)
  vcountMSDPerPathOft <- tapply(datGroupDispersion$MSD,datGroupDispersion$dispRange, NROW)
  vsdMSDPerPathOft    <- tapply(datGroupDispersion$MSD,datGroupDispersion$dispRange, sd,na.rm=TRUE)
  names(dispBreaks)=names(vmeanMSDPerPathOft)
  #boxplot(datGroupDispersion$Length~datGroupDispersion$dispRange,main=strTitle,col=colourDataScheme[[strGroupID]],ylim=c(0,35))
  return(list(muMSDPerT=vmeanMSDPerPathOft,sdMSDPerT=vsdMSDPerPathOft,N=vcountMSDPerPathOft,breaks=dispBreaks[1:length(vmeanMSDPerPathOft)]  ) )
}
...
plot_MSDPerPathLengthSummary <- function(lMSDPathLStat)
{
  ...
  if (!addPlot)
  {
    plot(seq(1,max(dispBreaks),length.out = NROW(vmeanMSDPerPathOverAllt)),
         vmeanMSDPerPathOverAllt,pch=pointTypeScheme[[strGroupID]],main=strTitle,xlab="Path Length (mm)",
         ylim=c(0,YLIM),xlim=c(0,max(dispBreaks)),col=colourDataScheme[[strGroupID]],cex=0.7 )
  }else
  {
    points(seq(1,max(dispBreaks),length.out = NROW(vmeanMSDPerPathOverAllt)),
         vmeanMSDPerPathOverAllt,pch=16,main=strTitle,xlab="Path Length (mm)",
         col=colourDataScheme[[strGroupID]],cex=0.7 )
    lines(seq(1,max(dispBreaks),length.out = NROW(vmeanMSDPerPathOverAllt)),
         vmeanMSDPerPathOverAllt,lwd=1,lty=lineTypeScheme[[strGroupID]])
  }
  
  if (!addPlot)
  {
    propFactor <- 1.0
    rpoints <- seq(0.5,max(dispBreaks), length.out = NROW(vmeanMSDPerPathOverAllt) )
...
makeMSDPathLengthSummary <- function ()
{
  ...
  baddPlot <- FALSE
  lMSDStat <- list()
  datmeanMSDPerPathOft <- data.frame()
  ...
  for (strG in vGroups)
  {
    ...
    { 
      ...
      datmeanMSDPerPathOft <- rbind(datmeanMSDPerPathOft, datNewTable ) 
      #vmeanMSDPerPathOverAllt <- colMeans(datmeanMSDPerPathOft,na.rm=TRUE)
      #vmeanMSDPerPathOverAllt <- as.matrix(vmeanMSDPerPathOverAllt[names(vmeanMSDPerPathOverAllt)]) #Convert to Vector 
      #vmeansdMSDPerPathOverAllt <- as.matrix(colMeans(do.call(rbind,vsdMSDPerPathOft),na.rm=TRUE))
      #vcountMSDPerPathOverAllt <- as.matrix(colSums(do.call(rbind,vcountMSDPerPathOft),na.rm=TRUE))
    }
  }
  
  saveRDS(lMSDStat,paste0(strDataExportDir,"lMSDPerPathLength2mmSummaryResults1-70sec.rds"))
  saveRDS(datmeanMSDPerPathOft,paste0(strDataExportDir,"datMSDPerPathLength2mmSummaryResults1-70sec.rds"))
  return(datmeanMSDPerPathOft)
}
...
{
  rm(lMSDStat);
  datMSDVsPathlengthSummaryData <<- readRDS(paste0(strDataExportDir,"datMSDPerPathLength2mmSummaryResults1-70sec.rds"));
 lMSDStat <<- readRDS(paste0(strDataExportDir,"lMSDPerPathLength2mmSummaryResults1-70sec.rds"))
}
)
if (!exists("lMSDStat"))
{
...
baddPlot <- FALSE
datmeanMSDPerPathOft <- list()
for (strG in vGroups)
{
  datmeanMSDPerPathOft[[strG]] <-tapply(datMSDVsPathlengthSummaryData[datMSDVsPathlengthSummaryData$groupID == strG,"muMSDPerT"] , datMSDVsPathlengthSummaryData[datMSDVsPathlengthSummaryData$groupID == strG,"breaks"],mean,na.rm=TRUE)
  xtick <- unique(datMSDVsPathlengthSummaryData[datMSDVsPathlengthSummaryData$groupID == strG,"breaks"]) 
  if (!baddPlot)
  {
    plot(xtick, (datmeanMSDPerPathOft[[strG]]),col=colourDataScheme[[strG]],pch=pointTypeScheme[[strG]],lty=lineTypeScheme[[strG]],type="b",
         xlab="Path length (mm)",ylab=" Mean Squared Displacement (mm)",xaxt="n" ,ylim=c(0.2,120))
    ...
    points(xtick,(datmeanMSDPerPathOft[[strG]]),col=colourDataScheme[[strG]],pch=pointTypeScheme[[strG]],lty=lineTypeScheme[[strG]],type="b" )
...
Findings:
  ...
  2. Non-linear MSD regime is evidence of super-diffusion
...
vLargeScaletimeWindows <- c(2,10,40,80,120,180,200,260) #360 No Data
vtimeWindows <- c(vsmallTimeWindows,70,80,90,95,100,120,180)# c(1,2,4) # vsmallTimeWindows##c(4,10,20,80,200,260,360)
##Process *  Serially * So we do not run out of memory
# Results are plotted and Aggregated In lLinModel
...
```{r mean-squared-distance vs Dispersion , fig.cap="Dispersion for LF spontaneous grows distinctively fast, in agreement with MSD", fig.show="hold", out.width="100%", echo=FALSE, cache=FALSE}
...
plot_MSDPerDispersionSummary <- function(lMSDPathLStat)
{
  ...
  {
    plot(seq(1,max(dispBreaks),length.out = NROW(vmeanMSDPerPathOverAllt)),
         vmeanMSDPerPathOverAllt,pch=pointTypeScheme[[strGroupID]],main=strTitle,xlab="Dispersion size (mm)",
         ylim=c(0,YLIM),xlim=c(0,max(dispBreaks)),col=colourDataScheme[[strGroupID]],cex=0.7 )
  }else
  {
    points(seq(1,max(dispBreaks),length.out = NROW(vmeanMSDPerPathOverAllt)),
         vmeanMSDPerPathOverAllt,pch=16,main=strTitle,xlab="Dispersion size (mm)",
         col=colourDataScheme[[strGroupID]],cex=0.7 )
    lines(seq(1,max(dispBreaks),length.out = NROW(vmeanMSDPerPathOverAllt)),
         vmeanMSDPerPathOverAllt,lwd=1,lty=lineTypeScheme[[strGroupID]])
  }
...
makeMSDDispersionSummary <- function ()
{
  ...
  saveRDS(lMSDStat,paste0(strDataExportDir,"lMSDPerDispersion05mmSummaryResults1-70sec.rds"))
  saveRDS(datmeanMSDPerPathOft,paste0(strDataExportDir,"datMSDPerDispersion05mmSummaryResults1-70sec.rds"))
  return(datmeanMSDPerPathOft)
}
...
try(
  {
    ...
    datMSDVsDispersionSummaryData <<- readRDS(paste0(strDataExportDir,"datMSDPerDispersion05mmSummaryResults1-70sec.rds"));
    lMSDDispersionStat <<- readRDS(paste0(strDataExportDir,"lMSDPerDispersion05mmSummaryResults1-70sec.rds"))
  }
)
...
# PLOT Summary per Path Range - Mean MSD Per Path Length over all T
baddPlot <- FALSE
datmeanMSDPerDispersionOft <- list()
for (strG in vGroups)
{
  datmeanMSDPerDispersionOft[[strG]] <- tapply(datMSDVsDispersionSummaryData[datMSDVsDispersionSummaryData$groupID == strG,"muMSDPerT"] , datMSDVsDispersionSummaryData[datMSDVsDispersionSummaryData$groupID == strG,"breaks"],mean,na.rm=TRUE)
  xtick <- unique(datMSDVsDispersionSummaryData[datMSDVsDispersionSummaryData$groupID == strG,"breaks"]) 
  if (!baddPlot)
  {
    plot(xtick, (datmeanMSDPerDispersionOft[[strG]]),col=colourDataScheme[[strG]],pch=pointTypeScheme[[strG]],lty=lineTypeScheme[[strG]],type="b",
         xlab="Dispersion size (mm)",ylab=" Mean Squared Displacement (mm)",xaxt="n" ,ylim=c(0.2,220))
  
    axis(side=1, at=xtick, labels = names(lMSDDispersionStat[[strG]]$`2`$breaks))
    #axis(side=2, at=(axisTicks(c(0.3,2),log=TRUE,nint=5)), labels = axisTicks(c(0.3,2),log=TRUE,nint=5) )  
  }
  else
    points(xtick,(datmeanMSDPerDispersionOft[[strG]]),col=colourDataScheme[[strG]],pch=pointTypeScheme[[strG]],lty=lineTypeScheme[[strG]],type="b" )
