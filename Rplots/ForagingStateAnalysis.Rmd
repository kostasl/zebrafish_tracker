...
loadHuntEventMergedWithPathStat <- function()
{
  
  datHEventDispersion <- data.frame()
  strFile <- paste0(strDataExportDir,"/huntEvent_mergedwith_Dispersion",tsec_timeWindow,"sec.rds")
...
mergeRangeOfLarvalDispersionPerLarvaOntoDispersion <- function(datDispersion,datExpPairs,tsec_timeWindow=0)
{
...
loadHuntEventMergedWithPathStat()
...
```{r  path-length-20sec duration- per dispersion   , fig.show="hold", out.width="50%", echo=FALSE, cache=TRUE,fig.cap="Mean path length per dispersion size for path-durations 20sec" }
...
plot_meanPathLengthPerDispersion <- function(datDispersion,strGroupID,addPlot=FALSE)
{
  ...
  if (!addPlot)
  {
    plot(seq(1,10.5,length.out = NROW(vmeanPathLengthPerDispersion)),
         vmeanPathLengthPerDispersion,xlab="Circle radius / Dispersion (mm)",ylab="Mean path length <l>",
...
In duration confined paths shows that LF evoked paths are longer within the same dispersions sizes to controls. 

It also appears that the length of every path contained in a dispersion circle with radius $r$ increases linearly with $r$, specifically the relationship appears to be :
\[
...
```{r  mean-squared-displacement over log-log distribution , fig.show="hold", out.width="50%", echo=FALSE, cache=TRUE }
...
calcBasicPathStatisticsPerDuration <-function(datDispersion,strG,addPlot=FALSE,powerLawRange = 100)
{
    ...
    xbreaks_MSD        <- seq(0,390,15)
    xbreaks_Dispersion <- seq(0,13,0.5)
    xbreaks_Length     <- seq(0,390,3)
  ...
  for (i in 1:NROW(lModelwithMSD))
  {
    if (!is.null(lModelwithMSD[[i]]$meanPooledMSD))
    {
      if (bUsePooledMean)
        ...
        vMSD[i] <- mean(lModelwithMSD[[i]]$meanMSDPerLarva)
...
makeMSDPathDurationSummary <- function(bParallel=FALSE)
{
  ...
  {
    ldatDispersion <- list()
    ldatDispersion[[as.character(t)]]<- readRDS(file=paste0(strDataExportDir,"/foragingState_Dispersion",t,"sec.rds") ) ##loadDispersionData(forceReload = TRUE,t)
...
vtimeWindows <- c(0,2,5,7,8,10,20,40,60,75,80,90,100,120,180,200,280)
#vtimeWindows <- c(200,260)
...
plotMSDVsTimePerLarva <- function(lModelwithMSD,addPlot=FALSE)
{
  ...
  if (!addPlot)
    plot(vTimeWindow,datMeanMSDVsTimePerLarva[,1],type="p",
         col=colourDataScheme[lModelwithMSD[[1]]$groupID][[1]],ylim=c(0,200),xlab="Time (sec)",ylab="MSD (mm)",xlim=c(0,200))
...
Interestignly, the above plot is representative of the speed of movement over time. The mean distance travelled over long periods of time depends on experience. 
  ...
  - LF change speed over time, Without prey, the speed up after a minute, and with prey they slow down. *(Adaptive behaviour)*
...
plotPathVsTimePerLarva <- function(lModelwithMSD, addPlot=FALSE)
{
  ...
  if (!addPlot)
    plot(vTimeWindow,datMeanMSDVsTimePerLarva[,1],type="p",col=colourDataScheme[lModelwithMSD[[1]]$groupID][[1]],
         ylim=c(0,200),xlim=c(0,200),ylab="mean larval path length (mm)",xlab="Time (sec)")
  for (i in 1:ncol(datMeanMSDVsTimePerLarva))
    lines(vTimeWindow,datMeanMSDVsTimePerLarva[,i],type="p",col=colourDataScheme[lModelwithMSD[[1]]$groupID][[1]])
...
Findings:
  1. Spontaneous MSD grows faster than Evoked - but not for DF.
...
```{r child = 'ForagingStateAnalysis_Clustering.Rmd'}
