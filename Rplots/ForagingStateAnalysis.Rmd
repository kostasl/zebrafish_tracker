...
It remains unkwown, however, whether the motions patterns arose as due to adaptive behaviour or whether observed patterns were an emergent property of the spatial distribution of prey (#sims2008scaling)
...
*Main Findings so far :*
  ...
  21. **Bout Structure**:
      ...
      - LF does not do long-pause/ Waits, in contrast to DF, NF.
...
plotMSDPerTime <- function(lModelwithMSD,addPlot=FALSE,bUsePooledMean=TRUE,bLogLog=T)
{
  ...
  xLim <- c(0.1,max(vTimeWindow ) *1.10 )
  ...
  vHurst <- log(vTimeWindow[idxOrder])/log(vMSD[idxOrder])
  message("mean Hurst Exponent:",mean(vHurst[2:length(vHurst)])/2)
  return(cbind.data.frame(time=vTimeWindow[idxOrder],MSD=vMSD[idxOrder]) )
}
...
##load Precalc Results
bLog <- TRUE
  d<- plotMSDPerTime(lMSDSummaryData[["NE"]],FALSE,TRUE,bLog)
  d<- plotMSDPerTime(lMSDSummaryData[["NL"]],TRUE,TRUE,bLog)
  d<-plotMSDPerTime(lMSDSummaryData[["LE"]],TRUE,TRUE,bLog)
  d<-plotMSDPerTime(lMSDSummaryData[["LL"]],TRUE,TRUE,bLog)
  d<-plotMSDPerTime(lMSDSummaryData[["DE"]],TRUE,TRUE,bLog)
  d<-plotMSDPerTime(lMSDSummaryData[["DL"]],TRUE,TRUE,bLog)
  ...
  d<-plotMSDPerTime(lMSDSummaryData[["NE"]],FALSE,FALSE,bLog)
  d<-plotMSDPerTime(lMSDSummaryData[["NL"]],TRUE,FALSE,bLog)
  d<-plotMSDPerTime(lMSDSummaryData[["LE"]],TRUE,FALSE,bLog)
  d<-plotMSDPerTime(lMSDSummaryData[["LL"]],TRUE,FALSE,bLog)
  d<-plotMSDPerTime(lMSDSummaryData[["DE"]],TRUE,FALSE,bLog)
  d<-plotMSDPerTime(lMSDSummaryData[["DL"]],TRUE,FALSE,bLog)
  ...
  ##Hurst Exponent per time
   cbind(d$time, (log(d$time)/log(d$MSD))/2)
# legend("topright", 
#            legend=c(paste("Spont μ",format(lLinModel_DE[[1]]$coeff,digits=2)),
#                     paste("Evoked μ",format(lLinModel_DL[[1]]$coeff,digits=2) ) ),
#        col=c(colourDataScheme$DE ,colourDataScheme$DL) ,pch=c(16,16) )
