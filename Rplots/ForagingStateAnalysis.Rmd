...
## Mean-Squared-Distance over time is different between groups
...
plotLogLogDistribution_MSD <- function(lLinModel_GE,addPlot=FALSE)
{
    ...
    {
        points(lLinModel_GE$xticks,lLinModel_GE$logDistribution,
         main=paste(" Log-Log percentage of MSD " ),
...
vsmallTimeWindows <- c(2,4,5,6,7,8,10,15,20,30)
vLargeScaletimeWindows <- c(2,10,40,80,120,180,200,260) #360 No Data
vtimeWindows <- c(2,4) # vsmallTimeWindows##c(4,10,20,80,200,260,360)
...
for (t in vtimeWindows)
{
  ...
  lLinModel <- processMSDPerIntegrationTime(ldatDispersion,"NE",lLinModel)
  #lLinModel <- plotMSDPerIntegrationTime(ldatDispersion,"NL",lLinModel)
  #lLinModel <- plotMSDPerIntegrationTime(ldatDispersion,"LE",lLinModel)
  #lLinModel <- plotMSDPerIntegrationTime(ldatDispersion,"LL",lLinModel)
  #lLinModel <- plotMSDPerIntegrationTime(ldatDispersion,"DE",lLinModel)
  #lLinModel <- plotMSDPerIntegrationTime(ldatDispersion,"DL",lLinModel)
...
##Now Plot results
 ...
 for (tt in names( lLinModel[[strG]]) )
  {
    plotLogLogDistribution_MSD(lLinModel[[strG]][[as.character(tt)]],bAddPlot )
  }

##Get Distribution of MSD and Mean MSD for each time integration window 


saveRDS(lLinModel,paste0(strDataExportDir,"MSDPerPathDurationSummaryResults-2-30sec.rds"))
...
```{r path-length to dispersion invariance - across path-durations}
## iNvariance Principle Theoretical Mean Path Length
theoreticalMeanLength <- function(D,propFactor=1.5)
{
  A <- pi*( (D)^2)
...
vsmallTimeWindows <- c(2,4,5,6,7,8,10,15,20)
...
saveRDS(lMSDStat,paste0(strDataExportDir,"MSDPerPathLengthSummaryResults.rds"))


  ##Combined
   lvmeanPathLengthPerMSD <- list()
   strG <- "NL"
   lvmeanPathLengthPerMSD[[strG]] <- as.matrix (do.call(rbind, lMSDStat[[strG]]$muMSDPerT) )
   plot(as.vector(lvmeanPathLengthPerMSD[[strG]]),col=colourDataScheme[[strG]],pch=pointTypeScheme[[strG]],lty=lineTypeScheme[[strG]],type="b",ylim=c(0,150),
        xlab="Path length (mm) ",ylab="Mean squared displacement")
   
   xtick<-seq(1,NCOL(lvmeanPathLengthPerMSD[[strG]]) )
   axis(side=1, at=xtick, labels = FALSE)
   text(x=xtick,  par("usr")[3], 
     labels = colnames(lvmeanPathLengthPerMSD[[strG]]), srt = 25, pos = 1, xpd = TRUE)
