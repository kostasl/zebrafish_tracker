...
calcPathLengthsInDomain <- function(datAllFrames,vgroupID,DomainRadius)
  ...
  saveRDS(datMeanPathLengthAll,file=paste0(strDataExportDir,"/stat_boundedPaths_RAllmm.rds") )
  
  datMeanPathLengthAll<-readRDS(file=paste0(strDataExportDir,"/stat_boundedPaths_RAllmm.rds"))
...
for (strG in as.character(unique(datMeanPathLengthAll$groupID) ) )
{
  ...
  if (!bAddPlot)
  {
    plot(datGroupMeanPathLength$DomainRadius,datGroupMeanPathLength$count/sum(datGroupMeanPathLength$count),main=strG,ylim=c(0,0.2),
...
```{r  bout and turn structure , fig.show="hold",fig.cap="Bout Frequency and free paths", out.width="100%", echo=FALSE, cache=TRUE, results=FALSE}
 ...
 #### PROCESS BOUTS ###
  ...
  if (length(MoveboutsIdx_cleaned))
    datDomainTrajectory[MoveboutsIdx_cleaned,]$IndMotionBout <- 1
  ...
  plot(vEventSpeed_smooth_mm,col=c("black","red","purple")[datDomainTrajectory$IndTurnBout+1],type="b" )  
