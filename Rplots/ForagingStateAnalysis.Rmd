...
#pdf(file= paste(strPlotExportPath,"/foraging/InvariancePrinciple_PathLengths.pdf",sep=""),width=7,height=4)
  ...
  legend("topleft", legend=names(colourDataScheme)[4:9],
...
#pdf(file= paste(strPlotExportPath,"/foraging/InvariancePrinciple_ResidenceTime.pdf",sep=""),width=7,height=4)
  ## Make PLot All groups, Add theoretical Bound  
  plot(datMeanPathLengthAll$DomainRadius,datMeanPathLengthAll$muPathDuration,ylim=c(0,20),xlim=c(0,12),
       col=unlist(colourDataScheme[ as.character(datMeanPathLengthAll$groupID) ]),
...
The 11mm radius is near the boundary, falling short by 0.5mm, and it is the biggest area we can analyse (Distribution of path length examined in later section)


## Path length per dispersion size Examined for Invariance
...
### The Mean length of Fixed-duration paths scales linearly with dispersion
...
The plots below examine both, the total mean path length contained with each dispersion $r$, and the cutting path length (here called *chord*) - the path between the two points that touch the circles boundary:
...
plot_meanPathLengthPerDispersion <- function(datDispersion, strGroupID, addPlot=FALSE, propFactor=1.0)
{
  ...
  vSDPathLengthPerDispersion <- tapply(datGroupDispersion$Length,datGroupDispersion$dispRange,sd,na.rm=TRUE)
  vCountPathLengthPerDispersion <- tapply(datGroupDispersion$Length,datGroupDispersion$dispRange,NROW)
  # Calc Std Error Of Mean 
  vSEPathLengthPerDispersion <- vSDPathLengthPerDispersion/sqrt(vCountPathLengthPerDispersion)
  ...
  {
    plot(rpoints,
         vmeanPathLengthPerDispersion,xlab="Circle radius / Dispersion (mm)",ylab="Total contained mean path length <l> (mm)",
         ylim=c(2,upLIM),xlim=c(0,11),log="y",
         col=colourDataScheme[[strGroupID]],pch=pointTypeScheme[[strGroupID]],cex=CEX )
  ...
  if (!addPlot & propFactor > 0)
  {
    points(rpoints,theoreticalMeanLength(rpoints,propFactor ),pch=2,cex=0.8 ) #Low Lim
    lines(seq(2,12,1),theoreticalMeanLength(seq(2,12,1),propFactor ),pch=2,cex=0.5 ) #Up Lim
    points(seq(1,11,1),theoreticalMeanLength(seq(1,11,1),propFactor ),pch=2,cex=0.5 ) #Up Lim
   # legend("topleft",pch=c(16,2),legend=c("Data","Theory"),col=c(colourDataScheme[[strGroupID]],"black") )
  }
  
  # Add STD Error bar About Mean
  if (NROW(vSEPathLengthPerDispersion)>0)
    arrows(x0=rpoints, y0 = vmeanPathLengthPerDispersion - vSEPathLengthPerDispersion, 
         x1=rpoints, y1 = vmeanPathLengthPerDispersion + vSEPathLengthPerDispersion, 
         code=3, angle=90, length=0.02, col=colourDataScheme[[strGroupID]], lwd=1)
...
Interestingly, the length of time-constrained paths (2sec...50sec) reveal that they grow linearly with Dispersion size. This is true whether we look at the mean path length of the full contained path or just the chord path (path connecting two points that define the dispersion size) and this linear relationship holds across all groups in the same way.
...
When examined over 50sec path duration we find that a linear relationship still holds, with the slope changing further. This can be explained as the development of the same dispersions now is much slower, this is apparently partly explained by increasingly tortuous paths from entry to exit of the same $r$ Dispersion circle domains.
*The linear relationship of dispersion to path-length is not invariant against a path's time-scale*
...
Usually mean free paths are measured, as the path between two changes in direction, or the time of entry and exit, or mean path (as above) from a circle bound area R.
...
The Chord-cutting path-length, which defines the dispersion size,  follow a linear rule more strictly, and that is clear over longer path durations.
...
## Mean Path length per dispersion size different between evoked and spontaneous

```{r  path-length-All duration- per dispersion, fig.show="hold", out.width="100%", echo=FALSE, cache=FALSE,fig.cap="Mean path length per dispersion size for full event path-lengths ", warning=FALSE }

##Load Non-Time constrained Paths (Sliding windows of t duration) - but instead dispersion calculated on  Full paths duration
tsec_timeWindow <- 0  
datDispersion <- loadDispersionData_norm(tsec_timeWindow)
strTitle <- paste("Path lengths vs dispersion size")

#pdf(file= paste(strPlotExportPath,"/foraging/plot_meanPathLengthPerDispersion_AllPath_log.pdf",sep=""),width=7,height=7)
## Take Group 
plot_meanPathLengthPerDispersion(datDispersion,'NE',propFactor = 2)
plot_meanPathLengthPerDispersion(datDispersion,'NL',addPlot = TRUE,propFactor = 2)
plot_meanPathLengthPerDispersion(datDispersion,'LE',addPlot = TRUE,propFactor = 2)
plot_meanPathLengthPerDispersion(datDispersion,'LL',addPlot = TRUE,propFactor = 2)
plot_meanPathLengthPerDispersion(datDispersion,'DE',addPlot = TRUE,propFactor = 2)
plot_meanPathLengthPerDispersion(datDispersion,'DL',addPlot = TRUE,propFactor = 2)
title(strTitle)
...
#dev.off()
```
Here I find that larva in evoked conditions larva show a relative constriction of their expansion when compared to the spontaneous conditions. 
For the same dispersion size the mean path lengths are longer in evoked conditions, and this can be explained in terms of response to prey.

Thus when we look at paths of varying duration, although the linear relationship between dispersion and mean-path length is maintained the slope is no longer identical between evoked and spontaneous conditions.
Combined with the fact that for a fixed path duration this relationship is identical between evoked and spontaneous conditions, this suggests that *it takes longer for evoked conditions to produce the same dispersion  as that in evoked

##  Path-length distribution per *dispersion* size

Expanding on the previous plot showing average path lengths, I now take particular dispersion size and examine the distribution of path-lengths contained within:
...
- Removed as not informative -

```{r  path-length to dispersion ratio distribution , fig.show="hold", out.width="33%", echo=FALSE, cache=TRUE,eval=FALSE }
...
## Distribution of path-length per domain R
...
plotLogLogDistribution_PathLength <- function(datBoundedTrack,strG,addPlot=FALSE)
{
    ...
    mxbreaks <- c(0,2^seq(0,8,1)) ## Do the 2k Binning for clarity over rare events
...
#pdf(file= paste(strPlotExportPath,"/foraging/InvariancePrinciple_DomainCrossingsDistribution_cdf.pdf",sep=""),width=7,height=5)
#par(mar = c(3.9,4.0,1,1))
...
for (strG in as.character(unique(datMeanPathLengthAll$groupID) ) )
{
  ...
  #datGroupSEPathLength <- sum(datGroupMeanPathLength$sdPathLength)/sqrt(sum(datGroupMeanPathLength$count))
  ...
  if (!bAddPlot)
  {
    plot(datGroupMeanPathLength$DomainRadius, cumsum(datGroupMeanPathLength$count/sum(datGroupMeanPathLength$count)),
         ylim=c(0.01,1),xlim=c(0,11),
       xlab="Domain radius (mm)",ylab="fraction of domain crossings",
       col=colourDataScheme[strG][[1]], pch=pointTypeScheme[strG][[1]],type="b")#,log="y"
    segments(x0 = 0,y0=0.5,x1=12,y1=0.5,lty=2,lwd=1)
    segments(x0 = 0,y0=0.3,x1=12,y1=0.3,lty=2,lwd=1)
  }else
  {
    lines(datGroupMeanPathLength$DomainRadius, cumsum(datGroupMeanPathLength$count/sum(datGroupMeanPathLength$count)),
          col=colourDataScheme[strG][[1]], pch=pointTypeScheme[strG][[1]],type="b")
  }
  
  #arrows(x0=datGroupMeanPathLength$DomainRadius, y0=vmeanPathLengthPerDispersion-vSEPathLengthPerDispersion,
   #      x1=datGroupMeanPathLength$DomainRadius, y1=vmeanPathLengthPerDispersion+vSEPathLengthPerDispersion, code=3, angle=90, length=0.02, col=colourDataScheme[[strGroupID]], lwd=1)
  
  bAddPlot <- TRUE
}
title("CDF of domain crossings")
...
plotMSDPerTime <- function(lModelwithMSD,addPlot=FALSE,bUsePooledMean=TRUE,bLogLog=T)
{
  ...
  {
    if(!bLogLog)
          plot(vTimeWindow[idxOrder],vMSD[idxOrder],xlim=xLim,ylim=c(1,200),
        ...
        plot(vTimeWindow[idxOrder],vMSD[idxOrder],xlim=xLim,ylim=c(1,200),log="xy",
         col=colourDataScheme[[lModelwithMSD[[1]]$groupID]],
         pch=pointTypeScheme[[lModelwithMSD[[1]]$groupID]],
         xlab="Path duration (sec)", ylab="Mean squared displacement (mm)")
  }else
...
plotDispersionPerTime <- function(lModelwithDispersion,addPlot=FALSE)
{
  ...
  if (!addPlot)
  {
    plot(vTimeWindow[idxOrder],vDispersion[idxOrder],xlim=xLim,ylim=c(0,13),
         col=colourDataScheme[[lModelwithDispersion[[1]]$groupID]],log="x",
         pch=pointTypeScheme[[lModelwithDispersion[[1]]$groupID]],
         xlab="Time window (sec)", ylab="Dispersion (mm)")
  }else
...
#pdf(file= paste(strPlotExportPath,"/foraging/DispersionPerTimewindow_log.pdf",sep=""),width=7,height=5)
par(mar = c(3.9,4.0,1,1))
...
##load Precalc Results
  ...
  legend("topleft", legend=names(colourDataScheme)[4:9],
         col=unlist(colourDataScheme[names(colourDataScheme)[4:9]]),
         pch=unlist(pointTypeScheme[names(colourDataScheme)[4:9]]) )
title("Dispersion over increasing time windows")
# legend("topright", 
...
## Step-size probability distribution - *Random walk propagators*

The distribution displacement or step length, over a fixed time t will reflect the underlying nature of stochastic process. I have to be aware that due to the Diameter of the ROI, and thus maximum displacement is limited, and thus the distributions is going to  be truncated. For this Reason I look at displacement over a small time frame

For Brownian motion the distribution of step lengths should be Gaussian, which would show as a rapid decay of step length in log log distribution of displacements in t=1sec. 
Super-diffusive or Levy processes will have long fat tails.

Here I am using the suggested method of Levy walk detection explained in #sims2007minimizing, where histogram counts are normalized to the sum, and bins follow a $2^k$ width progression, while each bin is normalized so as to calculate density by dividing each normalized count by the bin width.

```{r step-length Distribution,echo=FALSE,results=FALSE}

plot_loglogDisplacementWithinT <- function(strG,datDispersion,tsec_timeWindow,addPlot=FALSE)
{
  KBreaks <- c(0,2^seq(0,5,0.5))
  datGroupDispersion <- datDispersion[datDispersion$groupID %in% strG &
                                          !is.na(datDispersion$DisplacementSq),]
 
  hist_GDisplacement <- hist(sqrt(datGroupDispersion$DisplacementSq ),plot=FALSE ,breaks=KBreaks)
  hist_GDisplacement_norm <- (hist_GDisplacement$counts/sum(hist_GDisplacement$counts))/diff(KBreaks)
  
  
  if (addPlot == FALSE)
  {
  
    #hist_GDisplacement$counts/sum(hist_GDisplacement$counts
    plot(log10(hist_GDisplacement$mids), log10(hist_GDisplacement_norm),  xlim=log10(c(0.2,20)), ylim=log10(c(10^-6,1)), ##log10
        ylab="Density ",xlab="log Length (mm)", main=paste("Displacement in t= ",tsec_timeWindow),
        col=colourDataScheme[strG][[1]], pch=pointTypeScheme[[strG]])
    
    axXTick <- axisTicks(c(0,20) ,log=TRUE)
    axis(side=1, at=log10(axXTick), labels =  as.character(axXTick) ) 
    
  } else{
    #hist_GDisplacement$counts/sum(hist_GDisplacement$counts
    lines(log10(hist_GDisplacement$mids), log10(hist_GDisplacement_norm),
        col=colourDataScheme[strG][[1]], pch=pointTypeScheme[[strG]],type="b")
  }
  
  ##Plot Linear Model
  #hist_GDisplacement_norm[hist_GDisplacement_norm > 0] <- (10^-10)
  X_dat <- log10(hist_GDisplacement$mids[hist_GDisplacement_norm > 0 & KBreaks > 0.5])
  Y_dat <- log10(hist_GDisplacement_norm[hist_GDisplacement_norm > 0 & KBreaks > 0.5])
  
  linFit <- lm(  Y_dat ~ X_dat )  
  x_linModel <- linFit$model$X_dat
  y_linModel <- (linFit$coefficients[1] + (linFit$coefficients[2]* x_linModel ) )
  
  message(strG," mu is:",linFit$coefficients[2])
  
  lines(x_linModel,y_linModel,type="l",lwd=2,lty=2,col="grey")
  
  ## Re-Call For Paired condition
  strGE <- vGroups[which(vGroups == strG  ) - 1] # Find the complementrary Group-Conditions
  message(strGE)
  if (addPlot == FALSE)
  {
    ##Add paired - Evoked Conditions 
    plot_loglogDisplacementWithinT(strGE,datDispersion,tsec_timeWindow,addPlot=TRUE)
     ## When Pairs Done, Add plot Legend 
    legend("bottomleft",legend=c(strG,strGE,"Gaussian"),
         col=c(colourDataScheme[[strG]],colourDataScheme[[strGE]],"black"), pch=c(pointTypeScheme[[strG]], pointTypeScheme[[strGE]],3) )

  }
   
    ##Add Gaussian Model  
    Emean= mean(sqrt(datGroupDispersion$DisplacementSq ))
    Esd= sd(sqrt(datGroupDispersion$DisplacementSq ))
    hhNorm <-hist(abs(rnorm(n=12000,mean=Emean,sd=Esd)),plot=FALSE) # breaks=hist_GDisplacement$breaks
    #lines(hhNorm$mids,hhNorm$density,xlim=c(0.1,20),log="xy",type="b")
    xPt <- seq(0,20,0.1)
    lines(log10(xPt),log10(dnorm(xPt,mean=Emean,sd=Esd)),lty=2,lwd=2,pch=3,col="red")

}


tsec_timeWindow <-1
datDispersion <- loadDispersionData_norm(tsec_timeWindow,forceReload = FALSE)

#par(mfrow=c(3,1))
for (strG in vGroups[seq(2,6,2)])
{
  plot_loglogDisplacementWithinT(strG,datDispersion,tsec_timeWindow,addPlot=FALSE)
}
...
```
By plotting the empirical step length distribution along with Gaussian that has the same mean and std as the empirical one we can clearly see that the *step lengths have long-fat tails*, ie they larva do not perform simple Brownian motion.

It also appears that LF have shifted to larger displacements than cNF and DF
...
## Returns MSD and Time which we can use to calculate Hurst Coefficient
  ...
  title(main="(Pooled) Mean Squared Displacement")
...
#pdf(file= paste(strPlotExportPath,"/foraging/MSDPerTimeWindow_log.pdf",sep=""),width=7,height=5)
...
lMSDVsTimeData_Grouped <- list()  
  ...
  title(main="(Group) Mean Squared Displacement")
  legend("bottomright", legend=names(colourDataScheme)[4:9],
         col=unlist(colourDataScheme[names(colourDataScheme)[4:9]]),
         pch=unlist(pointTypeScheme[names(colourDataScheme)[4:9]]) )
...
## Use the Fixed Domain Data Paths to examine Durations distribution for fat-tails and And MSD per duration
  ...
  { 
    ...
    plot(durationBreaks[1:length(durationBreaks)-1],(vMSDPerDuration), xlab="Duration (sec)", ylab="MSD",
         main=paste(strG," Domain R",DomainRadius," path MSD vs duration" ),
         col=colourDataScheme[[strG]],
          pch=pointTypeScheme[[strG]]) 
    
    strG <- vGroups[which(vGroups == strG  ) + 1] # Find the complementrary Group-Conditions
  
    lBoundedTrackInfo[[strG]]$DurationBin <- cut(unlist(lBoundedTrackInfo[[strG]]$duration),breaks=durationBreaks)
    vMSDPerDuration <- tapply(unlist(lBoundedTrackInfo[[strG]]$SqDisplacement),lBoundedTrackInfo[[strG]]$DurationBin,mean)
    lines(durationBreaks[1:length(durationBreaks)-1],(vMSDPerDuration),
         col=colourDataScheme[[strG]],
          pch=pointTypeScheme[[strG]],type="p") 
...
## Path Durations within fixed domain R `r cD`

I am using histograms with progressively wider bins, and chose `r cD` based on the upper limit of R within which the invariance principle still holds - indicating that random walk statistics apply.

```{r,  fig.show="hold",fig.cap="Distribution of path durations crossing circular domain with 20mm Diameter", out.width="100%", echo=FALSE, cache=FALSE,fig.show="hold", out.width="80%" } 
## Use the Fixed Domain Data Paths to examine Durations distribution for fat-tails  
  DomainRadius <- cD
  kBreaks <- c(0,2^seq(0,8)) #seq(0,175,3)
  lBoundedTrackInfo <- readRDS(file=paste0(strDataExportDir,"/stat_boundedPathsB_R",DomainRadius,"mm.rds") )
  
  for (strG in vGroups[seq(1,6,2)])
  { 
    hist_Dur <- hist(unlist(lBoundedTrackInfo[[strG]]$duration),breaks=kBreaks, plot=FALSE )
    ##Plot Path Duration Distribution 
    plot(hist_Dur$mids,hist_Dur$density,log="xy",main=paste(strG," domain R",DomainRadius," path duration" ), 
         xlim=c(0.5,250), ylim=c(10^-6,0.5),
         ylab="Density", xlab="Duration (sec)", type="b",
          col=colourDataScheme[[strG]],
          pch=pointTypeScheme[[strG]])
    ...
    hist_Dur <- hist(unlist(lBoundedTrackInfo[[strG]]$duration),breaks=kBreaks, plot=FALSE )
    ##Plot Path Duration Distribution 
    lines(hist_Dur$mids,hist_Dur$density,main=paste(strG," domain R path duration" ), xlim=c(1,250),
          type="b",
          col=colourDataScheme[[strG]],
          pch=pointTypeScheme[[strG]])
  }    
...
*Long fat-Tails missing from LE*, thus with experience larva learn to not wait and explore their surroundings looking for other patches.



## Path Length within fixed domain R `r cD`

To detect the rare events as is the case in Levy Walks, I follow the advice to break the histograms at a x2 frequencies, ie $2^{1\cdots 8}$ in this case. This progressive widening of the  bin sizes allows for clearer results indeed in the log log plot.

```{r,  fig.show="hold",fig.cap="Distribution of path length crossing domain R", out.width="80%", echo=FALSE, cache=FALSE,fig.show="hold", out.width="80%" } 
## Use the Fixed Domain Data Paths to examine Durations distribution for fat-tails  
  DomainRadius <- cD
  kBreaks <- c(0,2^seq(0,8)) #seq(0,175,3)
  lBoundedTrackInfo <- readRDS(file=paste0(strDataExportDir,"/stat_boundedPathsB_R",DomainRadius,"mm.rds") )
  for (strG in vGroups[seq(1,6,2)])
  { 
    hist_Len <- hist(unlist(lBoundedTrackInfo[[strG]]$pathLength),breaks=kBreaks, plot=FALSE )
    ##Plot Path Duration Distribution 
    plot(hist_Len$mids,hist_Len$density,log="xy",main=paste(strG," domain R",DomainRadius," path length" ), xlim=c(0.5,250), ylim=c(10^-6,0.5),
         ylab="Density", xlab="Path length (mm)", type="b",
          col=colourDataScheme[[strG]],
          pch=pointTypeScheme[[strG]])
    ...
    hist_Len <- hist(unlist(lBoundedTrackInfo[[strG]]$pathLength),breaks=kBreaks, plot=FALSE )
    ##Plot Path Duration Distribution 
    lines(hist_Len$mids,hist_Len$density, xlim=c(1,250),
          type="b",
          col=colourDataScheme[[strG]],
          pch=pointTypeScheme[[strG]])
  }    
...
We find that in LF, the presence of prey can generates the rare occurrence of long path lengths, indicating extended local search patterns within the circular domain `r DomainRadius`


## *Hurst exponents* to Identify diffusion type using 

In fractal geometry, the Hurst exponent has been defined by H or by Hq. It quantifies the
relative tendency of a time series to return strongly to the mean or group in one direction
(Kleinow, 2002). Depending on the obtained result we can know if a temporal series is persistent or not persistent.
If persistence is found, it is a sign of the non-existence of white noise, so it could be said that there is some type of dependence between the data, that is, we would be checking if the data behave according to the ordinary Brownian movement or, on the contrary, there is memory within the series of data.
...
The basic Hurst exponent can be *related to the expected size of changes, as a function of the lag between observations*, as measured by $E(|X_{t+\tau}-X_t|^2)$. For the generalized form of the coefficient, the exponent here is replaced by a more general term, denoted by $q$. 
 In particular, the H1 exponent indicates persistent (H1 > ½) or antipersistent (H1 < ½) behavior of the trend. 
...
Lévy-modulated correlated random walks are genuinely superdiffusive across multiple spatial and temporal scales, 
whereas *correlated random walks appear superdiffusive (ballistic) only at short scales. The standard correlated random walk is diffusive at large scales.

*see also hurstexp(x, d = 50, display = TRUE)*
----
...
## This uses the sliding time window data to estimate MSD per path duration 
# and thus it does not reprwesent the distribution of path-durations

#pdf(file= paste(strPlotExportPath,"/foraging/HurstVsTimewindow_log.pdf",sep=""),width=7,height=5)
...
for (strG in vGroups)
{
  ...
  if (!addPlot)
    plot( lMSDVsTimeData_Grouped[[strG]]$time[2:n], vhurstCoeff[2:n],
        ylab="Hurst coefficient (0.5xlog(MSD)/log(duration))", xlab="time window (sec)",type="b",
        main="Hurst coefficient per time-window (Grouped)", ylim=c(0.5,4), log="xy", col=colourDataScheme[strG][[1]], pch=pointTypeScheme[strG][[1]] )
  else
...
## Here I use data from the paths crossing circular  Domain analysis, and calculate msd be duration bin
# then calculate Hurst coeff for each of those bins (1sec)
DomainRadius <- 9
lBoundedTrackInfo <- readRDS(file=paste0(strDataExportDir,"/stat_boundedPathsB_R",DomainRadius,"mm.rds") )
for (strG in vGroups[seq(1,6,2)])
{ 
    # EVOKED COnditions (Prey)
     durationBreaks <- seq(0,150,1)
     lBoundedTrackInfo[[strG]]$DurationBin <- cut(unlist(lBoundedTrackInfo[[strG]]$duration),breaks=durationBreaks)
     
    vMSDPerDuration <- tapply(unlist(lBoundedTrackInfo[[strG]]$SqDisplacement),lBoundedTrackInfo[[strG]]$DurationBin,mean)
    vHurst <- 0.5*log(vMSDPerDuration)/log(durationBreaks[1:length(durationBreaks)-1])
    #vHurst <- vHurst[!is.na(vHurst) & !is.nan(vHurst) & !is.infinite(vHurst) ]    
    plot(durationBreaks[1:length(durationBreaks)-1],vHurst, xlab="Duration (sec)", ylab="log Hurst (0.5xlog(MSD)/log(duration)",log="xy",xaxt="n",type="b",ylim=c(0.5,4),
         main=paste(strG," Domain R",DomainRadius," log Hurst Coefficient" ),
         col=colourDataScheme[[strG]],
          pch=pointTypeScheme[[strG]]) 
     
       axis(side=1, at=durationBreaks[1:length(durationBreaks)-1], labels = durationBreaks[1:length(durationBreaks)-1])
  # Spontaneous  (NO PREY)
    strG <- vGroups[which(vGroups == strG  ) + 1] # Find the complementrary Group-Conditions
    lBoundedTrackInfo[[strG]]$DurationBin <- cut(unlist(lBoundedTrackInfo[[strG]]$duration),breaks=durationBreaks)
    vMSDPerDuration <- tapply(unlist(lBoundedTrackInfo[[strG]]$SqDisplacement),lBoundedTrackInfo[[strG]]$DurationBin,mean)
    vHurst <- 0.5*log(vMSDPerDuration)/log(durationBreaks[1:length(durationBreaks)-1])
    lines(durationBreaks[1:length(durationBreaks)-1],vHurst,type="b",
         col=colourDataScheme[[strG]],
          pch=pointTypeScheme[[strG]]) 
    segments(0,1,150,1)
}
...
*Overall the evidence suggest that movements appear as a Correlated random walk (CRW).*
The generalized Hurst exponent scales in power-law fashion against time-interval - which is evidence of  *long-range dependence (LRD)*.
Slowly decaying variances, LRD, and a spectral density obeying a power-law are different manifestations of the property of the underlying covariance stationary process X. 
Over small timescales <15 sec the exponent indicates persistent behaviour, which is indicative of *Balistic* motion (H>1). These are stronger in LE indicating the ontogeny of intermittent search strategies, through balistic relocations. 
This is a result of experience because coefficients appear identical between controls in the respective test conditions.
These look like correlated random walks (CRW), because CRWs appear superdiffusive (ballistic) only at short scales while the standard correlated random walk is diffusive at large scales, as here.
...
[262]. 
An intermittent search strategy would be consistent with Marques et al 2020 assuming a switch between exploitation and exploration mode.

## Turn direction correlation

**Note:** Persistence and auto-correlation of turn-direction changes with temperature, so good to check differences in autocorrelation of turn-direction.
...
process_MSDVsPathLengthData <- function(ldatDispersion,strGroupID,addPlot=FALSE)
{
  ...
  #boxplot(datGroupDispersion$Length~datGroupDispersion$dispRange,main=strTitle,col=colourDataScheme[[strGroupID]],ylim=c(0,35))
  return(list(muMSDPerT=vmeanPooledMSDPerPathOft,
              sdPooledMSDPerT=vsdPooledMSDPerPathOft,
              N=vcountPooledMSDPerPathOft,
              breaks=dispBreaks[1:length(vmeanPooledMSDPerPathOft)]  ) )
}
...
#pdf(file= paste(strPlotExportPath,"/foraging/MSDPerPathLength.pdf",sep=""),width=7,height=5)
...
for (strG in vGroups)
{
  ...
  if (!baddPlot)
  {
    plot(xtick, (datmeanPooledMSDPerPathOft[[strG]]),col=colourDataScheme[[strG]],pch=pointTypeScheme[[strG]],lty=lineTypeScheme[[strG]],type="b",
         xlab="Path length (mm)",ylab=" Mean Squared Displacement (mm)",xaxt="n" ,ylim=c(1,250),log="y")
...
# ie length to hit the boundary of the observation circle, is only proportional to the circle (in 2D), volume in 3D- 
process_MSDVsDispersion <- function(ldatDispersion,strGroupID,addPlot=FALSE)
{
  YLIM <- 200
  dispBreaks <- seq(0,13,0.25)
  vmeanPooledMSDPerPathOft  <- list()
  vsdPooledMSDPerPathOft    <- list()
  vcountPooledMSDPerPathOft <- list()
  message("Extracting ", strGroupID )
  ...
  #vmeanPooledMSDPerPathOft  <- tapply(datGroupDispersion$DisplacementSq, datGroupDispersion$dispRange, mean,na.rm=TRUE)
  vmeanPooledMSDPerPathOft  <- tapply(datGroupDispersion$MSD, datGroupDispersion$dispRange, mean,na.rm=TRUE)
  vcountPooledMSDPerPathOft <- tapply(datGroupDispersion$MSD, datGroupDispersion$dispRange, NROW)
  vsdPooledMSDPerPathOft    <- tapply(datGroupDispersion$MSD, datGroupDispersion$dispRange, sd,na.rm=TRUE)
  names(dispBreaks)=names(vmeanPooledMSDPerPathOft)
...
plot_MSDPerDispersionSummary <- function(lMSDPathLStat)
{
    ...
    strTitle <- paste("Root MSD vs dispersion", strGroupID,"#")
...
makeMSDDispersionSummary <- function ()
{
  ...
  lMSDStat <- list()
  datmeanPooledMSDPerPathOft <- data.frame()
  useTimeWindowSubset <-  c(2,3,5,10,20,30)
  ## Loop to sequantially load and process Dispersion data across time scales - freeing memory before re-allocating
  for (strG in vGroups)
  {
    lMSDStat[[strG]] <- list()  
    
    for (t in useTimeWindowSubset)
    { 
  ...
  saveRDS(lMSDStat,paste0(strDataExportDir,"lMSDPerDispersion025mmSummaryResults_F2",min(vtimeWindows),"-",max(vtimeWindows),"sec.rds"))
  saveRDS(datmeanPooledMSDPerPathOft,paste0(strDataExportDir,"datMSDPerDispersion025mmSummaryResults_F2",min(vtimeWindows),"-",max(vtimeWindows),"sec.rds"))
  return(datmeanPooledMSDPerPathOft)
}
...
try(
  {
    ...
    datMSDVsDispersionSummaryData <<- readRDS(paste0(strDataExportDir,"datMSDPerDispersion025mmSummaryResults_F2",min(vtimeWindows),"-",max(vtimeWindows),"sec.rds"));
    lMSDDispersionStat <<- readRDS(paste0(strDataExportDir,"lMSDPerDispersion025mmSummaryResults_F2",min(vtimeWindows),"-",max(vtimeWindows),"sec.rds"))
  }
)
...
#pdf(file= paste(strPlotExportPath,"/foraging/MSDPerDispersion025_F2.pdf",sep=""),width=7,height=5)
  # c(bottom, left, top, right) gives the number of lines of margin to be specified on the four sides of the plot. 
  par(mar = c(3.9,4.0,1,1))
  ## Compare MSD to Dispersal
  # PLOT Mean MSD Per Dispersion -
  baddPlot <- FALSE
  ...
  for (strG in vGroups)
  {
    ...
    if (!baddPlot)
    {
      plot(xtick, (datmeanPooledMeanDisplacementPerDispersionOft),col=colourDataScheme[[strG]],
           pch=pointTypeScheme[[strG]],lty=lineTypeScheme[[strG]],type="b",
           xlab="Dispersion size (mm)",ylab="Root Mean Squared Displacement (mm)",xaxt="n", 
           ylim=c(0,15))
      ...
      points(xtick,(datmeanPooledMeanDisplacementPerDispersionOft),
             col=colourDataScheme[[strG]],pch=pointTypeScheme[[strG]],lty=lineTypeScheme[[strG]],type="b" )
    ...
    propFactor <- 1.0
    # 1-1 Diagonal (45 Deg)
    lines(xtick ,xtick,lty=2,cex=0.5,type="l",col="grey",pch=18 ) #Up Lim
    
    ##Approx Line Pass Through Data
    #lines(xtick ,(theoreticalMeanLength(xtick+0.5,propFactor )),pch=18,lty=2,cex=0.5,type="b" ) #
    
    legend("bottomright", legend=c(names(colourDataScheme)[4:9],"Linear "),
           col=c(unlist(colourDataScheme[names(colourDataScheme)[4:9]]),"grey" ),
           pch=c( unlist(pointTypeScheme[names(colourDataScheme)[4:9]]),18 ) )

#dev.off()

# Check Against Raw scatter MSD vs Dispersion
#datD <- datDispersion[!is.na(datDispersion$Dispersion),]
#datsD <- datD[sample(1:NROW(datD),10000), ]
#plot(datsD$Dispersion,sqrt(datsD$MSD))

```

Mean Displacement (calculated as $\sqrt{MSD}$ ) appears consistent against Dispersion, and these two measures are linearly related.
Thus  not only are Dispersion and mean Path length near linearly related, but so is mean displacement.
...
How does MSD relate to the Size or developmental time?
I use Pearson Correlation to check how MSD over time intervals is related to larval size. 
For MSD at t=5,10,20,50 I bootstrap the correlations and plot their estimated distribution.


```{r  mean-squared-distance vs larval size, fig.show="hold",fig.cap="Examing if Development can explain MSD - we find negative correlations overal- Larger size/increased development is not behind increased MSD", out.width="33%", echo=FALSE, cache=TRUE, results=FALSE}

library (plyr) #Split list, apply ldply function #Thanks to https://stackoverflow.com/questions/4227223/convert-a-list-to-a-data-frame
...
datFlatPxLength <- datFlatPxLength[!is.na(datFlatPxLength$expID) ,]
...
##Clear Warningss : assign("last.warning", NULL, envir = baseenv()
# I have only measured Larvae in the Evoked conditions (So I can link to hunt-success)
# To Look at Correlation To MSD in spontaneous conditions I need to find the respective evoked Experiment
## Link Evoked and Spontaneous Trajectories
datHuntStat <- makeHuntStat(datHuntLabelledEventsSBMerged_fixed)
datExpPairs <- getSpontaneousEvokedExperimentPairs(datHuntStat)

# Save A composite file of Lengths Across Both ExpID
datLarvaSize <- merge(datExpPairs,datFlatPxLength,by.y="expID",by.x="expID.E")
saveRDS(datLarvaSize,file= paste(strDataExportDir,"/FishLength3_ExpIDPairs.rds",sep=""))

vMeanMSDVsSizeCorr <- list()
vMeanMSDVsSHuntEvents <- list()
vMeanMSDVsEHuntEvents <- list()
datmuMSDtoSize_corr <- list()
datmuMSDtoSHuntEvents_corr <- list()
datmuMSDtoEHuntEvents_corr <- list()

for (strG in vGroups)
{
    for (tt in names(lMSDSummaryData[[strG]]))
    {
      if (NROW(lMSDSummaryData[[strG]][[tt]]$meanMSDPerLarva) == 0)
        next()
      vMSD       <- lMSDSummaryData[[strG]][[tt]]$meanMSDPerLarva
      
      vExpID     <- names(lMSDSummaryData[[strG]][[tt]]$meanMSDPerLarva)
      vExpID.S <- "" #initialize      
      vExpID.E <- ""
      
      ##Length Is retrieved using Evoked Condition Exp ID
      ##Check If Spontaneous Replace IDs with evoked so we can join Lenghts
      if (strG %in% datExpPairs$groupID.S)
      {
        # Obtain Group Name for Spontaneous and Evoked Conditions
        strG.S <- as.character(head(datExpPairs[as.character(datExpPairs$expID.S) %in% vExpID,]$groupID.S,1 ))
        strG.E <- as.character(head(datExpPairs[as.character(datExpPairs$groupID.S) == strG.S,]$groupID.E,1 ))
        vExpID.E <- as.character(datExpPairs[as.character(datExpPairs$expID.S) %in% vExpID,]$expID.E )
        vExpID.S <- as.character(datExpPairs[as.character(datExpPairs$expID.E) %in% vExpID.E,]$expID.S ) 
        
        ##Fix In case of missing records / Repeated Evoked Exp.
        vMSD <- vMSD[vExpID %in% as.character(vExpID.S)]
        vExpID <- vExpID.E
      }
      
      # In Evoked conditions
      if (strG %in% datExpPairs$groupID.E)
      {
        strG.E <- as.character(head(datExpPairs[as.character(datExpPairs$expID.E) %in% vExpID,]$groupID.E,1 ))
        strG.S <- as.character(head(datExpPairs[as.character(datExpPairs$groupID.E) %in% strG.E,]$groupID.S,1 ))
        ##Need this to Match the extra Evoked Experiments  Associated with the Same Spontaneous One
        vExpID.S <- as.character(datExpPairs[as.character(datExpPairs$expID.E) %in% vExpID,]$expID.S )
        vExpID.E <- as.character(datExpPairs[as.character(datExpPairs$expID.S) %in% vExpID.S,]$expID.E )
        vMSD <- vMSD[vExpID %in% as.character(vExpID.E)]
        vExpID <- vExpID.E ##Remove those Evoked Repeats - Keep set of 60 
      }

      ##Retrieve Spont and Evoked Hunt Rate for MSD 
      vHuntEventCount.S <- datHuntStat[,"vHLarvaEventCount"][[strG.S]][vExpID.S]
      vHuntEventCount.E <- datHuntStat[,"vHLarvaEventCount"][[strG.E]][vExpID.E]

      
      #vExpID     <- vExpID[vExpID %in% datFlatPxLength$expID]
      
      datMSD     <- data.frame(expID=vExpID,meanMSD=vMSD,
                               HuntEvents.S=as.vector(vHuntEventCount.S), 
                               HuntEvents.E=as.vector(vHuntEventCount.E),
                               HuntEvents.E.expID=names(vHuntEventCount.E), ##Sanity Check/Validation
                               HuntEvents.S.expID=names(vHuntEventCount.S))
      
      datMSDvsLength <- merge(datFlatPxLength,datMSD,by.x="expID",by.y="expID")
      ##plot(datMSDvsLength$LengthPx*DIM_MMPERPX,)
      
      YRange <- c(0,150)
      XRange <- c(0,5)
      pBw <- 0.05
      vMeanMSDVsSizeCorr[[tt]]    <- cor(datMSDvsLength$LengthPx*DIM_MMPERPX, datMSDvsLength$meanMSD, method="pearson")
      vMeanMSDVsSHuntEvents[[tt]] <- cor(datMSDvsLength$HuntEvents.S, datMSDvsLength$meanMSD, method="pearson")
      vMeanMSDVsEHuntEvents[[tt]] <- cor(datMSDvsLength$HuntEvents.E, datMSDvsLength$meanMSD, method="pearson")
      ##Selective Plot Of Densities
      if (tt %in%  c("10","20","50"))
      {
        stat_MSDVsSize <- bootStrap_stat(datMSDvsLength$LengthPx*DIM_MMPERPX,datMSDvsLength$meanMSD,1000,XRange,YRange,"pearson")
        plot(density(stat_MSDVsSize$corr,bw=pBw),main=paste("MSD vs Size ",strG," t",tt),
             col=colourDataScheme[[strG]],lwd=3,xlim=c(-0.5,0.5))
      }
      
    }
  ##Summarize Group over time into dataframe
  datmuMSDtoSize_corr[[strG]] <-  ldply ((vMeanMSDVsSizeCorr), data.frame)
  names(datmuMSDtoSize_corr[[strG]]) <- c("t","corr")
  datmuMSDtoSHuntEvents_corr[[strG]] <-  ldply ((vMeanMSDVsSHuntEvents), data.frame)
  names(datmuMSDtoSHuntEvents_corr[[strG]]) <- c("t","corr")
  datmuMSDtoEHuntEvents_corr[[strG]] <-  ldply ((vMeanMSDVsEHuntEvents), data.frame)
  names(datmuMSDtoEHuntEvents_corr[[strG]]) <- c("t","corr")
}

## Plot Summarized MSD Correlation Coefficients Against Interval time
for (g in c(1,3,5))
{   strG <- vGroups[g]

    plot(datmuMSDtoSize_corr[[strG]]$t,datmuMSDtoSize_corr[[strG]]$corr,col=colourDataScheme[[strG]],pch=pointTypeScheme[[strG]],
       xlab="Time",ylab="Pearson Correlation",main=paste("MSD correlation to larval size"), ylim=c(-0.6,0.6))
    strG <- vGroups[g+1]
    points(datmuMSDtoSize_corr[[strG]]$t,datmuMSDtoSize_corr[[strG]]$corr,col=colourDataScheme[[strG]],pch=pointTypeScheme[[strG]],
       xlab="Time",ylab="Pearson Correlation",main=paste("MSD correlation to larval size"), ylim=c(-0.6,0.6),cex=1.2)

}

```
Evidence that Developmental size/time does not explain MSD:
  
  * A inverse correlation of size to MSD exists in DF and NF, (like the HPI).
  * While No apparent correlation in LF of MSD to size
  
## MSD Vs Hunt Rates 
```{r  mean-squared-distance vs hunt-rate, fig.show="hold",fig.cap=" MSD vs Hunt Rate ", out.width="33%", echo=FALSE, cache=TRUE, results=FALSE}
...
{ 
    ...
    plot(datmuMSDtoEHuntEvents_corr[[strG]]$t,datmuMSDtoEHuntEvents_corr[[strG]]$corr,col=colourDataScheme[[strG]],pch=pointTypeScheme[[strG]],
       xlab="Time",ylab="Pearson Correlation",main=paste(strG," MSD correlation to Hunt Rate"), ylim=c(-0.6,0.6))
    
    strGP <- vGroups[g+1]
    points(datmuMSDtoSHuntEvents_corr[[strG]]$t,datmuMSDtoSHuntEvents_corr[[strG]]$corr,col=colourDataScheme[[strGP]],pch=pointTypeScheme[[strGP]],
       xlab="Time",ylab="Pearson Correlation", ylim=c(-0.6,0.6),cex=1.2)
...
{ 
    ...
    plot(datmuMSDtoEHuntEvents_corr[[strG]]$t,datmuMSDtoEHuntEvents_corr[[strG]]$corr, col=colourDataScheme[[strGP]],
         pch=pointTypeScheme[[strGP]], xlab="Time", ylab="Pearson Correlation", main=paste(strG," MSD correlation to Hunt Rate"), ylim=c(-0.6,0.6))

    points(datmuMSDtoSHuntEvents_corr[[strG]]$t,datmuMSDtoSHuntEvents_corr[[strG]]$corr,col=colourDataScheme[[strG]],pch=pointTypeScheme[[strG]],
       xlab="Time",ylab="Pearson Correlation", ylim=c(-0.6,0.6),cex=1.2)
...
```{r  mean-squared-distance over prey density , fig.show="hold", out.width="100%", echo=FALSE, cache=TRUE}

plot_MSDPerPreyDensitySummary <- function(lMSDPathLStat)
{
  
    strTitle <- paste("mean MSD per Prey Density ", strGroupID,"#")
...
##Calc the mean pooled MSD from each integration Window and show mean diffusion
plotMSDPerPreyDensity <- function(lModelwithMSD,addPlot=FALSE,bUsePooledMean=TRUE,bLogLog=T)
{
  vMSD <- vector()
  vTimeWindow <- vector()
  for (i in 1:NROW(lModelwithMSD))
...
##Integrate time out, and plot MSD per path-length
# ie legth to hit the boundary of the observation circle, is only proportional to the circle (in 2D), volume in 3D- 
process_MSDVsPreyDensityData <- function(ldatDispersion,strGroupID,addPlot=FALSE)
{
  YLIM <- 200
  dispBreaks <- seq(0,500,5) ## Prey Count Bins - Max Set High To Avoid breaking due to  some Noisy measurements
  vmeanPooledMSDPerPreyCount  <- list()
  vsdPooledMSDPerPreyCount    <- list()
  vcountPooledMSDPreyCount <- list()
  message("Extracting ", strGroupID )

  datGroupDispersion <- ldatDispersion[ldatDispersion$groupID %in% strGroupID & 
                                                !is.na(ldatDispersion$PreyCount) ,]
  if (NROW(datGroupDispersion) == 0)
  {
    warning("No prey count data for ", strGroupID)
    return(list(muMSDPerT=NA,sdPooledMSDPerT=NA,N=NA,breaks=NA))
  }

  datGroupDispersion$dispRange <- cut(datGroupDispersion[datGroupDispersion$groupID %in% strGroupID ,"PreyCount" ],breaks=dispBreaks)
  ## Estimate MSD per Prey-Density Bin
  vmeanPooledMSDPerPreyCount  <- tapply(datGroupDispersion$DisplacementSq,datGroupDispersion$dispRange, mean,na.rm=TRUE)
  vcountPooledMSDPerPreyCount <- tapply(datGroupDispersion$DisplacementSq,datGroupDispersion$dispRange, NROW)
  vsdPooledMSDPerPreyCount    <- tapply(datGroupDispersion$DisplacementSq,datGroupDispersion$dispRange, sd,na.rm=TRUE)
  names(dispBreaks)=names(vmeanPooledMSDPerPreyCount)
  #boxplot(datGroupDispersion$Length~datGroupDispersion$dispRange,main=strTitle,col=colourDataScheme[[strGroupID]],ylim=c(0,35))
  return(list(muMSDPerT=vmeanPooledMSDPerPreyCount,
              sdPooledMSDPerT=vsdPooledMSDPerPreyCount,
              N=vcountPooledMSDPerPreyCount,
              breaks=dispBreaks[1:length(vmeanPooledMSDPerPreyCount)]  ) )
}



makeMSDPreyDensitySummary <- function (vtimeWindows = c(2) )
{
  #vtimeWindows <- vsmallTimeWindows #c(1,2,10) #
  ##Load All
  baddPlot <- FALSE
  lMSDStat <- list()
  datmeanPooledMSDPerPreyCount <- data.frame()
  ## Loop to sequantially load and process Dispersion data across time scales - freeing memory before re-allocating
  for (strG in vGroups)
  {
    lMSDStat[[strG]] <- list()  
    ...
    { 
      ...
      rm(ldatDispersion)   #Clear Mem
      ldatDispersion<- list()
      message("Processing dispersions of for timewindow:", t)
      ##Load MSD /Dispersion/ Path Data
      ldatDispersion[[as.character(t)]] <- loadDispersionData(TRUE,t)  
      # Check If Prey Count Calculation Was included in this Dispersion
      if ( !any(grepl("PreyCount",names(ldatDispersion[[as.character(t)]])) ) )
      {
        message("*Prey Count Missing - Re-Calculate Trajectory Dispersions")
        loaddatAllFrames()
        datDispersion <<- calcTrajectoryDispersions(datAllFrames,tsec_timeWindow)
      }
      ...
      lMSDStat[[strG]][[as.character(t)]] <- process_MSDVsPreyDensityData(ldatDispersion[[as.character(t)]],strG)
      ...
      datNewTable$T <- as.character(t)
      datNewTable$groupID <- strG
      datmeanPooledMSDPerPreyCount <- rbind(datmeanPooledMSDPerPreyCount, datNewTable ) 
      #vmeanPooledMSDPerPathOverAllt <- colMeans(datmeanPooledMSDPerPathOft,na.rm=TRUE)
      #vmeanPooledMSDPerPathOverAllt <- as.matrix(vmeanPooledMSDPerPathOverAllt[names(vmeanPooledMSDPerPathOverAllt)]) #Convert to Vector 
      #vmeansdPooledMSDPerPathOverAllt <- as.matrix(colMeans(do.call(rbind,vsdPooledMSDPerPathOft),na.rm=TRUE))
      #vcountPooledMSDPerPathOverAllt <- as.matrix(colSums(do.call(rbind,vcountPooledMSDPerPathOft),na.rm=TRUE))
  ...
  saveRDS(lMSDStat,paste0(strDataExportDir,"lMSDPerPreyDensitySummaryResults",min(vtimeWindows),"-",max(vtimeWindows),"sec.rds"))
  saveRDS(datmeanPooledMSDPerPreyCount,paste0(strDataExportDir,"datMSDPerPreyDensitySummaryResults",min(vtimeWindows),"-",max(vtimeWindows),"sec.rds"))
  return(datmeanPooledMSDPerPreyCount)
}
...
lMSDStat <<- readRDS(paste0(strDataExportDir,"lMSDPerPreyDensitySummaryResults",min(vtimeWindows),"-",max(vtimeWindows),"sec.rds"))
datMSDVsPreyDensitySummaryData <<- readRDS(paste0(strDataExportDir,"datMSDPerPreyDensitySummaryResults",min(vtimeWindows),"-",max(vtimeWindows),"sec.rds"))


if (!exists("lMSDStat"))
{
  datMSDVsPreyDensitySummaryData <<- makeMSDPreyDensitySummary()
}


# PLOT Summary per PreyCount Range - Mean MSD Per Prey Count Length (over  T = 5)
#pdf(file= paste(strPlotExportPath,"/foraging/MSDPerPreyDensity.pdf",sep=""),width=7,height=7)
  ## bottom, left,top, right
  par(mar = c(4.3,4.3,2,1))
  
  baddPlot <- FALSE
  strRG <- list(LF=c("LE","LL"),NF=c("NE","NL"),DF=c("DE","DL"))
  datmeanPooledMSDPerPreyDensity <- list()
  ## Correct Noise In Prey Count In Empty Conditions 
  datMSDVsPreyDensitySummaryData[datMSDVsPreyDensitySummaryData$groupID %in% c("LE","DE","NE"),"breaks"] = 0
  for (strG in names(strRG) )
  {
    ## Add Complemenatry test condition
    datmeanPooledMSDPerPreyDensity[[strG]] <- tapply(datMSDVsPreyDensitySummaryData[datMSDVsPreyDensitySummaryData$groupID %in% strRG[[strG]],"muMSDPerT"],
                                                     datMSDVsPreyDensitySummaryData[ datMSDVsPreyDensitySummaryData$groupID%in% strRG[[strG]],"breaks"], mean, na.rm=TRUE)
    
    xtick <- unique(datMSDVsPreyDensitySummaryData[datMSDVsPreyDensitySummaryData$groupID %in% strRG[[strG]],"breaks"]) 
    xtick <- xtick[!is.na(xtick)]
    if (!baddPlot)
    {
      plot(xtick, (datmeanPooledMSDPerPreyDensity[[strG]]),col=colourDataScheme[[strG]]$Evoked, pch=pointTypeScheme[[strG]], lty=lineTypeScheme[[strG]], type="b",
           xlab="Prey Density (R/10ml)", ylab=" Mean Squared Displacement (mm/2sec)", ylim=c(1,50),xlim=c(5,70),asp=1,log="xy", xaxt="n",cex=1.3,cex.axes=1.3)#
      ...
      points(xtick,(datmeanPooledMSDPerPreyDensity[[strG]]), col=colourDataScheme[[strG]]$Evoked, pch=pointTypeScheme[[strG]], lty=lineTypeScheme[[strG]], type="b" )
    
    baddPlot <- TRUE
  }
    ##Add Theoretical MSD growth vs Time
    
    legend("bottomleft", legend=names(strRG)  ,
           col=c(unlist(colourDataScheme[names(strRG)])[c(2,4,6)]),
           pch=c( unlist(pointTypeScheme[names(strRG)][c(2,4,6)])), horiz=TRUE )
...
```{r Clustering-Dispersion, child = 'ForagingStateAnalysis_Clustering.Rmd',eval=TRUE}
```
...
Used  Min bout turn-speed threshold=`r MIN_BOUT_TURN_SPEED `
Min bout speed threshold=`r   G_THRES_MOTION_BOUT_SPEED `,
  minimum turn size criterion/threshold :`r MIN_TURN_SIZE_THRES`,
  minimum gap between frames `r MIN_GAP_FRAMES_BETWEEN_TURNS`.
...
I exclude bouts that belong to hunt events in order to reveal bout structure differences when the larvae are searching for prey and remove bias due to differences in hunt rate between spontaneous and evoked conditions.
...
# TODO: Revise  To Include Bout Mode- Hunt/Search Mode
makeBoutAndFreePathSummary <- function()
{
  # Check Bouts against Hunt Events and Label Bout accordingly
  #Default Time Window is 5 sec
  datHEventDispersion <- loadHuntEventMergedWithPathStat(tsec_timeWindow)
  ...
  for (DomainRadius in vAvailableDomainRadius)
  {
    ...
    {
      warning("Missing ",strBoutAnalysisFile)
      next()
    }
    ...
    for (strGroup in names(lBoundedTrackInfo) )
    {
      ...
      for (trackID in rownames( groupdomainTracks ) )
      {
        if (is.null(lMeanPathLengthAll[[trackID]]))
          ...
          expID <- groupdomainTracks$expID[[trackID]] #groupdomainTracks[trackID,"expID"][[1]]
          eventID <-groupdomainTracks$EventID[[trackID]]# groupdomainTracks[trackID,"EventID"][[1]]
          
          n = NROW(lMeanPathLengthAll[[trackID]])
          datTrackBouts <- lMeanPathLengthAll[[trackID]]
          
          datTrackHuntEvents <- datHEventDispersion[datHEventDispersion$expID == expID &
                                           datHEventDispersion$eventID == eventID ,  ]
          
          datTrackBouts$ForageMode <- factor("Search",levels=c("Search","Hunt","NA"))
          
          #If Huntevents Exist Check If bouts Are in Hunt Mode
          for (idxEvent in 1:NROW(datTrackHuntEvents) )
          {
            if (NROW(datTrackHuntEvents) ==0 )
              break;
            # Mark Bouts included in Hunt Event -
            datTrackBouts[datTrackBouts$boutStartFrame >=  datTrackHuntEvents[idxEvent,"frameN"] &
                          datTrackBouts$boutStartFrame <=  datTrackHuntEvents[idxEvent,"endFrame"]
                          ,"ForageMode"] <- factor("Hunt",levels=c("Search","Hunt","NA"))
          }
  ...
  saveRDS(ldatBoutsPerDomain , file=paste0(strDataExportDir,"/stat_BOUTandMode_PerDomainSize.rds") )
...
if (!file.exists(paste0(strDataExportDir,"/stat_BOUTandMode_PerDomainSize.rds")))
  makeBoutAndFreePathSummary()

ldatBoutsPerDomain <- readRDS(file=paste0(strDataExportDir,"/stat_BOUTandMode_PerDomainSize.rds"))
ldatFreePathsPerDomain <- readRDS(file=paste0(strDataExportDir,"/stat_BOUT_FreePathPerDomainSize.rds"))
  ...
  for (R in names(ldatFreePathsPerDomain) )
  {
    ...
    {
        i <- i + 1
        meanBoutLengthPathPerDomain[[i]] <- 
          list(meanBoutDistance = mean( unlist( 
            ldatBoutsPerDomain[[R]][ldatBoutsPerDomain[[R]]$groupID == strG &
                                      ldatBoutsPerDomain[[R]]$ForageMode == BOUT_MODE &
                                  ldatBoutsPerDomain[[R]]$turn_deg < MIN_TURN_SIZE_THRES &
                                  ldatBoutsPerDomain[[R]]$boutSeqID > 0 ,"travelDistance_mm"]), na.rm = T),
            
               sdBoutDistancePath = sd( unlist(ldatBoutsPerDomain[[R]][
                                                ldatBoutsPerDomain[[R]]$groupID == strG & 
                                                ldatBoutsPerDomain[[R]]$ForageMode == "Search" &
                                                ldatBoutsPerDomain[[R]]$turn_deg < MIN_TURN_SIZE_THRES &
                                                ldatBoutsPerDomain[[R]]$boutSeqID > 0 ,"travelDistance_mm"]), na.rm = T),
            
              n = NROW(sd( unlist(ldatBoutsPerDomain[[R]][
                                  ldatBoutsPerDomain[[R]]$groupID == strG & 
                                  ldatBoutsPerDomain[[R]]$ForageMode == BOUT_MODE &
                                  ldatBoutsPerDomain[[R]]$turn_deg < MIN_TURN_SIZE_THRES &
                                  ldatBoutsPerDomain[[R]]$boutSeqID > 0 ,"travelDistance_mm"]), na.rm = T)),
...
#pdf(file= paste(strPlotExportPath,"/foraging/freePathBoutFrequency_cdf.pdf",sep=""),width=7,height=5)
par(mar = c(3.9,4.0,1,1))
 ...
 {    
   ...
   hist_bT <- hist(vBoutCount,  breaks=seq(0,200,1),plot=FALSE )
    ...
    if (!addPlot){
        ...
        #plot(hist_bT$mids,hist_bT$counts/sum(hist_bT$counts),log="xy",
        #     col=colourDataScheme[strG][[1]],pch=pointTypeScheme[strG][[1]],type="b",xlim=c(1,50),
       #     xlab="Number of bouts",ylab="Log normalized bout frequency")
        
          plot(ecdf(vBoutCount), col=colourDataScheme[strG][[1]],pch=pointTypeScheme[strG][[1]],
               xlab="Number of bouts",ylab="Cumulative Density Function ", xlim=c(0,15),ylim=c(0.5,1), main=NA)
       }
    else{
      
        #lines(hist_bT$mids,hist_bT$counts/sum(hist_bT$counts),
        #      col=colourDataScheme[strG][[1]],pch=pointTypeScheme[strG][[1]],type="b")
       lines(ecdf(vBoutCount), col=colourDataScheme[strG][[1]],pch=pointTypeScheme[strG][[1]],main=NA)
    }
   addPlot <- TRUE
 }
...
#pdf(file= paste(strPlotExportPath,"/foraging/InterBoutInterval_cdf.pdf",sep=""),width=7,height=5)
#par(mar = c(3.9,4.0,1,1))
...
for (strG in vGroups)
{
    vInterBoutInterval <- ldatBoutsPerDomain[[cD]][
                            ldatBoutsPerDomain[[cD]]$groupID == strG & 
                            ldatBoutsPerDomain[[R]]$ForageMode == BOUT_MODE &
    ...
    if (!addPlot){
        #plot(hist_IBI$mids,hist_IBI$density,
        #     col=colourDataScheme[strG][[1]],pch=pointTypeScheme[strG][[1]],
        #     type="b",xlim=c(0.01,50),log="xy",
        #     xlab="Inter-bouts interval (sec)",ylab="Log Density inter-bout interval ")
        plot(ecdf(vInterBoutInterval) , col=colourDataScheme[strG][[1]],pch=pointTypeScheme[strG][[1]],
             do.points = FALSE,lty=lineTypeScheme[strG][[1]],lwd=3,
             xlim=c(0.01,2.5),#log="xy",
             xlab="Inter-bouts interval (sec)",ylab="Cumulative Density Function ",main=NA)

    }
    else{
        #lines(hist_IBI$mids,hist_IBI$density,col=colourDataScheme[strG][[1]],pch=pointTypeScheme[strG][[1]],type="b")
      #scaling*hist_IBI$counts/(sum(hist_IBI$counts))
       lines( ecdf(vInterBoutInterval) ,col=colourDataScheme[strG][[1]], 
              pch=pointTypeScheme[strG][[1]],main=NA, lty=lineTypeScheme[strG][[1]],lwd=3) 
    }
   # points( ecdf(vInterBoutInterval) ,col=colourDataScheme[strG][[1]],pch=pointTypeScheme[strG][[1]],main=NA ) 
   addPlot <- TRUE
}
title("CDF Inter-bout interval ")
...
Using `r MIN_TURN_SIZE_THRES` as turn threshold to define bouts that belong to a straight (free-path).
...
## Turn counts per domain R `r cD` path

I compare the number of turns contained in each path crossing a domain of size R.

Using `r MIN_TURN_SIZE_THRES` as turn threshold, and excluding bouts executed during hunt mode.
...
datTurnsPerDomain <- ldatBoutsPerDomain[[cD]]
...
for (strG in vGroups)
{
    turnsPerPath[[strG]] <- with(datTurnsPerDomain[datTurnsPerDomain$groupID == strG & 
                                                     ...
                                                     datTurnsPerDomain$ForageMode == BOUT_MODE  &
 ...
 names(runsPerPath[[strG]]) <- c("RunCount")
  ...
  for (strG in vGroups)
  {
   ...
   addPlot <- TRUE
   message(paste("Mean turn count per path ",strG,":", mean(unlist( turnsPerPath[[strG]] ),na.rm=TRUE )   ) )
  }
...
### Turn magnitudes per domain R `r cD` path

```{r turn magnitude per domain size, out.width="33%", echo=FALSE, cache=FALSE, results=FALSE}
 ##Turn Magnitude distributions
  ...
  for (strG in vGroups)
  {
    datGTurns<- datTurnsPerDomain[datTurnsPerDomain$groupID == strG & 
                                    abs(datTurnsPerDomain$turn_deg) < 120 & abs(datTurnsPerDomain$turn_deg) > 1 &
    ...
    if (!addPlot)
    {
      plot(density(datGTurns$turn_deg,bw=BW ),col=colourDataScheme[strG][[1]],pch=pointTypeScheme[strG][[1]],lty=lineTypeScheme[strG][[1]],lwd=3,ylim=c(0,0.05),xlim=c(-60,60),
           main=paste(strG,"Turns while crossing domain R",DomainRadius))
      legend("topleft",legend=paste(strG,"-R",DomainRadius,"mu:",format(mean(datGTurns$turn_deg),digits=3)  ),
             lty=lineTypeScheme[strG][[1]],lwd=3, col=colourDataScheme[strG][[1]] )
    }
    else
    {
     lines(density(datGTurns$turn_deg,bw=BW),col=colourDataScheme[strG][[1]],pch=pointTypeScheme[strG][[1]],lty=lineTypeScheme[strG][[1]],lwd=3)
      legend("topright",legend=paste(strG,"-R",DomainRadius,"mu:",format(mean(datGTurns$turn_deg),digits=3)  ),
             lty=lineTypeScheme[strG][[1]],lwd=3, col=colourDataScheme[strG][[1]] )
    }
...
I also discover that the turn distributions of *spontaneous events show a mode-tendency towards larger turns*, near the 35-45 degrees,  strangely,  these turns have been previously found to be is associated with successful hunt events.


## Bout durations crossing domain R `r cD`

Examine if macroscale search behaviour is modified by changing bout duration.

These are compared only for bouts not while in hunt-mode, such that hunt-rates do not influence the comparison. 

For clear search behaviour analysis Bout durations need to be examined without including motion during hunting episodes. This is because
hunt rates go up in evoked conditions and this may bias bout durations.

```{r bout-duration for a domain size, out.width="50%", echo=FALSE, cache=FALSE, results=FALSE}

#  MIN_TURN_SIZE_THRES
#CD is radius of domain
plotBoundedPathBoutDuration <- function(ldatBoutsPerDomain,strG,addPlot=FALSE,cD="10")  
{
    datBouts <- ldatBoutsPerDomain[[cD]][
              ldatBoutsPerDomain[[cD]]$groupID == strG & 
              ldatBoutsPerDomain[[cD]]$ForageMode == BOUT_MODE &
              ldatBoutsPerDomain[[cD]]$boutSeqID != 0 &
              ldatBoutsPerDomain[[cD]]$turn_deg < MIN_TURN_SIZE_THRES &
              ldatBoutsPerDomain[[cD]]$duration_sec < 10 ,]
    
    # Label  Hunt Mode Bouts
        
    vBoutDuration <- datBouts$duration_sec
...
## Bout distance  crossing domain R `r cD`

These are compared only for bouts not while in hunt-mode, such that hunt-rates do not influence the comparison. 

```{r bout-distance for fixed domain size, out.width="50%", echo=FALSE, cache=FALSE, results=FALSE,warning=FALSE}
#  MIN_TURN_SIZE_THRES
#CD is radius of domain
plotBoundedPathBoutDistance <- function(ldatBoutsPerDomain,strG,addPlot=FALSE,cD="10")  
{
    vBoutDistance <- unlist(ldatBoutsPerDomain[[cD]][
                            ldatBoutsPerDomain[[cD]]$groupID == strG & 
                              ldatBoutsPerDomain[[cD]]$ForageMode == BOUT_MODE &
                             ldatBoutsPerDomain[[cD]]$boutSeqID != 0 &
                               ldatBoutsPerDomain[[cD]]$turn_deg < MIN_TURN_SIZE_THRES &
...
#xbreaks <- c(seq(0,0.5,0.01),seq(0.51,1,0.1),seq(1.5,50,1)) #,seq(10,60,1)
    dens_GBD <- density(datPathBoutsDistance_perLarva[!is.na(datPathBoutsDistance_perLarva[,strG]),strG],bw=0.2 )
      ...
      if (!addPlot){
          plot(dens_GBD,col=colourDataScheme[strG][[1]],pch=pointTypeScheme[strG][[1]],lty=lineTypeScheme[strG][[1]],
               type="l",lwd=3,xlim=c(0.0,3),ylim=c(0.0,1.5),#log="xy",
               xlab="bout distance (mm)",ylab=" Density bout distance ",main="Mean bout distance per larva (Excl. hunt-events)")
      }else{
          lines(dens_GBD,col=colourDataScheme[strG][[1]],pch=pointTypeScheme[strG][[1]],lty=lineTypeScheme[strG][[1]],type="l",lwd=3) #scaling*hist_IBI$counts/(sum(hist_IBI$counts))
      }
     print(paste("Group bout distance ",strG," mu:",mean(datPathBoutsDistance_perLarva[,strG],na.rm=TRUE) ))
...
### Bout Distance Correlation To Larval Size

Examine if development can explain bout distances.
I use Spearman correlation in order to reveal whether progressively, the larger the larva is the longer the bouts, as ranked within its rearing group.


```{r bout-length to larval size correlation, out.width="50%", echo=FALSE, cache=FALSE, results=FALSE,warning=FALSE}
datLarvaSize <- readRDS(datLarvaSize,file= paste(strDataExportDir,"/FishLength3_ExpIDPairs.rds",sep=""))
datPathBoutsDistance_perLarva[,2]

datMeanBoutLengthPerExp <- rbind.data.frame(
      cbind.data.frame (names(datPathBoutsDistance_perLarva[!is.na(datPathBoutsDistance_perLarva[,1]),1]),datPathBoutsDistance_perLarva[!is.na(datPathBoutsDistance_perLarva[,1]),1]),
      cbind.data.frame(names(datPathBoutsDistance_perLarva[!is.na(datPathBoutsDistance_perLarva[,2]),2]), datPathBoutsDistance_perLarva[!is.na(datPathBoutsDistance_perLarva[,2]),2]),
      cbind.data.frame(names(datPathBoutsDistance_perLarva[!is.na(datPathBoutsDistance_perLarva[,3]),3]), datPathBoutsDistance_perLarva[!is.na(datPathBoutsDistance_perLarva[,3]),3]),
      cbind.data.frame(names(datPathBoutsDistance_perLarva[!is.na(datPathBoutsDistance_perLarva[,4]),4]), datPathBoutsDistance_perLarva[!is.na(datPathBoutsDistance_perLarva[,4]),4]),
      cbind.data.frame(names(datPathBoutsDistance_perLarva[!is.na(datPathBoutsDistance_perLarva[,5]),5]),datPathBoutsDistance_perLarva[!is.na(datPathBoutsDistance_perLarva[,5]),5]),
      cbind.data.frame(names(datPathBoutsDistance_perLarva[!is.na(datPathBoutsDistance_perLarva[,6]),6]),
            datPathBoutsDistance_perLarva[!is.na(datPathBoutsDistance_perLarva[,6]),6])
)
names(datMeanBoutLengthPerExp)  <- c("expID","meanBoutLength")

saveRDS(datMeanBoutLengthPerExp,file= paste(strDataExportDir,"/meanBoutLengthPerExp.rds",sep=""))

# Combine With Length
datBoutVsSize.E <- merge(datMeanBoutLengthPerExp,datLarvaSize,by.x="expID",by.y="expID.E")
datBoutVsSize.S <- merge(datMeanBoutLengthPerExp,datLarvaSize,by.x="expID",by.y="expID.S") 


lBoutVsSize_corr <- list()
llBoutVsSize_corrDens <- list()
llBoutVsSize_corrSuffDens <- list()
...
for (g in  c(1,3,5))
{
  ...
  lBoutVsSize_corr[[gStr]] <- bootStrap_stat(datBoutVsSize.E[datBoutVsSize.E$groupID.E == gStr,]$LengthPx*DIM_MMPERPX,
                 datBoutVsSize.E[datBoutVsSize.E$groupID.E == gStr,]$meanBoutLength,1000,XRange,YRange,"spearman" ) #spearman pearson
  #Obtain Density Of Boot Strapped Correlation
  llBoutVsSize_corrDens[[gStr]] <- density(lBoutVsSize_corr[[gStr]]$corr)
  llBoutVsSize_corrSuffDens[[gStr]] <- density(lBoutVsSize_corr[[gStr]]$corr_suffled)
  
  gStr <- vGroups[g+1] # Spontaneous
  lBoutVsSize_corr[[gStr]] <- bootStrap_stat(datBoutVsSize.S[datBoutVsSize.S$groupID.S == gStr,]$LengthPx*DIM_MMPERPX,
                 datBoutVsSize.S[datBoutVsSize.S$groupID.S == gStr,]$meanBoutLength,1000,XRange,YRange,"spearman" ) #spearman
...
for (strG in  vGroups)
{
  plot(llBoutVsSize_corrDens[[strG]],main=paste(" mean Bout-length vs larva size correlation "),xlim=c(-0.4,0.4), col=colourDataScheme[strG][[1]],lwd=3 )
  lines(llBoutVsSize_corrSuffDens[[strG]],lwd=3,lty=2)
  legend("topright",c(strG,"Suffled data"),col=c(colourDataScheme[strG][[1]],"black"),lty=c(1,2),lwd=3 )
}

```
The Ranked correlation of larva size to bout length appears identically positive in both NF and LF: The larger a larva is the longer its bouts both in spontaneous and evoked conditions.
*However, this correlation goes away in DF*, and larger larva no longer execute longer bouts!
What is the interpretation of this? 

  - It could be that DF slow down so the stay within the vicinity as their expectation is that the do not need to explore to find food.

# Bout Speed for paths crossing domain R `r cD`
...
plotBoundedPathBoutSpeed <- function(ldatBoutsPerDomain,strG,addPlot=FALSE,cD="10")  
{
    ...
    vBoutDuration <- unlist(ldatBoutsPerDomain[[cD]][
                            ldatBoutsPerDomain[[cD]]$groupID == strG & 
                               ldatBoutsPerDomain[[cD]]$ForageMode == BOUT_MODE &
                             ldatBoutsPerDomain[[cD]]$boutSeqID != 0 &
                               ldatBoutsPerDomain[[cD]]$turn_deg < MIN_TURN_SIZE_THRES &
...
boxplot(speed_mmsec ~ groupID,data=datMeanBoutSpeedPerLarva,main="Mean bout speed per larva",
        col=c(colourDataScheme$LL, colourDataScheme$LE, colourDataScheme$NL, colourDataScheme$NE, colourDataScheme$DL, colourDataScheme$DE ),ylim=c(0,25) )
...
## Bout Speed Vs Larva Size

Here I bootstrap the correlation of mean larval bout speed and larva size data, in order to obtain a statistics for the correlation.

```{r bout-speed to larval size correlation, out.width="50%", echo=FALSE, cache=FALSE, results=FALSE,warning=FALSE}
datLarvaSize <- readRDS(datLarvaSize,file= paste(strDataExportDir,"/FishLength3_ExpIDPairs.rds",sep=""))
datPathBoutsDistance_perLarva[,2]

datMeanBoutSpeedPerExp <- rbind.data.frame(
      cbind.data.frame (names(datPathBoutsSpeed_perLarva[!is.na(datPathBoutsSpeed_perLarva[,1]),1]),datPathBoutsSpeed_perLarva[!is.na(datPathBoutsSpeed_perLarva[,1]),1]),
      cbind.data.frame(names(datPathBoutsSpeed_perLarva[!is.na(datPathBoutsSpeed_perLarva[,2]),2]), datPathBoutsSpeed_perLarva[!is.na(datPathBoutsSpeed_perLarva[,2]),2]),
      cbind.data.frame(names(datPathBoutsSpeed_perLarva[!is.na(datPathBoutsSpeed_perLarva[,3]),3]), datPathBoutsSpeed_perLarva[!is.na(datPathBoutsSpeed_perLarva[,3]),3]),
      cbind.data.frame(names(datPathBoutsSpeed_perLarva[!is.na(datPathBoutsSpeed_perLarva[,4]),4]), datPathBoutsSpeed_perLarva[!is.na(datPathBoutsSpeed_perLarva[,4]),4]),
      cbind.data.frame(names(datPathBoutsSpeed_perLarva[!is.na(datPathBoutsSpeed_perLarva[,5]),5]),datPathBoutsSpeed_perLarva[!is.na(datPathBoutsSpeed_perLarva[,5]),5]),
      cbind.data.frame(names(datPathBoutsSpeed_perLarva[!is.na(datPathBoutsSpeed_perLarva[,6]),6]),
            datPathBoutsSpeed_perLarva[!is.na(datPathBoutsSpeed_perLarva[,6]),6])
)
names(datMeanBoutSpeedPerExp)  <- c("expID","meanBoutSpeed")

saveRDS(datMeanBoutSpeedPerExp,file= paste(strDataExportDir,"/meanBoutSpeedPerExp.rds",sep=""))

# Combine With Length
datBoutSpeedVsSize.E <- merge(datMeanBoutSpeedPerExp,datLarvaSize,by.x="expID",by.y="expID.E")
datBoutSpeedVsSize.S <- merge(datMeanBoutSpeedPerExp,datLarvaSize,by.x="expID",by.y="expID.S") 

lBoutSpeedVsSize_corr <- list()
lBoutSpeedVsSize_corrDens <- list()
lBoutSpeedVsSize_corrSuffDens <- list()
...
for (g in  c(1,3,5))
{
  ...
  lBoutSpeedVsSize_corr[[gStr]] <- bootStrap_stat(datBoutSpeedVsSize.E[datBoutSpeedVsSize.E$groupID.E == gStr,]$LengthPx*DIM_MMPERPX,
                 datBoutSpeedVsSize.E[datBoutSpeedVsSize.E$groupID.E == gStr,]$meanBoutSpeed,1000,XRange,YRange,"spearman" ) #spearman pearson
  #Obtain Density Of Boot Strapped Correlation
  lBoutSpeedVsSize_corrDens[[gStr]] <- density(lBoutSpeedVsSize_corr[[gStr]]$corr)
  lBoutSpeedVsSize_corrSuffDens[[gStr]] <- density(lBoutSpeedVsSize_corr[[gStr]]$corr_suffled)
  
  gStr <- vGroups[g+1] # Spontaneous
  lBoutSpeedVsSize_corr[[gStr]] <- bootStrap_stat(datBoutSpeedVsSize.S[datBoutSpeedVsSize.S$groupID.S == gStr,]$LengthPx*DIM_MMPERPX,
                 datBoutSpeedVsSize.S[datBoutSpeedVsSize.S$groupID.S == gStr,]$meanBoutSpeed,1000,XRange,YRange,"spearman" ) #spearman
...
for (strG in  vGroups)
{
  plot(  lBoutSpeedVsSize_corrDens[[strG]], main=paste(" mean Bout-speed vs larva size correlation "), 
         xlim=c(-0.4,0.4), col=colourDataScheme[strG][[1]],lwd=3 )
  lines(lBoutSpeedVsSize_corrSuffDens[[strG]],lwd=3,lty=2)
  legend("topright",c(strG,"Suffled data"),col=c(colourDataScheme[strG][[1]],"black"),lty=c(1,2),lwd=3 )
}

```
There is a positive correlation that shows larger larvae tend to have faster bouts. This is correlation is not affected by the presence of prey for NF and LF, but *the strange thing is it seems that the presence of prey changes Bout speed for DF! they are slow down for prey *
Perhaps this is consistent with their tendency to remain in place, yet why don't NF and LF slow down when prey is there?
 

# Hunt initiation and foraging motion

```{r Analysis of Hunt Events, child = 'ForagingStateAnalysis_HuntEvents.Rmd',eval=TRUE}
```
...
##  Foraging and Predator Avoidance (see (@stephens2008))
Foraging in a variable environment presents a classic problem of decision making with incomplete information. see also (@dunlap2012tracking) but also (@viswanathan2011physics)

Foraging may be optimal in terms of maximizing food uptake, but may involve a a balance between other factors such as risk of predation. Discrepancies between optimal foraging models and empirical field data have already been noted (see Phys of Foraging pg 15), which have suggested trade-offs between optimal foragings and predator risk.
