...
mergeDispersionOntoHuntEvents <- function(datDispersion, datAllFrames, datHuntLabelledEventsSBMerged_fixed)
{
  ...
  ## MERGE Dispersion With HuntEvent Records
    ...
    nmergemissingEvents <-  NROW(datHEventDispersion[!(datHEventDispersion$frameRow %in% datSubDispersion$frameRow),] )
...
plotHuntProb <- function(lmcmcRes,clustCol,groupIDOffset=1,clustIDOffset=1)
{
   ##plot prob of occupying each cluster
  breaksProb <- seq(0,1.01,by=0.01)
  list_histo <- hist(lmcmcRes$pHunt[[lmcmcRes$chain]],breaks=breaksProb,plot=FALSE )
  h1<-hist( lmcmcRes$pHunt[[lmcmcRes$chain]][,groupIDOffset], breaks=breaksProb,plot=FALSE) ##Cluster 1 ylim=c(0,max(list_histo$counts))
  h2 <- hist( lmcmcRes$pHunt[[lmcmcRes$chain]][,groupIDOffset + clustIDOffset], breaks=breaksProb, plot=FALSE ) ## ##Cluster 1 
  
  ##Colour Determines which Cluster is fast and slow
   barplot(rbind(h1$density,h2$density),beside = FALSE, col = c(clustCol[1],clustCol[2],"white"),
           names.arg = (h1$breaks[-length(h1$breaks)]),
           main=paste("Hunt initiation", lmcmcRes$groupID ),
           xlab = "Prob. of hunt initiation per cluster (pHunt)",
           #xlim =  c(0,1)
           )
   txtLeg <- c("Exploit","Explore")
   ## Make Legend order match the colour classes
...
#load(file=paste0(strDataExportDir,'jagsModelResults_NBMixGroupclustDispersionAndHuntEvents_All') )
```
...
runNBMixGroupClusterModel <- function(vDispersion,vHuntInitiation,vGroupID, bQuickTest = TRUE)
{
  ...
  if (bQuickTest)
  {
    idSample         <- sample(1:NROW(vDispersion),15000)
    vDispersion      <- vDispersion[idSample]
    vHuntInitiation  <- vHuntInitiation[idSample]
    vGroupID         <- vGroupID[idSample]
  }
...
if (bQuickTest)
{
    ...
    strOutFilename <- "jagsModelResults_NBMixTestGroupclustDispersionAndHuntEvents_All"
}
# add the exact frames of hunt events so we can cluster them in exploit/Explore easly datHEventDispersion$frameRow
# Frame Records of Huntevents not in SubSampled frames
missinghuntEvents.frameROW <- datHEventDispersion[!(datHEventDispersion$frameRow %in% datSubDispersion$frameRow),"frameRow"] 
...
barplot(table(datSubDispersion$clustID,datSubDispersion$groupID),
        col=clustCol,
        main="Frames per cluster/cond-group")
legend("topleft",legend=(paste("#",table(datSubDispersion$clustID) )),fill=clustCol  )
...
## Hierarchical clustering model to reveal difference in larval behaviour
...
load(file=paste0(strDataExportDir,strOutFilename) )
...
muC1_Sub <- mean(datDispersion[datDispersion$frameRow == datSubDispersion[datSubDispersion$clustID == 1,]$frameRow,]$Dispersion,na.rm=TRUE)
muC2_Sub <- mean(datDispersion[datDispersion$frameRow == datSubDispersion[datSubDispersion$clustID == 2,]$frameRow,]$Dispersion,na.rm=TRUE)
#
stopifnot(((muC1_Hunts < muC2_Hunts) & (muC1_Sub < muC2_Sub))  )
...
```{r hunt-outcomes-per-globally clustered dispersions-spontaneous, fig.show="hold", out.width="33%", echo=FALSE, cache=TRUE }
## Check NF
## SUCCESS
