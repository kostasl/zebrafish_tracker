...
calcBasicPathStatisticsPerDuration <-function(datDispersion,strG,addPlot=FALSE,powerLawRange = 100)
{
    ...
    lLinModel_GE$meanGroupMSD <- mean(lLinModel_GE$meanMSDPerLarva)
...
plotMSDPerTime <- function(lModelwithMSD,addPlot=FALSE,bUsePooledMean=TRUE)
{
  ...
  if (!addPlot)
  {
    plot(vTimeWindow[idxOrder],vMSD[idxOrder],xlim=xLim,ylim=c(0,300),
...
lAvailableDispersionFiles <- dir(strDataExportDir, pattern = "foragingState_Dispersion[0-9]{1,3}sec.rds", full.names = FALSE, ignore.case = TRUE)
...
vtimeWindows <- unlist(vAvailabletimeWindows[order(as.integer(unlist(vAvailabletimeWindows)))] )
