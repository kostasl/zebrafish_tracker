...
## Path length to dispersion ratio distributions 
...
loadDispersionData()
xbreaks <-seq(0,max(datDispersion$Length,na.rm=TRUE)+1,1)
hist(datDispersion[datDispersion$groupID %in% c('LE'), ]$Length,breaks=xbreaks,main="LF Spont.",xlab="Length (mm)",col=colourDataScheme$LF$Spont)
...
hist(datDispersion[datDispersion$groupID %in% c('LL'), ]$Length,breaks=xbreaks,main="LF Evoked.",xlab="Length (mm)",col=colourDataScheme$LF$Evoked)
...
hist(datDispersion[datDispersion$groupID %in% c('NE'), ]$Length,breaks=xbreaks,main="NF Spont.",xlab="Length (mm)",col=colourDataScheme$NF$Spont)
...
hist(datDispersion[datDispersion$groupID %in% c('NL'), ]$Length,breaks=xbreaks,main="NF Evoked.",xlab="Length (mm)",col=colourDataScheme$NF$Evoked)
...
hist(datDispersion[datDispersion$groupID %in% c('DE'), ]$Length,breaks=xbreaks,main="DF Spont.",xlab="Length (mm)",col=colourDataScheme$DF$Spont)
...
hist(datDispersion[datDispersion$groupID %in% c('DL'), ]$Length,breaks=xbreaks,main="DF Evoked.",xlab="Length (mm)",col=colourDataScheme$DF$Evoked)
...
```{r  hunt-event-group-length-dispersion histogram , fig.show="hold", out.width="50%", echo=FALSE, cache=TRUE }
...
For example, take a random walk until it hits a circle of radius r times the step length. The average number of steps it performs is $r^2$.[citation needed] This fact is the discrete version of the fact that a Wiener process walk is a fractal of Hausdorff dimension 2.[citation needed]
...
To see such an *invariance principle holds*, where by the mean path length only proportional to the Area/Circumfarence ratio for a random walk,  I look at mean path lengths for a set of fixed dispersion sizes:
...
theoreticalMeanLength <- function(D)
{
  A <- pi*(D^2)
  C <- 2*pi*D
  return(round(100*( pi*A/C)) /100 )
}
...
plot_meanPathLengthPerDispersion <- function(datDispersion,strGroupID)
{
  ...
  datGroupDispersion$dispRange <- cut(datGroupDispersion[datGroupDispersion$groupID %in% strGroupID ,"Dispersion" ],breaks=seq(1,11,1))
  ...
  plot(vmeanPathLengthPerDispersion,pch=16,main=strTitle,xlab="Circle radius / Dispersion (mm)",
       ylim=c(0,25),col=colourDataScheme[[strGroupID]] )
  rpoints <- seq(0.5,11,1)
  points(rpoints,theoreticalMeanLength(seq(0.5,11,1) ),pch=2,cex=0.8 ) #Low Lim
...
##  path-length to dispersion ratios 
```{r  path-length to dispersion ratio distribution , fig.show="hold", out.width="50%", echo=FALSE, cache=TRUE }
...
hist_pathLengthPerDispersion <- function(datDispersion,strGroupID)
{
  ...
  dispLevel <- cut(datGroupDispersion[datGroupDispersion$groupID %in% strGroupID ,"Dispersion" ],breaks=seq(1,11,1))
  #x <- datDispersion[!is.na(dispLevel) & dispLevel == '(3,4]',"Dispersion"]
  hist(y,breaks=seq(0,46,2),
  main=strTitle,
  col=colourDataScheme[[strGroupID]]  )
