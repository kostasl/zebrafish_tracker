...
```{r  mean-squared-distance over time , fig.show="hold", out.width="50%", echo=FALSE, cache=TRUE }
...
getLogLogDistribution_MSD <-function(datDispersion,strG,addPlot=FALSE,powerLawRange = 100)
{
    ...
    xtick<-log10(h_Length_GE$mids)
    
    lLinModel_GE <- getLogPowerSlope(h_Length_GE,powerLawRange)
    lLinModel_GE$xtick <-xtick
    lLinModel_GE$logDistribution <-log10(100*h_Length_GE$counts/sum(h_Length_GE$counts) )
...
plotLogLogDistribution_MSD <- function(lLinModel_GE,addPlot=FALSE)
{
    ...
    if (!addPlot) 
    {
      plot(lLinModel_GE$xtick,lLinModel_GE$logDistribution,
         main=paste(" Log-Log percentage of MSD "),
         xlab="log Length (mm)", col=colourDataScheme[strG][[1]], pch=16,xlim=c(min(lLinModel_GE$xtick)/2,max(lLinModel_GE$xtick)*1.01),ylim=c(-4,3),
         ylab="log N(l) %",xaxt="n",yaxt="n" )
        axis(side=1, at=lLinModel_GE$xtick, labels =  as.character(10^lLinModel_GE$xtick))
       #text(x=lLinModel_GE$xtick[seq(0,length(lLinModel_GE$xtick),3)],  par("usr")[3], labels = (10^xtick[seq(0,9,3)]), srt = 45, pos = 1, xpd = TRUE)
    }else
    {
        points(lLinModel_GE$xtick,lLinModel_GE$logDistribution,
         main=paste(" Log-Log percentage of MSD " ),
         xlab="log_10 Mean Squared Displacent (mm)", col=colourDataScheme[strG][[1]], pch=16,xlim=c(0,3.0),ylim=c(-4,3), ylab="log N(l) %")
    }
    ##Plot Linear Model
...
plotMSDPerTime <- function(lModelwithMSD,addPlot=FALSE)
{
  ...
  if (!addPlot)
  {
    plot(vTimeWindow[idxOrder],vMSD[idxOrder],xlim=xLim,ylim=c(0,70),
...
processMSDPerIntegrationTime <- function(ldatDispersion,strG,lLinModel,bAddPlot = FALSE)
{
  ...
  for (tt in names(ldatDispersion))
  {
    ...
    lLinModel[[strG]][[as.character(tt)]] = getLogLogDistribution_MSD(ldatDispersion[[tt]],strG,bAddPlot)
    bAddPlot <- TRUE
  }
...
makeMSDPathDurationSummary <- function()
{
  ...
  for (t in vtimeWindows)
  {
    ...
    for (strG  in vGroups)
      lMSDSummaryData <- processMSDPerIntegrationTime(ldatDispersion,strG,lLinModel)
    
    ##Clear Memory
    rm(ldatDispersion)
  }
  saveRDS(lLinModel,paste0(strDataExportDir,"MSDPerPathDurationSummaryResults-1-60sec.rds"))
...
vtimeWindows <- vsmallTimeWindows# c(1,2,4) # vsmallTimeWindows##c(4,10,20,80,200,260,360)
##Process *  Serially * So we do not run out of memory
# Results are plotted and Aggregated In lLinModel

try(
lMSDSummaryData <<- readRDS(paste0(strDataExportDir,"MSDPerPathDurationSummaryResults-1-60sec.rds"))
)
...
{
  bAddPlot <- FALSE
 for (tt in names( lLinModel[[strG]]) )
  {
...
### MSD to path-length reveals super-Diffusion - (Integrate time out)
...
The resulting plots show **evidence of superdiffusion **!
```{r  mean-squared-distance over path-length , fig.show="hold", out.width="50%", echo=FALSE, cache=TRUE}
## iNvariance Principle Theoretical Mean Path Length
theoreticalMeanLength <- function(D,propFactor=1.5)
{
  A <- pi*( (D)^2)
...
# ie legth to hit the boundary of the observation circle, is only proportional to the circle (in 2D), volume in 3D- 
plot_meanPathLengthPerMSD <- function(ldatDispersion,strGroupID,addPlot=FALSE)
{
  YLIM <- 200
  dispBreaks <- seq(1,40,4)
  vmeanMSDPerPathOft <- list()
  vsdMSDPerPathOft <- list()
  vcountMSDPerPathOft <- list()
  for (tt in names(ldatDispersion))
  {
    message("Extracting ", strGroupID," t:",tt )
    datGroupDispersion <- ldatDispersion[[tt]][ldatDispersion[[tt]]$groupID %in% strGroupID & 
                                                !is.na(ldatDispersion[[tt]]$Length) ,]
    if (NROW(datGroupDispersion) == 0)
    {
      warning("No path length data for ",strGroup," Int(t):",tt)
      next()
    }
    
    datGroupDispersion$dispRange <- cut(datGroupDispersion[datGroupDispersion$groupID %in% strGroupID ,"Length" ],breaks=dispBreaks)
    vmeanMSDPerPathOft[[tt]] <- tapply(datGroupDispersion$MSD,datGroupDispersion$dispRange,mean,na.rm=TRUE)
    vcountMSDPerPathOft[[tt]] <- tapply(datGroupDispersion$MSD,datGroupDispersion$dispRange,NROW)
    vsdMSDPerPathOft[[tt]] <- tapply(datGroupDispersion$MSD,datGroupDispersion$dispRange,sd,na.rm=TRUE)
  }
  
    datmeanMSDPerPathOft <- (do.call(rbind,vmeanMSDPerPathOft))
  ...
  #boxplot(datGroupDispersion$Length~datGroupDispersion$dispRange,main=strTitle,col=colourDataScheme[[strGroupID]],ylim=c(0,35))
    return(list(muMSDPerT=vmeanMSDPerPathOft,sdMSDPerT=vmeansdMSDPerPathOverAllt))
}
...
{
  vtimeWindows <- vsmallTimeWindows
  ##Load All
  baddPlot <- FALSE
  lMSDStat <- list()
  ...
  { 
    ...
    lMSDStat[["NE"]] <- plot_meanPathLengthPerMSD(ldatDispersion,'NE',addPlot=baddPlot)
    baddPlot <- TRUE
    lMSDStat[["NL"]] <- plot_meanPathLengthPerMSD(ldatDispersion,'NL',addPlot=baddPlot)
  
    lMSDStat[["LE"]] <- plot_meanPathLengthPerMSD(ldatDispersion,'LE',addPlot=baddPlot)
    lMSDStat[["LL"]] <- plot_meanPathLengthPerMSD(ldatDispersion,'LL',addPlot=baddPlot)
  
    lMSDStat[["DE"]] <- plot_meanPathLengthPerMSD(ldatDispersion,'DE',addPlot=baddPlot)
    lMSDStat[["DL"]] <- plot_meanPathLengthPerMSD(ldatDispersion,'DL',addPlot=baddPlot)
    
    plotMSDPerIntegrationTime(ldatDispersion,"NE",lLinModel)
  
  }
  saveRDS(lMSDStat,paste0(strDataExportDir,"MSDPerPathLengthSummaryResults1-60sec.rds"))
  return(lMSDStat)
}

##Try To load Precalc Results
try(
lMSDVsPathlengthSummaryData <<- readRDS(paste0(strDataExportDir,"MSDPerPathLengthSummaryResults1-60sec.rds"))
)
if (!exists("lMSDSummaryData"))
{
  lMSDVsPathlengthSummaryData <<- makeMSDPathLengthSummary()
}
  

  ##Combined
   lvmeanPathLengthPerMSD <- list()
   strG <- "NL"
   lvmeanPathLengthPerMSD[[strG]] <- as.matrix (do.call(rbind, lMSDVsPathlengthSummaryData[[strG]]$muMSDPerT) )
   plot(as.vector(lvmeanPathLengthPerMSD[[strG]]),col=colourDataScheme[[strG]],pch=pointTypeScheme[[strG]],lty=lineTypeScheme[[strG]],type="b",ylim=c(0,150),
        xlab="Path length (mm) ",ylab="Mean squared displacement",xaxt="n",yaxt="n")
   
   xtick<-seq(1,NCOL(lvmeanPathLengthPerMSD[[strG]]) )
   axis(side=1, at=xtick, labels = FALSE)
   text(x=xtick,  par("usr")[3], 
     labels = colnames(lvmeanPathLengthPerMSD[[strG]]), srt = 15, pos = 1, xpd = TRUE)

   strG <- "NE"
   lvmeanPathLengthPerMSD[[strG]] <- as.matrix (do.call(rbind, lMSDVsPathlengthSummaryData[[strG]]$muMSDPerT) )
   points(as.vector(lvmeanPathLengthPerMSD[[strG]]),col=colourDataScheme[[strG]],pch=pointTypeScheme[[strG]],lty=lineTypeScheme[[strG]],type="b")
   #lines(as.vector(lvmeanPathLengthPerMSD[[strG]]),col=colourDataScheme[[strG]],pch=pointTypeScheme[[strG]])
   strG <- "LE"
   lvmeanPathLengthPerMSD[[strG]] <- as.matrix (do.call(rbind, lMSDVsPathlengthSummaryData[[strG]]$muMSDPerT) )
   points(as.vector(lvmeanPathLengthPerMSD[[strG]]),col=colourDataScheme[[strG]],pch=pointTypeScheme[[strG]],lty=lineTypeScheme[[strG]],type="b")
   #lines(as.vector(lvmeanPathLengthPerMSD[[strG]]),col=colourDataScheme[[strG]],pch=pointTypeScheme[[strG]])
...
#dev.off()
## Classic Dispersion Growith with Path Length
#plot_meanPathLengthPerDispersion(ldatDispersion[["2"]],'NE')
#plot_meanPathLengthPerDispersion(ldatDispersion[["10"]],'NE',addPlot = TRUE)
#plot_meanPathLengthPerDispersion(ldatDispersion[["20"]],'NE',addPlot = TRUE)
#plot_meanPathLengthPerDispersion(ldatDispersion[["40"]],'NE',addPlot = TRUE)
#plot_meanPathLengthPerDispersion(ldatDispersion[["80"]],'NE',addPlot = TRUE)
#plot_meanPathLengthPerDispersion(ldatDispersion[["120"]],'NE',addPlot = TRUE)
