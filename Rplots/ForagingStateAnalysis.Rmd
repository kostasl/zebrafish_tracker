...
getMCMCEstimatedParams <- function(results,groupID,ichain = 2)
{
  ...
  mean = as.mcmc.list(results, vars="muOfClust"),
  sd = cbind(tauOfClust$`tauOfClust[1].sd`,tauOfClust$`tauOfClust[2].sd`),
...
plotClusterProb <- function(lmcmcRes,clustCol)
{
  ...
  list_histo <- hist(lmcmcRes$pClustcoeff[[lmcmcRes$chain]],breaks=breaksProb,main=groupID,freq = TRUE,xlab = "Prob. of cluster membership (pClust)",plot=FALSE )
  h1<-hist( lmcmcRes$pClustcoeff[[lmcmcRes$chain]][,1],xlim=c(0,1),ylim=c(0,max(list_histo$counts)),
        breaks=breaksProb,col=clustCol[1]
        ,xlab = NA, add=TRUE,plot=FALSE)
  h2 <- hist( lmcmcRes$pClustcoeff[[lmcmcRes$chain]][,2],xlim=c(0,1), breaks=breaksProb,col=clustCol[2],main=groupID,add=TRUE,plot=FALSE )
  
   barplot(rbind(h1$density,h2$density),beside = FALSE, col = c(clustCol,"white"),names.arg = (h1$breaks[-length(h1$breaks)]),main=paste("Clustered Occupancies", lmcmcRes$groupID ),xlab = "Prob. of cluster membership (pClust)")
}
...
plotClusteredDispersionHist <- function(lmcmcRes,vDispersion,clustCol)
{
  ...
  h0 <- hist(vDispersion, breaks=breaksSlots,plot=FALSE,freq=FALSE) # freq=TRUE,
  h1 <- hist(vDispersion[lmcmcRes$clustID == 1], breaks=breaksSlots,col=clustCol[1], plot=FALSE,freq=FALSE) #xlim=c(0,10),ylim=c(0,max(h0$counts))
  h2 <- hist(vDispersion[lmcmcRes$clustID == 2], breaks=breaksSlots,col=clustCol[2],main=groupID,xlab = NA, plot=FALSE,freq=FALSE)  #xlim=c(0,10),
...
plotNBFitcdf <- function(vDispersion,resParams)
{
  ...
  for (c in 1:2) {
    for (i in (NROW(resDF.S$q[,1])-ntail):NROW(resDF.S$q[,1]) )
    {
  ...
  c<-1
  plot(((dnbinom(x,size=mean(resParams$r[,c] ), prob= mean(resParams$q[,c]))  ) ), col=clustCol[c],lwd=2,main="NB Model and data distributions",xlim=c(0,XLim),ylim=c(0,2*max(cdfM)),type="l")
...
Negative binomial is a integer distribution and thus for fitting we discretize the dispesion values in 0.1mm intervals.
...
### NF NB Clustering
```{r ,plot-NBMix-clustering-results-NF,fig.show='hold', out.width="25%",dev='png'}
...
plotClusterModel(results.NF.S,vDispersion.NF.S, "NE",getMCMCNBEstimatedParams(results.NF.S,"NE",3))
plotNBFitcdf(vDispersion.NF.S, getMCMCNBEstimatedParams(results.NF.S,"NE",3))
```

### LF NB Clustering
```{r ,plot-NBMix-clustering-results-LF,fig.show='hold', out.width="25%",dev='png'}
...
plotClusterModel(results.LF.S,vDispersion.LF.S, "LE",getMCMCNBEstimatedParams(results.LF.S,"LE",1))
plotNBFitcdf(vDispersion.LF.S,getMCMCNBEstimatedParams(results.LF.S,"LE",3))
```

### DF NB Clustering
```{r ,plot-NBMix-clustering-results-DF,fig.show='hold', out.width="25%",dev='png'}
