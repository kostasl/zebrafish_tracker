...
calcBasicPathStatisticsPerDuration <-function(datDispersion,strG,addPlot=FALSE,powerLawRange = 100)
{
    ...
    if (NROW(datGroupDispersion[ ,"MSD"]) == 0)
    {
      ...
      return(list(meanPooledMSD=NA,groupID=strG,coeff=NA,ptx_line=NA,pty_line=NA))
    }
    ...
    ## Get MSD per Larva
    lLinModel_GE$meanMSDPerLarva <- tapply(datGroupDispersion[ ,"MSD"],datGroupDispersion[ ,"expID"],mean)
    lLinModel_GE$sdMSDPerLarva <- tapply(datGroupDispersion[ ,"MSD"],datGroupDispersion[ ,"expID"],sd)
    lLinModel_GE$countMSDPerLarva <- tapply(datGroupDispersion[ ,"MSD"],datGroupDispersion[ ,"expID"],NROW)
    
    lLinModel_GE$meanPooledMSD <- mean(datGroupDispersion[ ,"MSD"],na.rm=TRUE) ##Add Mean MSD for timeWindow
    lLinModel_GE$sdPooledMSD <- sd(datGroupDispersion[ ,"MSD"],na.rm=TRUE) ##Add Mean MSD for timeWindow
    lLinModel_GE$countPooledMSD <- NROW(datGroupDispersion[!is.na(datGroupDispersion$MSD),]) ##Add Mean MSD for timeWindow
...
plotMSDPerTime <- function(lModelwithMSD,addPlot=FALSE,bUsePooledMean=TRUE)
{
  ...
  {
    if (!is.null(lModelwithMSD[[i]]$meanPooledMSD))
    {
      if (bUsePooledMean)
        vMSD[i] <- lModelwithMSD[[i]]$meanPooledMSD
      else ##Use Group Mean
        vMSD[i] <- mean(lModelwithMSD[[i]]$meanPooledMSDPerLarva)
...
makeMSDPathDurationSummary <- function(bParallel=FALSE)
{
...
vLargeScaletimeWindows <- c(2,10,40,80,95,100,120,180,200,260) #360 No Data
vtimeWindows <- c(vsmallTimeWindows,80,90,95,98,100,120,180,200,260)# c(1,2,4) # vsmallTimeWindows##c(4,10,20,80,200,260,360)
##Process *  Serially * So we do not run out of memory
# Results are plotted and Aggregated In lLinModel
rm(lMSDSummaryData)
try(
lMSDSummaryData <<- readRDS(paste0(strDataExportDir,"statPerPathDurationSummaryResults-1-260sec.rds"))
)
...
##Collect the mean MSD from each Dispersion Calcl integration Window and show mean diffusion
plotMSDPerLarvaPerTime <- function(lModelwithMSD,addPlot=FALSE)
{
  vMSD <- list()
  vTimeWindow <- vector()
  for (i in 1:NROW(lModelwithMSD))
...
```{r mean-squared-distance distribution over larva , fig.show="hold",fig.cap=" Mean squared distance for paths of increasing duration.", out.width="50%", echo=FALSE, cache=FALSE}

##Get Distribution of MSD and Mean MSD for each time integration window 
##load Precalc Results
xbreaks = seq(0,210,10)
t <- as.character(vtimeWindows[9])
#  hist(lMSDSummaryData[["NE"]][[t]]$meanPooledMSDPerLarva, col=colourDataScheme["NE"][[1]],main=paste("NE",t),breaks=xbreaks)
#  hist(lMSDSummaryData[["NL"]][[t]]$meanPooledMSDPerLarva, col=colourDataScheme["NL"][[1]],main=paste("NL",t),breaks=xbreaks)
#  hist(lMSDSummaryData[["LE"]][[t]]$meanPooledMSDPerLarva, col=colourDataScheme["LE"][[1]],main=paste("LE",t),breaks=xbreaks)
#  hist(lMSDSummaryData[["LL"]][[t]]$meanPooledMSDPerLarva, col=colourDataScheme["LL"][[1]],main=paste("LL",t),breaks=xbreaks)
#  hist(lMSDSummaryData[["DE"]][[t]]$meanPooledMSDPerLarva, col=colourDataScheme["DE"][[1]],main=paste("DE",t),breaks=xbreaks)
#  hist(lMSDSummaryData[["DL"]][[t]]$meanPooledMSDPerLarva, col=colourDataScheme["DL"][[1]],main=paste("DL",t),breaks=xbreaks)
...
plotMSDVsTimePerLarva <- function(lModelwithMSD,addPlot=FALSE)
{
  ...
  for (i in 1:NROW(lModelwithMSD))
  {
    if (!is.null(lModelwithMSD[[i]]$meanPooledMSDPerLarva))
    {
        lMSD[[i]] <- lModelwithMSD[[i]]$meanPooledMSDPerLarva
        vTimeWindow[i] <-as.numeric( names(lModelwithMSD[i]) )
    }
  }
  ...
  if (!addPlot)
    plot(vTimeWindow,datMeanMSDVsTimePerLarva[,1],type="p",col=colourDataScheme[lModelwithMSD[[1]]$groupID][[1]],ylim=c(0,200))
...
```{r path-length among larvae across time, fig.show="hold",fig.cap="Mean dispersions over path lengths of increasing durations ", out.width="50%", echo=FALSE, cache=FALSE}
...
```{r prob-of-path-duration,fig.cap="Probability of path duration reveals differences in the amount of time larva dwell within an area. LF spontaneous move out faster than controls"}


plot_ProbOfPathDuration <- function(lMSDSummaryData,addPlot=FALSE)
{
 lLarvaFraction <- list()
  vTimeWindow <- vector()
  for (i in 1:NROW(lMSDSummaryData))
  {
    if (!is.null(lMSDSummaryData[[i]]$countPooledMSDPerLarva))
    {
        lLarvaFraction[[i]] <- nrow(lMSDSummaryData[[i]]$countPooledMSDPerLarva)
        vTimeWindow[i] <-as.numeric( names(lMSDSummaryData[i]) )
    }
  }
  if (!addPlot)
    plot(vTimeWindow,unlist(lLarvaFraction)/lLarvaFraction[[1]],type="b",
         col=colourDataScheme[lMSDSummaryData[[1]]$groupID][[1]],pch=pointTypeScheme[lMSDSummaryData[[1]]$groupID][[1]],
         ylim=c(0,1),ylab="Probability of path duration ",xlab="Time (sec)")
  for (i in 1:ncol(datMeanMSDVsTimePerLarva))
    lines(vTimeWindow,unlist(lLarvaFraction)/lLarvaFraction[[1]],type="b",
          col=colourDataScheme[lMSDSummaryData[[1]]$groupID][[1]],pch=pointTypeScheme[lMSDSummaryData[[1]]$groupID][[1]])
} 

plot_ProbOfPathDuration(lMSDSummaryData[["NE"]])
plot_ProbOfPathDuration(lMSDSummaryData[["NL"]],TRUE)

plot_ProbOfPathDuration(lMSDSummaryData[["LE"]],TRUE)
plot_ProbOfPathDuration(lMSDSummaryData[["LL"]],TRUE)

plot_ProbOfPathDuration(lMSDSummaryData[["DE"]],TRUE)
plot_ProbOfPathDuration(lMSDSummaryData[["DL"]],TRUE)
...
makeMSDPathLengthSummary <- function ()
{
  ...
  saveRDS(datmeanPooledMSDPerPathOft,paste0(strDataExportDir,"datMSDPerPathLength2mmSummaryResults1-70sec.rds"))
...
makeMSDDispersionSummary <- function ()
{
  ...
  saveRDS(datmeanPooledMSDPerPathOft,paste0(strDataExportDir,"datMSDPerDispersion05mmSummaryResults1-70sec.rds"))
  return(datmeanPooledMSDPerPathOft)
}
