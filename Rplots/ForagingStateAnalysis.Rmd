...
rm(ldatDispersion)
ldatDispersion <- list()
#vtimeWindows <- c(2,4,6,8,10,15,20,40,60,80,100,120,180)
vtimeWindows <- c(2,4,10,20,80)
...
saveRDS(lLinModel,paste0(strDataExportDir,"MSDPerPathDurationSummaryResults.rds"))
...
# ie legth to hit the boundary of the observation circle, is only proportional to the circle (in 2D), volume in 3D- 
plot_meanPathLengthPerMSD <- function(datDispersion,strGroupID,addPlot=FALSE)
{
  YLIM <- 100
  datGroupDispersion <- datDispersion[datDispersion$groupID %in% strGroupID,]
  dispBreaks <- seq(1,30,1)
  datGroupDispersion$dispRange <- cut(datGroupDispersion[datGroupDispersion$groupID %in% strGroupID ,"Length" ],breaks=dispBreaks)
  strTitle <- paste("mean MSD per path length", strGroupID,"#",NROW(datGroupDispersion$dispRange))
  
  vmeanPathLengthPerMSD <- tapply(datGroupDispersion$MSD,datGroupDispersion$dispRange,mean,na.rm=TRUE)
  if (!addPlot)
  {
    plot(seq(1,max(dispBreaks),length.out = NROW(vmeanPathLengthPerMSD)),
         vmeanPathLengthPerMSD,pch=16,main=strTitle,xlab="Path Length (mm)",
         ylim=c(0,YLIM),xlim=c(0,max(dispBreaks)),col=colourDataScheme[[strGroupID]],cex=0.7 )
  }else
  {
    points(seq(1,max(dispBreaks),length.out = NROW(vmeanPathLengthPerMSD)),
         vmeanPathLengthPerMSD,pch=16,main=strTitle,xlab="Path Length (mm)",
         col=colourDataScheme[[strGroupID]],cex=0.7 )
    lines(seq(1,max(dispBreaks),length.out = NROW(vmeanPathLengthPerMSD)),
         vmeanPathLengthPerMSD,lwd=1,lty=lineTypeScheme[[strGroupID]])
  }
  
  if (!addPlot)
  {
    propFactor <- 1.0
    rpoints <- seq(0.5,max(dispBreaks), length.out = NROW(vmeanPathLengthPerMSD) )
...
plot_meanPathLengthPerMSD(ldatDispersion[["2"]],'NE')
plot_meanPathLengthPerMSD(ldatDispersion[["4"]],'NE',addPlot = TRUE)
plot_meanPathLengthPerMSD(ldatDispersion[["10"]],'NE',addPlot = TRUE)
plot_meanPathLengthPerMSD(ldatDispersion[["2"]],'NL',addPlot = TRUE)
plot_meanPathLengthPerMSD(ldatDispersion[["4"]],'NL',addPlot = TRUE)
plot_meanPathLengthPerMSD(ldatDispersion[["10"]],'NL',addPlot = TRUE)


plot_meanPathLengthPerMSD(ldatDispersion[["2"]],'LE')
plot_meanPathLengthPerMSD(ldatDispersion[["4"]],'LE',addPlot = TRUE)
plot_meanPathLengthPerMSD(ldatDispersion[["10"]],'LE',addPlot = TRUE)
plot_meanPathLengthPerMSD(ldatDispersion[["2"]],'LL',addPlot = TRUE)
plot_meanPathLengthPerMSD(ldatDispersion[["4"]],'LL',addPlot = TRUE)
plot_meanPathLengthPerMSD(ldatDispersion[["10"]],'LL',addPlot = TRUE)


plot_meanPathLengthPerMSD(ldatDispersion[["2"]],'DE')
plot_meanPathLengthPerMSD(ldatDispersion[["4"]],'DE',addPlot = TRUE)
plot_meanPathLengthPerMSD(ldatDispersion[["10"]],'DE',addPlot = TRUE)
plot_meanPathLengthPerMSD(ldatDispersion[["2"]],'DL',addPlot = TRUE)
plot_meanPathLengthPerMSD(ldatDispersion[["4"]],'DL',addPlot = TRUE)
plot_meanPathLengthPerMSD(ldatDispersion[["10"]],'DL',addPlot = TRUE)
...
plot_meanPathLengthPerDispersion(ldatDispersion[["2"]],'LE')
plot_meanPathLengthPerDispersion(ldatDispersion[["10"]],'LE',addPlot = TRUE)
plot_meanPathLengthPerDispersion(ldatDispersion[["20"]],'LE',addPlot = TRUE)
plot_meanPathLengthPerDispersion(ldatDispersion[["40"]],'LE',addPlot = TRUE)
plot_meanPathLengthPerDispersion(ldatDispersion[["80"]],'LE',addPlot = TRUE)
