...
## TODO add the frames of hunt events for easier classification of hunt events : datHEventDispersion$frameRow
datSubDispersion <- datSubDispersion[sample(1:NROW(datSubDispersion),size=2500),] 

## Lastly / Model and Cluster All Data so we can then run a relative comparison between groupscompare
results.All <- runNBMixGroupClusterModel(datSubDispersion$Dispersion,as.numeric(datSubDispersion$groupID))
write.jagsfile(results.All, file=paste0(strDataExportDir,'jagsModel_NBMixGroupclustDispersion_All.txt') )
save(list=c("results.All","datSubDispersion"),file=paste0(strDataExportDir,'jagsModelResults_NBMixGroupclustDispersion_All.RData') )
...
datDispersion[datSubDispersion$frameRow,"clustID"] <- lparamsAll$clustID
