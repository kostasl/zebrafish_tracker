...
makeMSDPathLengthSummary <- function ()
{
  ...
  saveRDS(lMSDStat,paste0(strDataExportDir,"lMSDPerPathLengthSummaryResults1-60sec.rds"))
  saveRDS(datmeanMSDPerPathOft,paste0(strDataExportDir,"datMSDPerPathLengthSummaryResults1-60sec.rds"))
...
try(
{datMSDVsPathlengthSummaryData <<- readRDS(paste0(strDataExportDir,"datMSDPerPathLengthSummaryResults1-60sec.rds"));
lMSDStat <<- readRDS(paste0(strDataExportDir,"lMSDPerPathLengthSummaryResults1-60sec.rds"))}
)
if (!exists("lMSDSummaryData"))
{
...
for (strG in vGroups)
{
  ...
  if (!baddPlot)
  {
    plot(xtick, datmeanMSDPerPathOft[[strG]],col=colourDataScheme[[strG]],pch=pointTypeScheme[[strG]],lty=lineTypeScheme[[strG]],type="b",
         xlab="Path length (mm)",ylab="Mean Squared Displacement (mm)",xaxt="n" ,ylim=c(0,120))
  
    axis(side=1, at=xtick, labels = names(lMSDStat[["DE"]]$`60`$breaks))  
  }
  else
  ...
  propFactor <- 2.0
  lines(xtick,theoreticalMeanLength(xtick,propFactor ),pch=2,cex=0.5 ) #Up Lim
