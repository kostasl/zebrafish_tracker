...
```{r  bout and turn structure , fig.show="hold",fig.cap="Bout Frequency and free paths", out.width="100%", echo=FALSE, cache=TRUE, results=FALSE}
...
getTrajectoryTurnsAndRuns<- function(TrackLetInfo)  
{
 ...
 #### PROCESS BOUTS ###
  ...
  blockTurnFramesIDs <- split(TurnboutsIdx+as.numeric(TrackLetInfo$startFrame), 
                                     ##Sum Increments When the following Indicators Of Changing Class Become TRUE (I added Track Splitters when Exp or Event Change)
                               cumsum(c(1, diff(TurnboutsIdx) != 1 )  ) 
                              )
  ...
  for (tID in as.character(datTrackletTurns[abs(datTrackletTurns$turnMagnitude) >  MIN_TURN_SIZE_THRES, ]$turnID) )
  {
      #message(tID,":",datTrackletTurns[tID,]$turnMagnitude)
      datDomainTrajectory[datDomainTrajectory$frameN >= datTrackletTurns[tID,]$turnStartFrame &
                        datDomainTrajectory$frameN <= datTrackletTurns[tID,]$turnEndFrame,"IndTurnBout"] = 1
  ...
  TurnboutsIdx <- which(datDomainTrajectory$IndTurnBout == 1)
  ...
  plot(datDomainTrajectory$posX,datDomainTrajectory$posY,col=c("black","red","purple")[datDomainTrajectory$IndMotionBout+1+datDomainTrajectory$IndTurnBout*2] )  
  ...
  ##Show Turns /Bouts##
  #points(datDomainTrajectory$posX,datDomainTrajectory$posY,col=c("black","purple","red")[+1] )  
  #plot(vEventSpeed_smooth_mm,col=c("black","red","purple")[datDomainTrajectory$IndTurnBout+1],type="p",cex=0.1 )  
  #points(TailboutsIdx,vEventSpeed_smooth_mm[TailboutsIdx],col="red",type="p",cex=0.1)
  #plot(vTurnSpeed,col=c("black","red","purple")[datDomainTrajectory$IndTurnBout+1],pch=2,cex=1) 
  #plot(vTailDispFilt,type='l')
  
  # plot(datDomainTrajectory$posX,datDomainTrajectory$posY,col=c("black","red","purple")[datDomainTrajectory$IndTurnBout+1] )  
  # ##Check Change IN angle for Turns Detected
  # plot(vAngleDisplacement)
  # points(which(datDomainTrajectory$IndTurnBout == 1),vAngleDisplacement[datDomainTrajectory$IndTurnBout == 1],col="red")
  # 
  # plot(vAngleDisplacement)
  # j <- 2
  # idxRange <-  (turnStartFrame[j]):(turnEndFrame[j]) - as.numeric(TrackLetInfo$startFrame)+1
  # points(idxRange,vAngleDisplacement[idxRange],col="red")
  # 
```
