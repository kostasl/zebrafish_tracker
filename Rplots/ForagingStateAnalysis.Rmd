...
plot_loglogDisplacementWithinT <- function(strG,datDispersion,tsec_timeWindow)
{
  datGroupDispersion <- datDispersion[datDispersion$groupID %in% strG &
                                          !is.na(datDispersion$DisplacementSq),]
 
  hist_GDisplacement <- hist(sqrt(datGroupDispersion$DisplacementSq ),plot=FALSE )
  #hist_GDisplacement$counts/sum(hist_GDisplacement$counts
  plot(hist_GDisplacement$mids, hist_GDisplacement$density, log="xy", xlim=c(0.1,20), ylim=c(10^-4,1),
 ...
 ##Evoked Conditions 
  strGE <- vGroups[which(vGroups == strG  ) - 1] # Find the complementrary Group-Conditions
   datGroupDispersion <- datDispersion[datDispersion$groupID %in% strGE &
                                          !is.na(datDispersion$DisplacementSq),]

  hist_GDisplacement <- hist(sqrt(datGroupDispersion$DisplacementSq ),plot=FALSE ) #,breaks=hist_GDisplacement$breaks
  ##Plot Path Duration Distribution 
  lines(hist_GDisplacement$mids,hist_GDisplacement$density, 
          type="b",
          col=colourDataScheme[[strGE]],
          pch=pointTypeScheme[[strGE]])
  ...
  xPt <- seq(0,20,0.1)
  lines(xPt,dnorm(xPt,mean=Emean,sd=Esd),lty=2,lwd=2,pch=3)
...
tsec_timeWindow <-1
datDispersion <- loadDispersionData_norm(tsec_timeWindow,forcereload = TRUE)

#par(mfrow=c(3,1))
for (strG in vGroups[seq(2,6,2)])
{
  plot_loglogDisplacementWithinT(strG,datDispersion,tsec_timeWindow)
}
