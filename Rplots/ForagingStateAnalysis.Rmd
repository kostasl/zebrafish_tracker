...
datSubDispersion  <- datDispersion[ !is.na(datDispersion$Dispersion), ]
datSubDispersion <- datSubDispersion[seq(1,NROW(datSubDispersion),by=nsubsampleInterval ),]
strOutFilename <- "jagsModelResults_NBMixGroupclustDispersion_All"
##Quicker Test - Subsampling
if (bQuickTest)
{
    datSubDispersion <- datSubDispersion[sample(1:NROW(datSubDispersion),size=3500),] 
    strOutFilename <- "jagsModelResults_NBMixTestGroupclustDispersion_All"
}
# add the exact frames of hunt events so we can cluster them in exploit/Explore easly datHEventDispersion$frameRow
# Frame Records of Huntevents not in SubSampled frames
missinghuntEvents.frameROW <- datHEventDispersion[!(datHEventDispersion$frameRow %in% datSubDispersion$frameRow),"frameRow"] 
...
```{r plot-NBMix-Groupclustering-results-Spontaneous,fig.show='hold', out.width="33%",dev='png',include=FALSE}
...
```
#### Evoked
```{r plot-NBMix-Groupclustering-results-Evoked,fig.show='hold', out.width="33%",dev='png',include=FALSE}
lparamsAll <- getMCMCNBGroupEstimatedParams(results.All,"All",2)
...
datHEventDispersion <- merge(datHEventDispersion, cbind(frameRow=datSubDispersion$frameRow,clustID=datSubDispersion$clustID),
                             by="frameRow",all.x =TRUE,no.dups = FALSE)
...
barplot(table(datHEventDispersion$clustID,datHEventDispersion$groupID),
        col=c(colourClusters[2],colourClusters[1]),
        main="Hunt-events per cluster and cond-group")
legend("topleft",legend=(paste("#",table(datHEventDispersion$clustID) )),fill=c(colourClusters[2],colourClusters[1])  )
...
```{r hunt-outcomes-per-globally clustered dispersions-evoked, fig.show="hold", out.width="50%", echo=FALSE, cache=TRUE }
## Check NF
## SUCCESS
plotClusteredHuntEventsHist_norm(datHEventDispersion[datHEventDispersion$groupID=='NL' &
...
plotClusteredHuntEventsHist_norm(datHEventDispersion[datHEventDispersion$groupID=='LL' &
                      grepl("Success", as.character(  convertToScoreLabel(datHEventDispersion$huntScore) ) ) ,],
                      c(colourClusters[2],colourClusters[1]) ) 
legend("topright",legend="Success")
...
```{r hunt-outcomes-per-globally clustered dispersions-spontaneous, fig.show="hold", out.width="50%", echo=FALSE, cache=TRUE }
## Check NF
## SUCCESS
plotClusteredHuntEventsHist(datHEventDispersion[datHEventDispersion$groupID=='NE' ,],
                      c(colourClusters[2],colourClusters[1]) ) 
legend("topright",legend="Spontaneous")

## Check LF 
plotClusteredHuntEventsHist(datHEventDispersion[datHEventDispersion$groupID=='LE'  ,],
                      c(colourClusters[2],colourClusters[1]) ) 
legend("topright",legend="Spontaneous")
#DF
plotClusteredHuntEventsHist(datHEventDispersion[datHEventDispersion$groupID=='DE'  ,],
                      c(colourClusters[2],colourClusters[1]) ) 
legend("topright",legend="Spontaneous")
...
## Clustered based on Global NB Mix Model  
