...
calcFreePathLength <- function(datTrackBouts)
{
    ...
    if (NROW(datStraightMotionBouts) > 1)
    {
      ...
      lTrackFreePaths <- tapply(unlist(datTrackBouts[datTrackBouts$turn_deg < MIN_TURN_SIZE_THRES,  ]$travelDistance_mm), ##pathLength_mm
...
plotBoundedFreePath <- function(datBoundedTrack,strG,addPlot=FALSE,XLIM=20)  
{
  ...
  for (R in names(ldatFreePathD) )
  {
    ...
    {
        i <- i + 1
        meanBoutLengthPathPerDomain[[i]] <-list( meanFreePath = mean( unlist(ldatFreePathD[[R]][ldatFreePathD[[R]]$groupID == strG & 
                                                                       ldatFreePathD[[R]]$turn_deg < MIN_TURN_SIZE_THRES &
                                                                   ldatFreePathD[[R]]$boutSeqID > 0 ,"travelDistance_mm"]), na.rm = T),
                                                 sdFreePath = sd( unlist(ldatFreePathD[[R]][ldatFreePathD[[R]]$groupID == strG & 
                                                                       ldatFreePathD[[R]]$turn_deg < MIN_TURN_SIZE_THRES &
                                                                   ldatFreePathD[[R]]$boutSeqID > 0 ,"travelDistance_mm"]), na.rm = T),
                                                 n = NROW(sd( unlist(ldatFreePathD[[R]][ldatFreePathD[[R]]$groupID == strG & 
                                                                       ldatFreePathD[[R]]$turn_deg < MIN_TURN_SIZE_THRES &
                                                                   ldatFreePathD[[R]]$boutSeqID > 0 ,"travelDistance_mm"]), na.rm = T)),
                                                groupID=strG,
 ...
 ### Mean Free Path
  ...
  for (R in names(ldatFreePathsPerDomain) )
  {
    ...
    {
        i <- i + 1
        meanFreePathPerDomain[[i]] <-list( meanFreePath = mean(unlist(ldatFreePathsPerDomain[[R]][ldatFreePathsPerDomain[[R]]$groupID == strG,"freePathLength_mm"]), na.rm = T),
                                                 sdFreePath = sd( unlist(ldatFreePathsPerDomain[[R]][ldatFreePathsPerDomain[[R]]$groupID == strG,"freePathLength_mm"]), na.rm = T),
                                                 n = NROW(sd( unlist(ldatFreePathsPerDomain[[R]][ldatFreePathsPerDomain[[R]]$groupID == strG ,"freePathLength_mm"]), na.rm = T)),
                                                groupID=strG,
                                                DomainRadius = as.numeric(R))
    }
  }
 ...
 ## Distribution of Free Path Lengths For Domain Size ##
   ...
   cD <- as.character(DomainRadius) 
   addPlot <- FALSE
   maxFreePath <- max(tapply(unlist(ldatFreePathsPerDomain[[cD ]]$freePathLength_mm),ldatFreePathsPerDomain[[cD ]]$groupID,max,na.rm=TRUE))
  for (strG in vGroups)
  {
