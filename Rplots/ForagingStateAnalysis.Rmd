...
```{r dispersal-RJags-GaussianMix-pooled-data model,include=TRUE}
...
```{r dispersion-GaussianMixture-clustering-model, eval=FALSE, include=FALSE}
...
save(list=c("results.DF.S","vDispersion.DF.S"),file=paste0(strDataExportDir,'jagsModelResults_clustDispersion_DE.RData') )
  ...
  write.jagsfile(resultsGmix.All, file=paste0(strDataExportDir,'jagsModel_clustDispersion_DE.txt') )
...
# results.NF.E <- run.jags(paste0(strDataExportDir,'jagsModel_clustDispersion_NL.txt'),sample=100)
# save(list=c("results.NF.E","vDispersion.NF.E"),file=paste0(strDataExportDir,'jagsModelResults_clustDispersion_NL.RData') )
...
```{r dispersion-clustering-plot-results,fig.show='hide', out.width="33%", eval=TRUE, include=FALSE,cache=FALSE,dev='png'}
...
plotNBFitcdf <- function(vDispersion,resParams)
{
  ...
  c<-1
  plot(((dnbinom(x,size=mean(resParams$r[,c] ), prob= mean(resParams$q[,c]))  ) ), col=clustCol[c],lwd=2,main="NB Model and data distributions",xlim=c(0,XLim),ylim=c(0,2*max(cdfM)),type="l",ylab="Density",xlab="Dispersal (mm)")
  lines(density(vDispersion[resParams$clustID == c]*10), col=clustCol[c],lty=2,lwd=3)
...
```{r plot-clustering-results-NF,fig.show='hold', out.width="33%",dev='png',warnings=FALSE}
...
```{rplot-clustering-results-LF,fig.show='hold', out.width="33%",dev='png'}
...
\]
, whereby marginalizing over the top-level model for ?t, we recover a negative-binomial distribution for yt
...
to model number of disperal step Hunt Events Per Larvae
...
```{r dispersal-RJags-NBMixture-Run-model,include=FALSE,eval=FALSE}
...
plotClusterModel(results.All,datSubDispersion$Dispersion, "LL",getMCMCNBEstimatedParams(results.All,"LL",2))
plotNBFitcdf(datSubDispersion$Dispersion,getMCMCNBEstimatedParams(results.All,"LL",2))
```
...
To compare differences in dispersal between groups It would be best to define consistent models for the clusters that define the exploration and exploitation state by using data across groups. From there we may then compare the relative occupancy in each state depending on test condition and rearing group, such that we can detect changes in behaviour relevant to a global frame of reference. Aims:
...
Thus we need toconstruct a hierarchical statistical model to describe the mean group dispersal behaviours in spontaneous and evoked conditions, based on the estimated behaviour of $i=60$ larva from each group $G$.
...
load(file=paste0(strDataExportDir,'jagsModelResults_NBMixGroupclustDispersion_All.RData') )
```


```{r dispersal-RJags-NBMixture-AllData-Run-model,include=FALSE,eval=FALSE}
## Cluster Using Negative Binomial Model
...
runNBMixGroupClusterModel <- function(vDispersion,vGroupID)
{
  ##Make Integer Discrete
  vDispersion <- round(vDispersion*10.0)
...
datSubDispersion  <- datDispersion[ !is.na(datDispersion$Dispersion), ]
datSubDispersion <- datSubDispersion[seq(1,NROW(datSubDispersion),by=nsubsampleInterval ),]
strOutFilename <- "jagsModel_NBMixGroupclustDispersion_All"
##Quicker Test - Subsampling
if (bQuickTest)
{
    datSubDispersion <- datSubDispersion[sample(1:NROW(datSubDispersion),size=3500),] 
    strOutFilename <- "jagsModel_NBMixTestGroupclustDispersion_All"
}
# add the exact frames of hunt events so we can cluster them in exploit/Explore easly datHEventDispersion$frameRow
# Frame Records of Huntevents not in SubSampled frames
missinghuntEvents.frameROW <- datHEventDispersion[!(datHEventDispersion$frameRow %in% datSubDispersion$frameRow),"frameRow"] 
...
## Lastly / Model and Cluster All Data so we can then run a relative comparison between groupscompare
results.All <- runNBMixGroupClusterModel(datSubDispersion$Dispersion,as.numeric(datSubDispersion$groupID))
...
### Results from NB Group clustering 
```{r plot-NBMix-Groupclustering-results-All,fig.show='hold', out.width="33%",dev='png'}
...
```{r plot-NBMix-Groupclustering-results-Spontaneous,fig.show='hold', out.width="33%",dev='png'}
...
```
#### Evoked
```{r plot-NBMix-Groupclustering-results-Evoked,fig.show='hold', out.width="33%",dev='png'}
lparamsAll <- getMCMCNBGroupEstimatedParams(results.All,"All",2)
...
## Examine relationship of hunt-outcome to *Clustered foraging state*
...
```{r results from clustered dispersion on hunt-events, fig.show="hold", out.width="50%", echo=FALSE, cache=TRUE }
...
datHEventDispersion <- merge(datHEventDispersion, cbind(frameRow=datSubDispersion$frameRow,clustID=datSubDispersion$clustID) ,by="frameRow",all.x =TRUE,no.dups = FALSE)


hist(datHEventDispersion[datHEventDispersion$clustID == 1,"Dispersion" ],col=colourClusters[2],breaks =seq(0,11))
hist(datHEventDispersion[datHEventDispersion$clustID == 2,"Dispersion" ],col=colourClusters[1],breaks =seq(0,11),add=TRUE)

##Check The cluster with Most Hunt Events (Appears to Be )
boxplot(Dispersion ~ clustID,data=datHEventDispersion,col=c(colourClusters[2],colourClusters[1]) )

hist(datHEventDispersion[datHEventDispersion$clustID == 1,"clustID" ],col=colourClusters[2],breaks =seq(0,2),main="Count Hunt Events in each cluster")
hist(datHEventDispersion[datHEventDispersion$clustID == 2,"clustID" ],col=colourClusters[1],breaks =seq(0,2),add=TRUE)
```
...
## Check NF
## SUCCESS
hist(datHEventDispersion[datHEventDispersion$groupID=='NL' &
                           grepl("Success", as.character(  convertToScoreLabel(datHEventDispersion$huntScore) ) )&
                           datHEventDispersion$clustID == 1, ]$Dispersion_larval_norm,breaks=nBreaks,main="NF Capture Success",xlab="Dispersion ",xlim=c(0,1),col=colourClusters[2],ylim=c(0,ylimUp))#colourDataScheme$NF$Evoked
hist(datHEventDispersion[datHEventDispersion$groupID=='NL' &
                           grepl("Success", as.character(  convertToScoreLabel(datHEventDispersion$huntScore) ) )&
                           datHEventDispersion$clustID == 2, ]$Dispersion_larval_norm,breaks=nBreaks,main="NF Capture Success",xlab="Dispersion ",xlim=c(0,1),col=colourClusters[1],ylim=c(0,ylimUp),add=TRUE)#colourDataScheme$NF$Evoked

##FAIL
hist(datHEventDispersion[datHEventDispersion$groupID=='NL' &
                           grepl("Fail", as.character(  convertToScoreLabel(datHEventDispersion$huntScore) ) )&
                           datHEventDispersion$clustID == 1, ]$Dispersion_larval_norm,breaks=nBreaks,main="NF Capture Failure",xlab="Dispersion ",xlim=c(0,1),col=colourClusters[2],ylim=c(0,ylimUp)) ##colourDataScheme$NF$Spont

hist(datHEventDispersion[datHEventDispersion$groupID=='NL' &
                           grepl("Fail", as.character(  convertToScoreLabel(datHEventDispersion$huntScore) ) )&
                           datHEventDispersion$clustID == 2, ]$Dispersion_larval_norm,breaks=nBreaks,main="NF Capture Failure",xlab="Dispersion ",xlim=c(0,1),col=colourClusters[1],ylim=c(0,ylimUp),add=TRUE) ##colourDataScheme$NF$Spont



## Check LF Successs/Fail Only
# SUCCESS
  hist(datHEventDispersion[datHEventDispersion$groupID=='LL' &
                             grepl("Success", as.character( convertToScoreLabel(datHEventDispersion$huntScore) ) ) &
                             datHEventDispersion$clustID == 1, ]$Dispersion_larval_norm,breaks=nBreaks,main="LF Capture Success",xlab="Dispersion ",xlim=c(0,1),col=colourClusters[2],ylim=c(0,ylimUp)) ##colourDataScheme$LF$Evoked
  hist(datHEventDispersion[datHEventDispersion$groupID=='LL' &
                             grepl("Success", as.character( convertToScoreLabel(datHEventDispersion$huntScore) ) ) &
                             datHEventDispersion$clustID == 2, ]$Dispersion_larval_norm,breaks=nBreaks,main="LF Capture Success",xlab="Dispersion ",xlim=c(0,1),col=colourClusters[1],ylim=c(0,ylimUp),add=TRUE) ##colourDataScheme$LF$Evoked
  
  
# FAIL  
  hist(datHEventDispersion[datHEventDispersion$groupID=='LL' &
                             grepl("Fail", as.character( convertToScoreLabel(datHEventDispersion$huntScore) ) ) &
                             datHEventDispersion$clustID == 1, ]$Dispersion_larval_norm,breaks=nBreaks,main="LF Capture Fail",xlab="Dispersion ",xlim=c(0,1),col=colourClusters[2],ylim=c(0,ylimUp)) ##colourDataScheme$LF$Evoked
  hist(datHEventDispersion[datHEventDispersion$groupID=='LL' &
                             grepl("Fail", as.character( convertToScoreLabel(datHEventDispersion$huntScore) ) ) &
                             datHEventDispersion$clustID == 2, ]$Dispersion_larval_norm,breaks=nBreaks,main="LF Capture Fail",xlab="Dispersion ",xlim=c(0,1),col=colourClusters[1],ylim=c(0,ylimUp),add=TRUE) ##colourDataScheme$LF$Evoked
  

## Check DF
# Success
hist(datHEventDispersion[datHEventDispersion$groupID=='DL' &
                           grepl("Success", as.character(  convertToScoreLabel(datHEventDispersion$huntScore) ) )&
                           datHEventDispersion$clustID == 1, ]$Dispersion_larval_norm,breaks=nBreaks,main="DF Capture Success",xlab="Dispersion ",xlim=c(0,1),col=colourClusters[2],ylim=c(0,ylimUp))#colourDataScheme$NF$Evoked
hist(datHEventDispersion[datHEventDispersion$groupID=='DL' &
                           grepl("Success", as.character(  convertToScoreLabel(datHEventDispersion$huntScore) ) )&
                           datHEventDispersion$clustID == 2, ]$Dispersion_larval_norm,breaks=nBreaks,main="DF Capture Success",xlab="Dispersion ",xlim=c(0,1),col=colourClusters[1],ylim=c(0,ylimUp),add=TRUE)#colourDataScheme$NF$Evoked


#FAIL
hist(datHEventDispersion[datHEventDispersion$groupID=='DL' &
                           grepl("Fail", as.character(  convertToScoreLabel(datHEventDispersion$huntScore) ) )&
                           datHEventDispersion$clustID == 1, ]$Dispersion_larval_norm,breaks=nBreaks,main="DF Capture Fail",xlab="Dispersion ",xlim=c(0,1),col=colourClusters[2],ylim=c(0,ylimUp))#colourDataScheme$NF$Evoked
hist(datHEventDispersion[datHEventDispersion$groupID=='DL' &
                           grepl("Fail", as.character(  convertToScoreLabel(datHEventDispersion$huntScore) ) )&
                           datHEventDispersion$clustID == 2, ]$Dispersion_larval_norm,breaks=nBreaks,main="DF Capture Fail",xlab="Dispersion ",xlim=c(0,1),col=colourClusters[1],ylim=c(0,ylimUp),add=TRUE)#colourDataScheme$NF$Evoked
