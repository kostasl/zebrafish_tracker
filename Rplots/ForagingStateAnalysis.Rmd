...
for (strG in vGroups)
{
    for (tt in names(lMSDSummaryData[[strG]]))
    {
      ...
      if (any(vExpID %in% datExpPairs$expID.S) )
      {
        vExpID_C <-  as.character(datExpPairs[as.character(datExpPairs$expID.S) %in% vExpID,]$expID.E )
        ##Fix In case of missing records
        vMSD <- vMSD[vExpID %in% as.character(datExpPairs$expID.S)]
        vExpID <- vExpID_C
      }
      ...
      XRange <- c(0,5)
      pBw <- 0.05
      vMeanCorr[[tt]] <- cor(datMSDvsLength$LengthPx*DIM_MMPERPX,datMSDvsLength$meanMSD, method="pearson")
  ...
  datmuMSDtoSize_corr[[strG]] <-  ldply ((vMeanCorr), data.frame)
