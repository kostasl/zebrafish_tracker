...
*Main Findings so far :*
  ...
  21. **Bout Structure**:
    1. LF Lower number of of bouts per free-path
    2. Longer distance traveled per bout *longer bouts*
    3. Bout durations decrease for LF,NF in evoked conditions, but not for DF.
...
```{r bout-duration per domain size, out.width="33%", echo=FALSE, cache=FALSE, results=FALSE}
#  MIN_TURN_SIZE_THRES
...
```
Hint that in LE bouts last longer, which could also explain the that LE travel quicker.
...
*Bout distance distribution shows increases for LF, compared to DF and NF. In all groups bout distances are shorter in evoked rather than spontaneous conditions.
I need to verify that this is not due to hunt events.
