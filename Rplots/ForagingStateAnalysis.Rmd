...
```{r  path-length-All duration- per dispersion, fig.show="hold", out.width="50%", echo=FALSE, cache=FALSE,fig.cap="Mean path length per dispersion size for full event path-lengths " }
...
To understand how the macroscopic observations within the domain-constrained paths arise, I take each domain size (R=1...11mm) and analyze paths into bout sequences, measuring the angle change, distance traveled (straight line from start-end of bout), and the path lenght inscribed during the bout. 
...
calcFreePathLength <- function(datTrackBouts)
{
    if (NROW(datTrackBouts) > 1)
    {
      ...
      lTrackFreePaths <- tapply(unlist(datTrackBouts[datTrackBouts$turn_deg < MIN_TURN_SIZE_THRES,  ]$pathLength_mm), 
                                unlist(datTrackBouts[datTrackBouts$turn_deg < MIN_TURN_SIZE_THRES,  ]$freePathID),sum )

    }else # Return Path lentgh of the only available Single Bout that is not a turn
      lTrackFreePaths <- array(datTrackBouts[datTrackBouts$turn_deg < MIN_TURN_SIZE_THRES,  ]$pathLength_mm,1)
    

  return(lTrackFreePaths)          
}
...
plotBoundedFreePath <- function(datBoundedTrack,strG,addPlot=FALSE)  
{
  ...
  ## Combine Bouts of a Radius onto a Single Data  frame for all Groups /Exp.
  ...
  for (DomainRadius in vAvailableDomainRadius)
  {
    ...
    for (strGroup in names(lBoundedTrackInfo) )
    {
      ...
      for (trackID in rownames( groupdomainTracks ) )
      {
        if (is.null(lMeanPathLengthAll[[trackID]]))
          ...
          lTrackFreePaths <- calcFreePathLength(datTrackBouts)
          nP = NROW(lTrackFreePaths)
          lFreePathdf[[trackID]] <- cbind.data.frame(freePathLength_mm=lTrackFreePaths, 
    ...
    ldatFreePathsPerDomain[[cD ]] <- datBoutsPerDomain[datBoutsPerDomain$DomainRadius == cD ,]
  ...
  meanBoutLengthPathPerDomain <- list()
   i<- 0
  for (R in names(ldatFreePathD) )
  {
    
    for (strG in vGroups)
    {
        i <- i + 1
        meanBoutLengthPathPerDomain[[i]] <-list( meanFreePath = mean( ldatFreePathD[[R]][ldatFreePathD[[R]]$groupID == strG & 
                                                                       ldatFreePathD[[R]]$turn_deg < MIN_TURN_SIZE_THRES &
                                                                   ldatFreePathD[[R]]$boutSeqID > 0 ,"pathLength_mm"], na.rm = T),
                                                 sdFreePath = sd( ldatFreePathD[[R]][ldatFreePathD[[R]]$groupID == strG & 
                                                                       ldatFreePathD[[R]]$turn_deg < MIN_TURN_SIZE_THRES &
                                                                   ldatFreePathD[[R]]$boutSeqID > 0 ,"pathLength_mm"], na.rm = T),
                                                 n = NROW(sd( ldatFreePathD[[R]][ldatFreePathD[[R]]$groupID == strG & 
                                                                       ldatFreePathD[[R]]$turn_deg < MIN_TURN_SIZE_THRES &
                                                                   ldatFreePathD[[R]]$boutSeqID > 0 ,"pathLength_mm"], na.rm = T)),
                                                groupID=strG,
 ...
 ### MEAN FREE PATH PER DOMAIN SIZE
 addPlot =FALSE
 for (strG in vGroups)
 {
  if (!addPlot)
  plot(datmeanBoutLengthPathPerDomain[datmeanBoutLengthPathPerDomain$groupID ==strG,"DomainRadius"],
       datmeanBoutLengthPathPerDomain[datmeanBoutLengthPathPerDomain$groupID ==strG,"meanFreePath"],
       col=colourDataScheme[strG][[1]],pch=pointTypeScheme[strG][[1]],ylim=c(0,3),ylab="Mean motion-bout size (mm)",xlab="Domain radius (mm)" )
  else
...
#
 ...
 ##TODO: FIX THIS -  This IS nOT FREE Path ! Only bout travel-distance
  ...
  for (strG in vGroups)
  {
   datBoundedTrack <- ldatFreePathD[[cD ]][ldatFreePathD[[cD ]]$groupID == strG & ldatFreePathD[[cD ]]$travelDistance_mm < 30,]
   ##Choose Turn-Motion Bouts, No Stops
   plotBoundedFreePath(datBoundedTrack[datBoundedTrack$boutSeqID >0,],strG,addPlot)
   message(paste(strG,":",mean(unlist(datBoundedTrack[datBoundedTrack$boutSeqID >0,"travelDistance_mm"])) ) )
   addPlot <- TRUE
  }
...
datTurnsPerDomain <- datBoutsPerDomain

  ## Count Turns Per Tracklet
  turnsPerPath <- list()
  for (strG in vGroups)
  {
      turnsPerPath[[strG]] <- with(datTurnsPerDomain[datTurnsPerDomain$groupID == strG & datTurnsPerDomain$turn_deg > MIN_TURN_SIZE_THRES,],{
  ...
  for (strG in vGroups)
  {
   ...
   addPlot <- TRUE
   message(paste(strG,":", mean(unlist( turnsPerPath[[strG]] ),na.rm=TRUE )   ) )
  }
 ...
 ##Turn Magnitude
  for (strG in vGroups)
  {
    ...
    hist(datGTurns$turn_deg,breaks=seq(-80,80,2),main=paste(strG,"R",DomainRadius) ) #ylim=c(0,80)
  }
