...
## Interbout Intervals & Pauses
addPlot <- FALSE
for (strG in vGroups)
{
    vInterBoutInterval <- ldatBoutsPerDomain[[cD]][ldatBoutsPerDomain[[cD]]$groupID == strG & 
                             ldatBoutsPerDomain[[cD]]$boutSeqID == 0 &
                                ldatBoutsPerDomain[[cD]]$duration_sec > 0 ,"duration_sec"]
    hist_IBI <- hist(vInterBoutInterval,
              breaks=seq(0,60,0.2),plot=FALSE   )
    message(paste("Inter-Bout Interval ",strG," mu:",mean(vInterBoutInterval) ))
    if (!addPlot){
        plot(hist_IBI$mids,hist_IBI$counts/sum(hist_IBI$counts),log="xy",col=colourDataScheme[strG][[1]],pch=pointTypeScheme[strG][[1]],type="b",xlim=c(0.1,20),
             xlab="Inter-bouts interval (sec)",ylab="Log normalized inter-bout interval frequency")
    }
    else{
        lines(hist_IBI$mids,hist_IBI$counts/sum(hist_IBI$counts),col=colourDataScheme[strG][[1]],pch=pointTypeScheme[strG][[1]],type="b")
    }
   addPlot <- TRUE
}
...
Found that *inter-bout intervals are shorter for LF*, but also that long > 2 sec pauses are missing from LF, while naive fish will stay in one place, LF remain active.
*LF does not do long-pauses between starting to move* as seen in both DF and NF.
...
datTurnsPerDomain <- datBoutsPerDomain
 ...
 ##Turn Magnitude distributions
  ...
  for (strG in vGroups)
  {
    ...
    #if (!addPlot)
    #  plot(density(datGTurns$turn_deg,bw=BW ),col=colourDataScheme[strG][[1]],pch=pointTypeScheme[strG][[1]],lty=lineTypeScheme[strG][[1]],lwd=3,ylim=c(0,0.08),main=strG)
    #else
