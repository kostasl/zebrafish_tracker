...
calcPathLengthsInDomain <- function(datAllFrames,vgroupID,DomainRadius)
  ...
  #  for (DomainRadius in c(1,2,3,4)) #mm
  #  {
  #   for(g in vGroups)
  #   {
  #     message("Group:",g)
...
```{r distribution of number of paths per domain sizes,fig.show="hold", out.width="50%", echo=FALSE, cache=TRUE,fig.cap="Distribution of number of domain crossings, with increasing domain radius shows effect of prey is to constraint movement within smaller central regions, as revealed by the breaking down of the linear invariant regime in evoked conditions. (*Note regions 10+11mm have been concatenated*)"}  
datMeanPathLengthAll <- readRDS(file=paste0(strDataExportDir,"/stat_boundedPathsB_RAllmm.rds") )
...
```{r stat-Path-calculations , fig.show="hold", out.width="50%", echo=FALSE, cache=TRUE }
...
Thus  not only are Dispersion and mean Path length near linearly related, but so is mean displacement, albet some differences between conditions are apparrent,
...
```{r  extract bout and turn structure from domain paths,eval=FALSE,out.width="100%", echo=FALSE, cache=TRUE, results=FALSE}
  ...
  MIN_GAP_FRAMES_BETWEEN_TURNS <- 10
...
getTrajectoryTurnsAndRuns<- function(TrackLetInfo)  
{
 ...
 #### PROCESS BOUTS ###
  ...
  vDeltaBodyAngle      <- diffPolar(datDomainTrajectory$BodyAngle) #(  ( 180 +  180/pi * atan2(datRenderPrey$Prey_X -datRenderPrey$posX,datRenderPrey$posY - datRenderPrey$Prey_Y)) -datRenderPrey$BodyAngle    ) %% 360 - 180
  ...
  #MoveboutsIdx <- detectMotionBouts(vEventSpeed_smooth_mm,G_THRES_MOTION_BOUT_SPEED) # +min()
  #TailboutsIdx <- detectTailBouts(lwlt$freqMode)
  
  ##Note that sensitivity of this Determines detection of 1st turn to Prey
  TurnboutsIdx <- detectMotionBouts(vTurnSpeed,MIN_BOUT_TURN_SPEED)  ##detectTurnBouts(vTurnSpeed,lwlt$freqMode,MIN_BOUT_TURN_SPEED)
  
  ## Identify Blocks of frames belonging to the same turn
  blockTurnFramesIDs <- split(TurnboutsIdx+as.numeric(TrackLetInfo$startFrame), 
                                     ##Sum Increments When the following Indicators Of Changing Class Become TRUE (I added Track Splitters when Exp or Event Change)
                               cumsum(c(1, diff(TurnboutsIdx) > MIN_GAP_FRAMES_BETWEEN_TURNS )  ) 
                              )
  #tblockTurnFramesIDsurnBlock <- rle(datDomainTrajectory$IndTurnBout)
  ## Tabulate DEtected Turns 
  datTurnIds <- ldply (blockTurnFramesIDs, data.frame)
  names(datTurnIds) <- c("turnID","frameN")
  turnStartFrame <- tapply(datTurnIds$frameN,datTurnIds$turnID,min)
  turnEndFrame <- tapply(datTurnIds$frameN,datTurnIds$turnID,max)
  turnMagnitude <- vAngleDisplacement[(turnEndFrame) - as.numeric(TrackLetInfo$startFrame)-1   ] - vAngleDisplacement[(turnStartFrame) - as.numeric(TrackLetInfo$startFrame)  +1 ]
  if (length(datTurnIds) == 1 )
    turnMagnitude <- 0 #There were No Turns detected but 1 frame- Handle Exception
  
  
  datTrackletTurns <- data.frame(turnID=names(turnEndFrame),turnStartFrame,turnEndFrame,turnMagnitude)
  datTrackletTurns <- datTrackletTurns[!is.na(datTrackletTurns$turnMagnitude), ]
  orderSeq <- order(as.numeric(as.character(datTrackletTurns$turnID)) ) ##Fix Turn Order 
  datTrackletTurns <- datTrackletTurns[orderSeq,]
  
  
  #plot(as.numeric(TrackLetInfo$startFrame)+1:length(vTurnSpeed), vTurnSpeed)
  #points(TurnboutsIdx+as.numeric(TrackLetInfo$startFrame),vTurnSpeed[TurnboutsIdx],col="red")
  
  ## Filter Turns And Correct Frame Indicators for turns
  datDomainTrajectory$IndTurnBout <- 0
  for (tID in as.character(datTrackletTurns$turnID) )
  {
    if (abs(datTrackletTurns[tID,]$turnMagnitude) >  MIN_TURN_SIZE_THRES  )
    {
      message("Accept ",tID,":",datTrackletTurns[tID,]$turnMagnitude)
      datDomainTrajectory[datDomainTrajectory$frameN >= datTrackletTurns[tID,]$turnStartFrame &
                       datDomainTrajectory$frameN <= datTrackletTurns[tID,]$turnEndFrame,"IndTurnBout"] = as.numeric(tID)
    }else
    {
      message("Reject ",tID,":",datTrackletTurns[tID,]$turnMagnitude)
      datTrackletTurns <- datTrackletTurns[datTrackletTurns$turnID != tID,]
    }
  ...
  TurnboutsIdx <- which(datDomainTrajectory$IndTurnBout > 0)
  #-------  Process Straight Runs Motion  ---------#
  ##Exclude The Turns Add the Tail Frequency to detect motion
  MoveboutsIdx <- 1:NROW(datDomainTrajectory)
  MoveboutsIdx  <- c(MoveboutsIdx[!(MoveboutsIdx %in% TurnboutsIdx) ]) ###c(TailboutsIdx, MoveboutsIdx,TurnboutsIdx ) ##
  ...
  datDomainTrajectory$IndMotionBout <- 0
  if (length(MoveboutsIdx))
    datDomainTrajectory[MoveboutsIdx,]$IndMotionBout <- 1

  blockMotionBoutFramesIDs <- split(MoveboutsIdx+as.numeric(TrackLetInfo$startFrame), 
                                       ##Sum Increments When the following Indicators Of Changing Class Become TRUE (I added Track Splitters when Exp or Event Change)
                                 cumsum(c(1, diff(MoveboutsIdx) != 1 )  ) 
                                )
    
  ##Here I should Extract the Straight Movements Only Not Turns
  datMoveIds <- ldply (blockMotionBoutFramesIDs, data.frame)
  names(datMoveIds) <- c("motionID","frameN")
  motionStartFrame <- tapply(datMoveIds$frameN,datMoveIds$motionID,min)
  motionEndFrame <- tapply(datMoveIds$frameN,datMoveIds$motionID,max)
  motionMagnitude <- vPathLength_mm[(motionEndFrame) - as.numeric(TrackLetInfo$startFrame)   ] - vPathLength_mm[(motionStartFrame) - as.numeric(TrackLetInfo$startFrame)+ 1 ]
  datTrackletMotion <- data.frame(motionID=names(motionEndFrame),motionStartFrame,motionEndFrame,travelDistance_mm=motionMagnitude,stringsAsFactors=FALSE)
  datTrackletMotion <- datTrackletMotion[!is.na(datTrackletMotion$travelDistance_mm), ]
  ##Set to correct order
  orderSeq <- order(as.numeric(as.character(datTrackletMotion$motionID)) )
  datTrackletMotion <- datTrackletMotion[orderSeq,]
  
  ### PLOT Trajectory Results ##
  #plot(datDomainTrajectory$posX,datDomainTrajectory$posY,
  #     col=c("purple","black","red")[datDomainTrajectory$IndMotionBout+1+(datDomainTrajectory$IndTurnBout > 0)*2],
  #     pch =c(25,20,3)[datDomainTrajectory$IndMotionBout+1+(datDomainTrajectory$IndTurnBout > 0)*2],
  #     cex=c(1,1,1.2)[datDomainTrajectory$IndMotionBout+1+(datDomainTrajectory$IndTurnBout > 0)*2] )  
  
  
  
  ##Add The Frame Duration of Each Mode -Run-Length Encoding (RLE) Between Runs and Turns
  return(list(datTurns=datTrackletTurns,datRuns=datTrackletMotion,Runs_rle=rle(datDomainTrajectory$IndMotionBout),fps=head(datDomainTrajectory$fps,1)  ) )
}
  ...
  {   
      ...
      for (strGroup in names(lBoundedTrackInfo) )
      {
        ...
        #TrackLetInfo <- lBoundedTrackInfo$LL["228-1-11",]
      ...
      saveRDS(lMeanPathLengthAll,file=paste0(strDataExportDir,"/stat_BOUT_boundedPathsB_R",DomainRadius,"mm.rds"))
  }##For Each Domain Radius
  
   for (TrackLetInfo in lBoundedTrackInfo[[strGroup]])
     message(TrackLetInfo)

  
  
```

```{r  analyse bout structure within domains, fig.show="hold",fig.cap="Distribution of free paths (Straight runs before turning)", out.width="100%", echo=FALSE, cache=TRUE, results=FALSE}

plotBoundedFreePath <- function(datBoundedTrack,strG,addPlot=FALSE)  
{
   mxbreaks=seq(0,30,1)
   strTitle <- paste("free path R",DomainRadius)
 
  hist_D <- hist(unlist(datBoundedTrack$travelDistance_mm),breaks=mxbreaks, plot=FALSE )  
  XLIM <- c( min(log10(hist_D$mids),na.rm=TRUE), max(log10(hist_D$mids), na.rm=TRUE)*1.01)
  if (!addPlot)
  {
  plot(log10(hist_D$mids),log10( (100*hist_D$counts)/sum(hist_D$counts) ) , 
  ...
  ### Analyse Free Paths - Look at distribution Run lengths between turns 
  DomainRadius <- 6

  lMeanPathLengthAll <- readRDS(file=paste0(strDataExportDir,"/stat_BOUT_boundedPathsB_R",DomainRadius,"mm.rds") )
  lBoundedTrackInfo <- readRDS(file=paste0(strDataExportDir,"/stat_boundedPathsB_R",DomainRadius,"mm.rds") )
  ...
  for (strGroup in names(lBoundedTrackInfo) )
  {
    ...
    for (trackID in rownames( groupdomainTracks ) )
    {
      if (is.null(lMeanPathLengthAll[[trackID]]))
        ...
        n = NROW(lMeanPathLengthAll[[trackID]]$datRuns)
        ##Remove Rounding <0 errors
        lMeanPathLengthAll[[trackID]]$datRuns$travelDistance_mm <- lapply(lMeanPathLengthAll[[trackID]]$datRuns$travelDistance_mm,max,c(0))
        lRundf[[trackID]] <- cbind.data.frame(lMeanPathLengthAll[[trackID]]$datRuns, 
        ...
        n = NROW(lMeanPathLengthAll[[trackID]]$datTurns)
        lTurndf[[trackID]] <- cbind.data.frame(lMeanPathLengthAll[[trackID]]$datTurns, 
                                      DomainRadius =rep(groupdomainTracks$DomainRadius[[trackID]],n ) ,
                                      expID=rep(groupdomainTracks$expID[[trackID]],n),
                                      EventID=rep(groupdomainTracks$EventID[[trackID]],n),
                                      TrackletUID=rep(trackID,n), ##groupdomainTracks$TrackletUID[[
                                      groupID=rep(strGroup,n) )

      
    }
  }
  datFreePathPerDomain <- do.call(rbind.data.frame,lRundf)
  datTurnsPerDomain <- do.call(rbind.data.frame,lTurndf)
  
  
  
  cD <- as.character(DomainRadius) 
  ldatFreePathD <- list()
  ldatFreePathD[[cD ]] <- datFreePathPerDomain[datFreePathPerDomain$DomainRadius == cD ,]
  
  ##Split Path Lengths Per Group
  freePathDat <- list()
  for (strG in vGroups)
      freePathDat[[strG]] <- ldatFreePathD[[cD ]][ldatFreePathD[[cD ]]$groupID == strG & ldatFreePathD[[cD ]]$travelDistance_mm < 30,]
  
  
  ## 
  #hist(unlist(freePathDat[["LL"]]$travelDistance_mm), breaks=seq(-1,30,1),ylim=c(0,700),xlab="Free Path Length (Straight line)" )
  #hist(unlist(freePathDat[["LE"]]$travelDistance_mm), breaks=seq(-1,30,1),ylim=c(0,700) )
  #hist(unlist(freePathDat[["NL"]]$travelDistance_mm), breaks=seq(-1,30,1),ylim=c(0,700) )
  #hist(unlist(freePathDat[["NE"]]$travelDistance_mm), breaks=seq(-1,30,1),ylim=c(0,700) )
  #hist(unlist(freePathDat[["DL"]]$travelDistance_mm), breaks=seq(-1,30,1),ylim=c(0,700) )
  #hist(unlist(freePathDat[["DE"]]$travelDistance_mm), breaks=seq(-1,30,1),ylim=c(0,700) )

   addPlot <- FALSE
  for (strG in vGroups)
  {
   datBoundedTrack <- freePathDat[[strG]]  
   plotBoundedFreePath(datBoundedTrack,strG,addPlot)
   addPlot <- TRUE
  }
  
```
Suprisingly, no difference between the distributions of free-paths (lengths of straigh runs before a turn), can be seen between groups, for a domain radius of 10mm
...
```{r path-turn-frequency per domain size, out.width="100%", echo=FALSE, cache=FALSE, results=FALSE}
  ...
  for (strG in vGroups)
  {
      turnsPerPath[[strG]] <- with(datTurnsPerDomain[datTurnsPerDomain$groupID == strG & datTurnsPerDomain$turnMagnitude > MIN_TURN_SIZE_THRES,],{
      as.data.frame( tapply(TrackletUID,TrackletUID, NROW), col.names = c("TurnCount")) ##COUNT Turns Pewr
    })
      names(turnsPerPath[[strG]]) <- ("TurnCount")
  }
  ...
  # Number of Turns Per Path/tracklet crossing domain
  hist(turnsPerPath[["LL"]]$TurnCount, breaks=seq(0,35,1),ylim=c(0,200),xlim=c(0,20))
  hist(turnsPerPath[["LE"]]$TurnCount, breaks=seq(0,35,1),ylim=c(0,200),xlim=c(0,20))
  
  hist(turnsPerPath[["NL"]]$TurnCount, breaks=seq(0,35,1),ylim=c(0,200),xlim=c(0,20))
  hist(turnsPerPath[["NE"]]$TurnCount, breaks=seq(0,35,1),ylim=c(0,200),xlim=c(0,20))
  
  hist(turnsPerPath[["DL"]]$TurnCount, breaks=seq(0,35,1),ylim=c(0,200),xlim=c(0,20))
  hist(turnsPerPath[["DE"]]$TurnCount, breaks=seq(0,35,1),ylim=c(0,200),xlim=c(0,20))
