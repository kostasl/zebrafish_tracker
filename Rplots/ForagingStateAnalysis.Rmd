...
plotBoundedFreePath <- function(datBoundedTrack,strG,addPlot=FALSE,XLIM=20)  
{
 ...
 ### Mean Free Path
  ...
  for (R in names(ldatFreePathsPerDomain) )
  {
    ...
    for (strG in vGroups)
    {
        ...
        datFreePathsOfGroupInDomain <- ldatFreePathsPerDomain[[R]][ldatFreePathsPerDomain[[R]]$groupID == strG & ldatFreePathsPerDomain[[R]]$freePathLength_mm > 0,]
 ...
 ## Distribution of Free Path Lengths For Domain Size ##
   ...
   cD <- as.character(DomainRadius) 
   addPlot <- FALSE
   datfreePathPerGroup <- data.frame(cbind(ldatFreePathsPerDomain[[cD ]]$freePathLength_mm, ldatFreePathsPerDomain[[cD ]]$groupID) )
