...
for (strG in as.character(unique(datMeanPathLengthAll$groupID) ) )
{
  ...
  if (!bAddPlot)
  {
    plot(datGroupMeanPathLength$DomainRadius,datGroupMeanPathLength$count/sum(datGroupMeanPathLength$count),ylim=c(0,0.5),xlim=c(0,11),
...
```{r  extract bout and turn structure from domain paths, fig.show="hold",fig.cap="Bout Frequency and free paths", out.width="100%", echo=FALSE, cache=TRUE, results=FALSE}
...
getTrajectoryTurnsAndRuns<- function(TrackLetInfo)  
{
 ...
 #### PROCESS BOUTS ###
  ...
  {   
      ...
      saveRDS(lMeanPathLengthAll,file=paste0(strDataExportDir,"/stat_BOUT_boundedPaths_R",DomainRadius,"mm.rds"))
...
```{r  analyse bout structure within domains, fig.show="hold",fig.cap="Bout Frequency and free paths", out.width="100%", echo=FALSE, cache=TRUE, results=FALSE}
    ...
    for (trackID in rownames( groupdomainTracks ) )
    {
      if (is.null(lMeanPathLengthAll[[trackID]]))
        ...
        lRundf[[trackID]] <- cbind.data.frame(lMeanPathLengthAll[[trackID]]$datRuns, 
                                              ...
                                              TrackletUID=rep(groupdomainTracks$TrackletUID[[trackID]],n),
  ...
  ldatFreePathD <- list()
  ldatFreePathD[["10"]] <- datFreePathPerDomain[datFreePathPerDomain$DomainRadius == "10",]
  
  freePathDat <- list()
  freePathDat[["LL"]] <- ldatFreePathD[["10"]][ldatFreePathD[["10"]]$groupID == "LL" & ldatFreePathD[["10"]]$travelDistance_mm < 30,]
  freePathDat[["LE"]] <- ldatFreePathD[["10"]][ldatFreePathD[["10"]]$groupID == "LE" & ldatFreePathD[["10"]]$travelDistance_mm < 30,]
  
  freePathDat[["NL"]] <- ldatFreePathD[["10"]][ldatFreePathD[["10"]]$groupID == "NL" & ldatFreePathD[["10"]]$travelDistance_mm < 30,]
  freePathDat[["NE"]] <- ldatFreePathD[["10"]][ldatFreePathD[["10"]]$groupID == "NE" & ldatFreePathD[["10"]]$travelDistance_mm < 30,]
  
  hist(unlist(freePathDat[["LL"]]$travelDistance_mm), breaks=seq(-1,30,1) )
  hist(unlist(freePathDat[["LE"]]$travelDistance_mm), breaks=seq(-1,30,1) )
  hist(unlist(freePathDat[["NL"]]$travelDistance_mm), breaks=seq(-1,30,1) )
  hist(unlist(freePathDat[["NE"]]$travelDistance_mm), breaks=seq(-1,30,1) )
