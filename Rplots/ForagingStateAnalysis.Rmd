...
calcPathLengthsInDomain <- function(datAllFrames,vgroupID,DomainRadius)
{
  ...
  ##split(datDomainTrajectories$frameN    ,cumsum(c(1, datDomainTrajectories$DnextFrame != 1)) ) <- Label Consecutive Groups
  ...
  entryFramePerLarvaEvent <- tapply(datBoundaryDomainTrajectories$frameN,
                                    paste0(datBoundaryDomainTrajectories$expID,"-",datBoundaryDomainTrajectories$eventID,"-",datBoundaryDomainTrajectories$trackletID),min )
  exitFramePerLarvaEvent <- tapply(datBoundaryDomainTrajectories$frameN,
                                   paste0(datBoundaryDomainTrajectories$expID,"-",datBoundaryDomainTrajectories$eventID,"-",datBoundaryDomainTrajectories$trackletID),max )
  
  pathDurations <- (exitFramePerLarvaEvent - entryFramePerLarvaEvent)
  pathDurations <- pathDurations[!is.na(pathDurations)]

  #hist(pathDurations/410,breaks=100,main=vgroupID)
  ...
  for (strTrackID in names(pathDurations) )
  {
        ...
        TrackletInfo <- list(expID = vTrackID[[1]][1],EventID=vTrackID[[1]][2],TrackletID=vTrackID[[1]][3], DomainRadius = rDomainRadius*DIM_MMPERPX)
