...
mergeMaxLarvalDispersionPerLarvaOntoDispersion <- function(datDispersion,datExpPairs)
{
  ...
  datMaxDispersionPerLarva <- cbind.data.frame(expID=as.character(datDispesionMax_Pairs$expID),max_Dispersion=(datDispesionMax_Pairs$max) )
...
## Merge Max Dispersion on there - If it is not already there
if (! any(grepl("max_Dispersion",names(datDispersion)) ) )
...
hist(datDispersion[datDispersion$groupID %in% c('LE','LL'), ]$Dispersion/datDispersion[datDispersion$groupID %in% c('LE','LL'), ]$max_Dispersion, breaks=100,main="LF S+E ",xlab="Dispersion  (norm per larva)")
hist(datDispersion[datDispersion$groupID %in% c('NE','NL'), ]$Dispersion/datDispersion[datDispersion$groupID %in% c('NE','NL'), ]$max_Dispersion, breaks=100,main="NF S+E",xlab="Dispersion  (norm per larva)")
hist(datDispersion[datDispersion$groupID %in% c('DE','DL'), ]$Dispersion/datDispersion[datDispersion$groupID %in% c('DE','DL'), ]$max_Dispersion, breaks=100,main="DF S+E",xlab="Dispersion  (norm per larva)")
