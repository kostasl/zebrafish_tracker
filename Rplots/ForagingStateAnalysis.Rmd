...
```{r path-lenght-within-domain calculations , fig.show="hold", out.width="50%", echo=FALSE, cache=TRUE }
...
#  saveRDS(lBoundedTrackInfo,file=paste0(strDataExportDir,"/stat_boundedPaths_R",DomainRadius,"mm.rds") )
  ...
  plot(datMeanPathLengthAll$DomainRadius,datMeanPathLengthAll$muPathLength,ylim=c(0,18),xlim=c(0,12),
  ...
  ## Make PLot All groups, Add theoretical Bound  
  plot(datMeanPathLengthAll$DomainRadius,datMeanPathLengthAll$muPathDuration,ylim=c(0,18),xlim=c(0,10),
       col=unlist(colourDataScheme[ as.character(datMeanPathLengthAll$groupID) ]),
...
The *invariance principle holds*, with mean path length being simply linearly related to the geometrical size of the domain:
...
Note that tracker ROI is `r format(259*DIM_MMPERPX,digits=3)` mm in diameter, and its circular boundary is approximatelly `r format((DIM_PXDIAMETER-259*2)*DIM_MMPERPX/2,digits=3)` mm away from the dish's wall. Yet, already from a 10mm domain radius (ie ~7mm away from boundary), we find the mean path length falls below the theoretical, indicating that the distribution of path lengths has been modified towards shorter paths, *especially so for the LE group, this is evident from R=7mm onwards.*
...
mxbreaks <- seq(0,100,1)
hist_D <- hist(datBoundedTrack$pathLength,breaks=mxbreaks, plot=FALSE )  
plot(hist_D$mids,hist_D$counts)

 #hist_LL <- hist(unlist(lBoundedTrackInfo[["LL"]]$duration),breaks=seq(0,65,1))
 #hist_LE <- hist(unlist(lBoundedTrackInfo[["LE"]]$duration),breaks=seq(0,65,1))

 #plot(hist_LL$counts,hist_LL$mids,log="xy")
 #lines(hist_LE$counts,hist_LE$mids,log="xy",type="b")
...
```{r distribution of number of paths per domain sizes,fig.show="hold", out.width="50%", echo=FALSE, cache=TRUE}  
...
```
  ...
  I found that *across groups evoked conditions have longer overall paths than spontaneous.* The fitted exponents show evoked condintions having $\mu>2.2$ while spontaneous conditions have $\mu<1.8$, likely because of long tails in the distributions of evoked being caused by engaging with prey.
