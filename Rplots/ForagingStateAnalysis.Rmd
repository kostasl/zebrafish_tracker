...
vDispersion.LF.S <- sample(datDispersion[datDispersion$groupID == 'LE' & !is.na(datDispersion$Dispersion),"Dispersion",],nsubsample)
vDispersion.NF.S <- sample(datDispersion[datDispersion$groupID == 'NE' & !is.na(datDispersion$Dispersion),"Dispersion",],nsubsample)
vDispersion.DF.S <- sample(datDispersion[datDispersion$groupID == 'DE' & !is.na(datDispersion$Dispersion),"Dispersion",],nsubsample)
...
plotClusterModel <- function(results,vDispersion,groupID)
{
  ...
  clustID <- round(colMeans(custcoeff[[1]]))

    #hist( mucoeff[[3]],xlim=c(0,10), breaks=20,col=colourDataScheme[[groupID]],main=groupID,xlab = "Sample Means (mm)" )  
  tauClustAllChain <- as.data.frame(tauOfClustcoeff[[1]])#rbind( as.data.frame(tauOfClustcoeff[[3]]),as.data.frame(tauOfClustcoeff[[2]]),as.data.frame(tauOfClustcoeff[[1]]))
  muClustAllChain <- as.data.frame(muOfClustcoeff[[1]])#rbind( as.data.frame(muOfClustcoeff[[3]]),as.data.frame(muOfClustcoeff[[2]]),as.data.frame(muOfClustcoeff[[1]]) )
  clustCol <- c("red","blue")
  ...
  #hist(muClustAllChain$`muOfClust[1]`,xlim=c(0,10), breaks=10,col=clustCol[1],main=paste("Cluster means", groupID ),xlab = "Estimated means of each cluster (mu)")
  #hist(muClustAllChain$`muOfClust[2]`,xlim=c(0,10), breaks=10,col=clustCol[2],main=groupID,xlab = NA,add=TRUE)  
  ...
  hist( pClustcoeff[[3]][,1],xlim=c(0,1), breaks=10,col=clustCol[1],main=groupID,xlab = "Prob. of cluster membership (pClust)" )
  hist( pClustcoeff[[3]][,2],xlim=c(0,1), breaks=10,col=clustCol[2],main=groupID,add=TRUE )
...
Next we plot the clustering results from nsampled dispersion data from each group/ condition

```{r dispersion-clustering-results,fig.show="hold", out.width="33%", eval=FALSE, include=FALSE}
##plot Cluster Dist
## Evoked
...
##Spont
plotClusterModel(results.NF.S,vDispersion.NF.E,"NE")
plotClusterModel(results.LF.S,vDispersion.LF.E,"LE")
plotClusterModel(results.DF.S,vDispersion.DF.E,"DE")
