...
## Hierarichal clustering model to reveal difference in larval behaviour
...
## Relationship of hunt-outcome to *Globally clustered foraging state*
...
```{r load-results-NBGlobal-clustered-verify, fig.show="hold", out.width="33%", echo=FALSE, cache=TRUE }
...
plotClusteredHuntEventsHist_norm <- function(datHEventDispersion,clustCol)
{
  ...
  h1 <- hist(datHEventDispersion[datHEventDispersion$clustID == 1,"Dispersion_larval_norm" ], breaks=breaksSlots, plot=FALSE) #xlim=c(0,10),ylim=c(0,max(h0$counts))
  h2 <- hist(datHEventDispersion[datHEventDispersion$clustID == 2,"Dispersion_larval_norm" ], breaks=breaksSlots, plot=FALSE)  #xlim=c(0,10),
...
barplot(table(datHEventDispersion$clustID,datHEventDispersion$groupID),col=c(colourClusters[2],colourClusters[1]),
        main="Hunt-events per cluster and cond-group")

## Sanity Check The cluster with Most Hunt Events (Appears to Be )
boxplot(Dispersion ~ clustID*groupID,data=datHEventDispersion,col=c(colourClusters[2],colourClusters[1]),  main="hunt-event dispersion per cluster",ylab="Dispersion (mm)")
legend("topright",legend=(paste("#",table(datHEventDispersion$clustID) )),fill=c(colourClusters[2],colourClusters[1])  )

## Sanity Check The cluster with Most Hunt Events (Appears to Be )
boxplot(Dispersion_larval_norm ~ clustID*groupID,data=datHEventDispersion,col=c(colourClusters[2],colourClusters[1]),ylab="Normalized Dispersion per larva" )
legend("topright",legend=(paste("#",table(datHEventDispersion$clustID) )),fill=c(colourClusters[2],colourClusters[1])  )
...
```{r results-hunt-events dispersion- global clustered, fig.show="hold", out.width="50%", echo=FALSE, cache=TRUE }


plotClusteredHuntEventsHist(datHEventDispersion,c(colourClusters[2],colourClusters[1]))
plotClusteredHuntEventsHist_norm(datHEventDispersion,c(colourClusters[2],colourClusters[1]))
...
plotClusteredHuntEventsHist(datHEventDispersion[datHEventDispersion$groupID=='NL' &
                      ...
                      c(colourClusters[2],colourClusters[1]) ) 
