...
calcPathLengthsInDomain <- function(datAllFrames,vgroupID,DomainRadius)
  ...
  for (DomainRadius in vAvailableDomainRadius)
  {
      ...
      {
          lMeanPathLengthAll[[paste0(strGroup,DomainRadius)]] <- list(muPathLength=mean(unlist(lBoundedTrackInfo[[strGroup]]$pathLength) ),
                                   sdPathLength=sd(unlist(lBoundedTrackInfo[[strGroup]]$pathLength) ),
                                   muPathDuration=mean(unlist(lBoundedTrackInfo[[strGroup]]$pathLength) ),
                                   sdPathDuration=mean(unlist( lBoundedTrackInfo[[strGroup]]$pathLength) ),
                                   count=(NROW(lBoundedTrackInfo[[strGroup]]) ),
                                   groupID=strGroup,
  ...
  plot(datMeanPathLengthAll$DomainRadius,datMeanPathLengthAll$muPathLength,ylim=c(0,10),xlim=c(0,10),
       col=unlist(colourDataScheme[ as.character(datMeanPathLengthAll$groupID) ]),
       pch=unlist(pointTypeScheme[ as.character(datMeanPathLengthAll$groupID) ]))
...
hist(unlist(lBoundedTrackInfo[["NE"]]$pathLength),breaks=seq(0,60,1),ylim=c(0,50),main=paste("NE",mean(unlist(lBoundedTrackInfo[["NE"]]$pathLength))  ) )
