...
vtimeWindows <- c(vsmallTimeWindows,80,90,100,120,180)# c(1,2,4) # vsmallTimeWindows##c(4,10,20,80,200,260,360)
...
```{r mean-squared-distance over time , fig.show="hold", out.width="100%", echo=FALSE, cache=FALSE}
...
vLargeScaletimeWindows <- c(2,10,40,80,120,180,200,260) #360 No Data
vtimeWindows <- c(vsmallTimeWindows,70,80,90,100,120,180)# c(1,2,4) # vsmallTimeWindows##c(4,10,20,80,200,260,360)
##Process *  Serially * So we do not run out of memory
# Results are plotted and Aggregated In lLinModel
rm(lMSDSummaryData)
try(
lMSDSummaryData <<- readRDS(paste0(strDataExportDir,"MSDPerPathDurationSummaryResults-1-180sec.rds"))
)
if (!exists("lMSDSummaryData"))
{
  lMSDSummaryData <<- makeMSDPathDurationSummary()
...
```{r mean-squared-distance vs Dispersion , fig.show="hold", out.width="100%", echo=FALSE, cache=FALSE}
...
```{r  mean-squared-distance over Dispersion , fig.show="hold", out.width="100%", echo=FALSE, cache=TRUE}
## iNvariance Principle Theoretical Mean Path Length
theoreticalMeanLength <- function(D,propFactor=1.5)
{
  A <- pi*( (D)^2)
