...
plotClusterProb <- function(lmcmcRes,clustCol)
{
  ...
  list_histo <- hist(lmcmcRes$pClustcoeff[[lmcmcRes$chain]],breaks=breaksProb,main=groupID,freq = TRUE,plot=FALSE )
  h1<-hist( lmcmcRes$pClustcoeff[[lmcmcRes$chain]][,1],ylim=c(0,max(list_histo$counts)), breaks=breaksProb,plot=FALSE) ##Cluster 1 
  h2 <- hist( lmcmcRes$pClustcoeff[[lmcmcRes$chain]][,2],xlim=c(0,1), breaks=breaksProb,plot=FALSE ) ## ##Cluster 1 
  
  ##Colour Determines which Cluster is fast and slow
   barplot(rbind(h1$density,h2$density),beside = FALSE, col = c(clustCol[1],clustCol[2],"white"),names.arg = (h1$breaks[-length(h1$breaks)]),main=paste("Clustered Occupancies", lmcmcRes$groupID ),xlab = "Prob. of cluster membership (pClust)")
