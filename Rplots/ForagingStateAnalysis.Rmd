...
plot_meanPathLengthPerDispersion <- function(datDispersion, strGroupID, addPlot=FALSE, propFactor=1.0)
{
  ...
  arrows(x0=rpoints, y0=vmeanPathLengthPerDispersion-vSEPathLengthPerDispersion, x1=rpoints, y1=vmeanPathLengthPerDispersion+vSEPathLengthPerDispersion, code=3, angle=90, length=0.02, col=colourDataScheme[[strGroupID]], lwd=1)
...
## Path Durations within fixed domain R

```{r,  fig.show="hold",fig.cap="Distribution of path durations crossing circular domain with 20mm Diameter", out.width="100%", echo=FALSE, cache=FALSE,fig.show="hold", out.width="33%" } 
## Use the Fixed Domain Data Paths to examine Durations distribution for fat-tails  
  DomainRadius <- 10
  lBoundedTrackInfo <- readRDS(file=paste0(strDataExportDir,"/stat_boundedPathsB_R",DomainRadius,"mm.rds") )
  for (strG in vGroups[seq(1,6,2)])
  { 
    hist_Dur <- hist(unlist(lBoundedTrackInfo[[strG]]$duration),breaks=seq(0,175,3), plot=FALSE )
    ##Plot Path Duration Distribution 
    plot(hist_Dur$mids,hist_Dur$density,log="xy",main=paste(strG," domain R",DomainRadius," path duration" ), xlim=c(1,250),
