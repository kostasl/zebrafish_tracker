...
plotClusteredDispersionHist <- function(lmcmcRes,vDispersion,clustCol)
{
  ...
  h1 <- hist(vDispersion[lmcmcRes$clustID == 1], breaks=breaksSlots,col=clustCol[1],plot=FALSE) #xlim=c(0,10),ylim=c(0,max(h0$counts))
  h2 <- hist(vDispersion[lmcmcRes$clustID == 2], breaks=breaksSlots,col=clustCol[2],main=groupID,xlab = NA,plot=FALSE)  #xlim=c(0,10),
...
, thereby looking at the size of dispersal generated during Xsec as of a sum of independent bernoulli random variables. Each dispersal size amounts to a fixed size sequence in which $e$ steps produced expansion and $k$ did not increase dispersion (they were inward for example).
...
## Extracts the monitor values and returns them packaged in a list
getMCMCNBEstimatedParams <- function(results,groupID,ichain = 2)
{
  clustcoeff = as.mcmc.list(results, vars="clust")
  ...
  q = q[[ichain]],
  r = r[[ichain]],
  mean = q[[ichain]]*r[[ichain]]/(1-q[[ichain]]), ## Calc NB Mean
  sd = sqrt(q[[ichain]]*r[[ichain]]/(1-q[[ichain]])^2), ## Calc NB Var
  pClustcoeff = as.mcmc.list(results, vars="pClust"),
  clustID = round(colMeans(clustcoeff[[ichain]]) ) 
  )
...
plotNBFitcdf <- function(vDispersion,resParams)
{
  ...
  for (c in 1:2) {
    for (i in (NROW(resDF.S$q[,1])-ntail):NROW(resDF.S$q[,1]) )
    {
      ...
      lines(x,cumsum(cdfM),col=colourClusters[c],lty=1) #add=TRUE,
    }
  }
  ##Model AND Data Densities
  c<-1
  plot((density(dnbinom(x,size=mean(resParams$r[,c] ), prob= mean(resParams$q[,c]))  ) ),col=colourClusters[c],lwd=2,main="NB Model and data distributions")
  lines(density(vDispersion[resParams$clustID == c]*10), col=colourClusters[c],lty=2,lwd=2)
  
  c<-2
  lines(density(dnbinom(x,size=resParams$r[,c],prob= resParams$q[,c] )),col=colourClusters[c],lwd=2)
  lines(density(vDispersion[resParams$clustID == c]*10),col=colourClusters[c],lty=2,lwd=2)
...
plotClusterModel(results.NF.S,vDispersion.NF.S, "NE",getMCMCNBEstimatedParams(results.NF.S,"NE",2))
plotNBFitcdf(vDispersion.NF.S, getMCMCNBEstimatedParams(results.NF.S,"NE",2))
```

```{r ,plot-NBMix-clustering-results-LF,fig.show='hold', out.width="33%",dev='png'}
plotClusterModel(results.LF.E,vDispersion.LF.E, "LL",getMCMCNBEstimatedParams(results.LF.E,"LL",2))
plotClusterModel(results.LF.S,vDispersion.LF.S, "LE",getMCMCNBEstimatedParams(results.LF.S,"LE",2))
```

```{r ,plot-NBMix-clustering-results-DF,fig.show='hold', out.width="33%",dev='png'}
plotClusterModel(results.DF.E,vDispersion.DF.E, "LL",getMCMCNBEstimatedParams(results.DF.E,"DL",2))
plotClusterModel(results.DF.S,vDispersion.DF.S, "LE",getMCMCNBEstimatedParams(results.DF.S,"DE",2))
```
