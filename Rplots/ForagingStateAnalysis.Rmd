...
*Main Findings so far :*
  ...
  21. **Bout Structure**:
    1. LF has Lower number of bouts per free-path
    2. LF has Longer distance traveled per bout - *longer bouts*
    3. Bout duration and distances decrease for LF,NF in evoked conditions, but not for DF.
    4. Lower number of bouts between turns (maybe).
    5. LF does not do long-pause/ Waits, in contrast to DF, NF.
    6. Lower mean number of turns when crossing Domain 10mm for DE, and then LL,DL, and then LE- NE has highest
    7. Increasing turn Modes around 40-50 deg, in spontaneous conditions, across groups. (Surprise as these turn modes are associated with Successful hunts)
    8. *Bout Speed increases in absence of prey*, with LF having the fastest bouts on average and DF the slowest.
...
```{r stat-Path-calculations , fig.show="hold", out.width="50%", echo=FALSE, cache=TRUE ,warning=FALSE}
...
for (strG in vGroups)
{
    vInterBoutInterval <- ldatBoutsPerDomain[[cD]][ldatBoutsPerDomain[[cD]]$groupID == strG & 
                             ldatBoutsPerDomain[[cD]]$boutSeqID == 0 &
                                ldatBoutsPerDomain[[cD]]$duration_sec < 50 ,"duration_sec"]
    xbreaks <- c(seq(0,0.1,0.02),seq(0.11,1,0.1),seq(1.5,50,1)) #,seq(10,60,1)
    ...
    message(paste("Inter-Bout Interval ",strG," mu:",mean(vInterBoutInterval) ))
    if (!addPlot){
...
title("Inter-bout interval duration")
...
for (strG in vGroups)
{
 ...
 ##Turn Magnitude distributions
  ...
  for (strG in vGroups)
  {
    ...
    message(strG," Mean turn when crossing domain R",DomainRadius," mu: ",mean(datGTurns$turn_deg))
  }
...
boxplot(duration_sec ~ groupID,data=datPathBouts,main="Bout duration",
        col=c(colourDataScheme$LL, colourDataScheme$LE, colourDataScheme$NL, colourDataScheme$NE, colourDataScheme$DL, colourDataScheme$DE ),ylim=c(0,0.5) )
...
```{r bout-distance for fixed domain size, out.width="33%", echo=FALSE, cache=FALSE, results=FALSE}
#  MIN_TURN_SIZE_THRES
#CD is radius of domain
...
In all groups bout distances are shorter in evoked rather than spontaneous conditions. 
This agrees with spontaneous conditions having longer bout durations.
I need to verify that this applies to exploratory movements, and i is not simply due to hunt event bouts being shorter in evoked conditions when reacting to prey.

# Bout Speed

```{r bout-speed for fixed domain size, out.width="33%", echo=FALSE, cache=FALSE, results=FALSE}
#  MIN_TURN_SIZE_THRES
#CD is radius of domain
plotBoundedPathBoutSpeed <- function(ldatBoutsPerDomain,strG,addPlot=FALSE,cD="10")  
{
    ...
    xbreaks <- c(seq(0,20,1),seq(20.1,200,5)) #,seq(10,60,1)
    #scaling <- c(rep(1/1,11),rep(1/10,98)) #,seq(10,60,1) ## Because We are Changing the bin size - The fractrions needs top be scaled according to bin size-rations
    ...
    if (!addPlot){
        plot(hist_BS$mids,hist_BS$density,col=colourDataScheme[strG][[1]],pch=pointTypeScheme[strG][[1]],
             type="b",xlim=c(0.1,55),ylim=c(0.001,50),log="xy",
