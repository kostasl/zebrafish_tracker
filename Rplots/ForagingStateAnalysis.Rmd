...
#  datMoveIds <- ldply (blockMotionBoutFramesIDs, data.frame)
  ...
  {   
      ...
      saveRDS(lMeanPathLengthAll,file=paste0(strDataExportDir,"/stat_BOUT_boundedPathsB_R",DomainRadius,"mm2.rds"))
...
plotBoundedFreePath <- function(datBoundedTrack,strG,addPlot=FALSE)  
{
  ...
  for (strGroup in names(lBoundedTrackInfo) )
  {
    ...
    for (trackID in rownames( groupdomainTracks ) )
    {
      if (is.null(lMeanPathLengthAll[[trackID]]))
        ...
        lMeanPathLengthAll[[trackID]]$travelDistance_mm <- lapply(lMeanPathLengthAll[[trackID]]$travelDistance_mm,max,c(0))
        lRundf[[trackID]] <- cbind.data.frame(lMeanPathLengthAll[[trackID]], 
  ...
  ldatFreePathD <- list()
  ldatFreePathD[[cD ]] <- datBoutsPerDomain[datBoutsPerDomain$DomainRadius == cD ,]
...
datTurnsPerDomain <- datBoutsPerDomain
 ...
 ##Turn Magnitude
  for (strG in vGroups)
  {
    datGTurns<- datTurnsPerDomain[datTurnsPerDomain$groupID == strG & abs(datTurnsPerDomain$turn_deg) < 80 & abs(datTurnsPerDomain$turn_deg) > 10 &
...
*I do NOT see an apparent difference in turn-frequency distributions or the distribution of free-path lengths!* within the paths in crossing a bounded circular domain.
But that is strange cause the groups seem to macroscopically behave differently.
Nevertheless I discover that the turn distributions of spontaneous events show a mode-tendency towards larger turns, near the 35-45 degrees, such as that I have previously found 
is associated with sucessfull hunt events
