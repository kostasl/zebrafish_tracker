...
modelGCInd  <- "model
{
  ...
  for( i in 1 : N ) {
   #s[hidx[i],i] <- step( distP[i] - u1[hidx[i]] )*step(u0[ hidx[i] ] - distP[i]  ) * step(u0[ hidx[i] ] - distP[i]  )
    phi_hat[ hidx[i],i] <- phi_0[hidx[i]] -   lambda[ hidx[i] ] * pow(gamma[hidx[i]],u0[ hidx[i] ] - distP[i] )   
...
limDist <- max(distMax)
 for(i in 1:max(hidx) ) { 
    ...
    u0[i] ~ dunif(u1[i], 2*limDist) ##Start Hunt Distance -Far 
...
plotGCRes <- function (drawS,dataSubset,n=NA,groupID){
  ...
  ## compute 2D kernel density, see MASS book, pp. 130-131
   ...
   # X11()
    ...
    vX  <- seq(0,max(drawS$u0[vsampleP]),by=0.01)
    vY  <-    median(drawS$phi_0[vsampleP] ) - ( median(drawS$lambda[vsampleP]))*((median(drawS$gamma[vsampleP])^( median(drawS$u0[vsampleP] ) - (vX) ) ) ) # 
    ## gamma quantiles 
    vY_u <-  median(drawS$phi_0[vsampleP])-(median(drawS$lambda[vsampleP]))*((quantile(drawS$gamma[vsampleP])[4]^( median(drawS$u0[vsampleP]) - (vX) ) ) )
    vY_l <-  median(drawS$phi_0[vsampleP])-(median(drawS$lambda[vsampleP]))*((quantile(drawS$gamma[vsampleP])[2]^( median(drawS$u0[vsampleP]) - (vX) ) ) )
    ...
    for (pp in vsampleP)
    {
      ...
      vX  <- seq(0,drawS$u0[pp],by=0.01)
      vY    <- (drawS$phi_0[pp] ) - ( (drawS$lambda[pp]))*(((drawS$gamma[pp])^( drawS$u0[pp] - (vX) ) ) ) #
      vY_l  <- quantile(drawS$phi_0[pp,,])[2]   - ( (drawS$lambda[pp]) )*((( quantile(drawS$gamma[pp,,])[2] )^( quantile(drawS$u0[pp,,])[2] - (vX) ) ) ) #
      vY_u  <- quantile(drawS$phi_0[pp,,])[4]   - ( (drawS$lambda[pp]) )*((( quantile(drawS$gamma[pp,,])[4] )^( quantile(drawS$u0[pp,,])[4] - (vX) ) ) ) #
      ...
      #points(dataSubset$distP[vPP],dataSubset$phi[vPP],pch=19,xlim=c(0,5),ylim=c(-85,85),main=paste(strGroupID[groupID],pp), bg=colourP[groupID],col=colourP[groupID],cex=0.5)
...
writeLines(modelGCInd,fileConn);
close(fileConn)
