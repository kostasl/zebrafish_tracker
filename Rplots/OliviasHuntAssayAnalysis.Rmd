...
```{r, hunteventsperGroup, echo = FALSE}
  ...
  densHuntEventCounts <- list(HOM = density(datEventsPerExpID[datEventsPerExpID$groupID == "HOM",]$nHuntEvents),
                        HET = density(datEventsPerExpID[datEventsPerExpID$groupID == "HET",]$nHuntEvents),
                        WT  =  density(datEventsPerExpID[datEventsPerExpID$groupID == "WT",]$nHuntEvents)               
                        )
  BW = "20"
  plot(densHuntEventCounts$HOM,col=unlist(colourH["HOM"]), lwd=3,main="Distribution of Hunt events per group ")
  lines(densHuntEventCounts$HET,col=(colourH["HET"][[1]] ), lwd=3)
  lines(densHuntEventCounts$WT,col=unlist(colourH["WT"]), lwd=3)
  legend("topright",legend=paste(vGroupIDs),lwd=3,col=c(colourH[[vGroupIDs[1] ]], colourH[[vGroupIDs[2] ]], colourH[[vGroupIDs[3] ]] ))
...
mcmc_drawEventCountModels <- function(datHuntVsPrey,preyCntRange,strModelFilename)
{
  ...
  datSliceF <- datHuntVsPrey[datHuntVsPrey$nInitPrey >= preyCntRange[1] & datHuntVsPrey$nInitPrey <= preyCntRange[2], ]
  
  ##Select the event Count Column 2
  nEvents=as.numeric(datSliceF$nHuntEvents)
  nDat = length(nEvents);
  dataG=list(n=nEvents,NTOT=nDat,
             prey=as.integer(datSliceF$nInitPrey));
...
plotsamples <- 10
drawSamples_HOM <- mcmc_drawEventCountModels(datEventsPerExpID[datEventsPerExpID$groupID == "HOM",],preyCntRange,"modelGroupEventRate.tmp")
drawSamples_HET <- mcmc_drawEventCountModels(datEventsPerExpID[datEventsPerExpID$groupID == "HET",],preyCntRange,"modelGroupEventRate.tmp")
drawSamples_WT <- mcmc_drawEventCountModels(datEventsPerExpID[datEventsPerExpID$groupID == "WT",],preyCntRange,"modelGroupEventRate.tmp")
```
...
After checking model fit with CDF, then compare inferred huntrates per group
...
pBW = 10
densHPoissonRate_HOM <- density( HEventHuntGammaShape_HOM*1/HEventHuntGammaRate_HOM,bw=pBW)
densHPoissonRate_HET <- density( HEventHuntGammaShape_HET*1/HEventHuntGammaRate_HET,bw=pBW)
densHPoissonRate_WT <- density( HEventHuntGammaShape_WT*1/HEventHuntGammaRate_WT,bw=pBW)

##  Estimate  Hunt Rates Distributions ###
densHPoissonRate <- list(HOM= density( HEventHuntGammaShape_HOM*1/HEventHuntGammaRate_HOM,bw=pBW),
                         HET= density( HEventHuntGammaShape_HET*1/HEventHuntGammaRate_HET,bw=pBW),
                         WT= density( HEventHuntGammaShape_WT*1/HEventHuntGammaRate_WT,bw=pBW)
                         )

  Ylim <- 0.03
  plot(densHPoissonRate$HOM$x, densHPoissonRate$HOM$y,type='l',lty=lineTypeL[1],col=colourH["HOM"][[1]] ,lwd=4,ylab=NA,xlab=NA,xlim=c(0,350),ylim=c(0,Ylim),cex=cex,cex.axis=cex )
  ...
  mtext(side = 1,cex=cex, line = lineAxis, expression(paste("Estimated hunt rate (N/20min)") )  ) #(",lambda," )
  mtext(side = 2,cex=cex, line = lineAxis, " Density function ")
 title("Model inferred hunt rates ")
