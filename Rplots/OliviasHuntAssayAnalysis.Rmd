...
```{r preyDynamics, echo = FALSE, eval=TRUE}
  ...
  for (grpID in unique(datAllFrames$groupID) )
  {
    ...
    sdPreyCount[[grpID]] = apply(mat_y[[grpID]],2,sd,na.rm=T)
...
x <- 1:NROW(muPreyCount$WT)/(50*60)
plot(x,muPreyCount$WT,col=colourH$WT,type="l",xlim=c(1,18),xlab="Time (min)",ylab="Mean prey count",lwd=3,ylim=c(40,70))
...
x <- which(!is.na(muPreyCount$ROT))##Because Rot tracks have missing frames /recorded at intervals
lines(x/(50*60) ,meanf(muPreyCount$ROT[x],3500),col=colourH$ROT,type="l",lwd=3,cex=0.5)
...
lines(x,muPreyCount$HET,col=colourH$HET,type="l",lwd=3)
...
legend("topright",legend=c("WT","HET","HOM"), col=c(colourH$WT,colourH$HET,colourH$HOM),lty=1,lwd=2 )
...
vInitPreyPerExp <- apply(mat_y[["ROT"]][,1:2000],1,mean,na.rm=T)
vFinalPreyPerExp <- apply(mat_y[["ROT"]][,(nFrames-6000):(nFrames-5000)],1,mean,na.rm=T)
vNaturalPreyReduction <- vInitPreyPerExp-vFinalPreyPerExp
vNaturalPreyReduction <- vNaturalPreyReduction[!is.nan(vNaturalPreyReduction)]

datPreyReductionPerGroup <- rbind.data.frame(cbind(as.numeric(datHuntStat["WT",]$vHPreyReductionPerLarva),"WT"),
      cbind(as.numeric(datHuntStat["HET",]$vHPreyReductionPerLarva),"HET"),
      cbind(as.numeric(datHuntStat["HOM",]$vHPreyReductionPerLarva),"HOM"),
      cbind(as.numeric(vNaturalPreyReduction),"ROT"),
...
datPreyReductionPerGroup <- rbind.data.frame(cbind(as.numeric(datHuntStat["WT",]$vHPreyReductionPerLarva),"WT"),
      cbind(as.numeric(datHuntStat["HET",]$vHPreyReductionPerLarva),"HET"),
      cbind(as.numeric(datHuntStat["HOM",]$vHPreyReductionPerLarva),"HOM"),stringsAsFactors=FALSE
      )
datPreyReductionPerGroup$V1 <- as.numeric(datPreyReductionPerGroup$V1) #Convert to seconds 50 fps
names(datPreyReductionPerGroup)=c("PreyReductionPerLarva","Group")
