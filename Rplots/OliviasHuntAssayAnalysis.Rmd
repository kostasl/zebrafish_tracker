...
##Linear Regression 
lm.WT <- lm(x[6000:nFrames] ~ muPreyCount$WT[6000:nFrames])
abline(lm.WT,lwd=3,lty=2,col=colourH$WT)
lm.WT$effects
...
##Linear Regression 
lm.HET <- lm(x ~ muPreyCount$HET)
abline(lm.HET,lwd=3,lty=2,col=colourH$HET)
...
##Linear Regression 
lm.HOM <- lm(x ~ muPreyCount$HOM)
abline(lm.HOM,lwd=3,lty=2,col=colourH$HOM)
...
for (grpID in names(muPreyCount))
{
  ...
  legend("topright",
         legend= paste(grpID,"w=",format(lfit[[grpID]]$coefficients,digits=4),"r2=",format(summary(lfit[[grpID]])$r.squared,digits=4) ))
}
